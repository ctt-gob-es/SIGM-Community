...
public class Register extends FolderSession implements ServerKeys, Keys, HibernateKeys {
	...
	public static FolderDataSession createNewFolder(String sessionID, Integer bookID, AxSf axsfNew, List<?> inter, Map<?, ?> documents, Integer launchDistOutRegister, Locale locale, String entidad, boolean consolidacion) throws BookException, SessionException, ValidationException {

		FolderDataSession data = createFolderWithDocuments(sessionID, bookID, axsfNew, inter, documents, launchDistOutRegister, locale, entidad, consolidacion, false);
	 ...
	 */
	public static FolderDataSession updateFolder(String sessionID, Integer bookID, AxSf axsfNew, Integer folderId, List<?> inter, Map<?, ?> documents, Integer launchDistOutRegister, Locale locale, String entidad) throws BookException, SessionException, ValidationException {

		boolean updateDate = false;
		// se buscan en la config si se debe o no actualizar la fecha de trabajo (FLD4)
		Integer modifySystemDate = new Integer(BookSession.invesicresConf( entidad).getModifySystemDate());
		...
		return updateFolder(sessionID, bookID, folderId.intValue(), axsfNew, inter, documents, updateDate, launchDistOutRegister, locale, entidad);
	}
	...
	public static FolderDataSession importNewFolder(String sessionID, Integer bookID, AxSf axsfNew, List<?> inter, Map<?, ?> documents, Integer launchDistOutRegister, Locale locale, String entidad) throws BookException, SessionException, ValidationException {

		FolderDataSession data = createFolderWithDocuments(sessionID, bookID, axsfNew, inter, documents, launchDistOutRegister, locale, entidad, false, true);

		return data;
	}

	public static InfoRegister[] findFolder(String sessionID, Integer bookID, AxSfQuery axsfQuery, Locale locale, String entidad) throws BookException, SessionException, TecDocException, ValidationException {
		...
		AxSfQueryResults queryResults = RegisterServicesUtil .openRegistersQuery(sessionID, axsfQuery, bookIds, entidad);
		...
		if (queryResults != null) {
			...
			registersInfo = getRegistersInfo(sessionID, bookID, entidad, queryResults, user, scrofic);
	...
	public static AxSfQueryResults navigateRegistersQuery(String sessionID, AxSfQueryResults queryResults, AxSfQuery axsfQuery, String queryNavigationType, Locale locale, String entidad) throws BookException, SessionException, ValidationException {
		Validator.validate_String_NotNull_LengthMayorZero(sessionID, ValidationException.ATTRIBUTE_SESSION);
		...
		try {
			...
			Collection<?> col = axSfEntity.findByAxSfQuery(axsfQuery, axsf, queryResults, queryNavigationType, filter, entidad);
			...
			HibernateUtil.rollbackTransaction(tran);
			log.error("Impossible to execute the book query for [" + bookID + "] for the session [" + sessionID + "]", e);
			throw new BookException(BookException.ERROR_CANNOT_FIND_REGISTERS);
		} finally {
	...
	public static Map<?, ?> getValidationFields(AxSf axsf, String sessionID, Integer bookID, int page, Locale locale, String entidad) throws ValidationException, AttributesException, BookException, SessionException {
		
		List<Integer> list = getAttributesForValidationForForm(sessionID, bookID, axsf, page, entidad);

		Map<?, ?> result = new HashMap<Object, Object>();
		
		if (!list.isEmpty()) {
			Map<Integer, List<?>> fldids = new HashMap<Integer, List<?>>(list.size());
			Integer fldid = null;
			String fldName = null;
			...
			result = AttributesSession.getExtendedValidationFieldValues( sessionID, bookID, fldids, locale, entidad);
		}
	...
	private static List<Integer> getAttributesForValidationForForm(String sessionID, Integer bookID, AxSf axsf, int page, String entidad) throws ValidationException, AttributesException, BookException, SessionException {
		
		String data = axsf.getFormat().getData();
		FormFormat formFormat = new FormFormat(data);
		List<Integer> result = new ArrayList<Integer>();
		List<?> flds = AttributesSession.getExtendedValidationFieldsForBook( sessionID, bookID, entidad);
	...
	private static boolean existAttributeInQueryFormat(FormFormat ff, int fldid, int page) {
	 ...
	 */
	private static InfoRegister[] getRegistersInfo(String sessionID, Integer bookId, String entidad, AxSfQueryResults queryResults, AuthenticationUser user, ScrOfic scrOfic) throws BookException, SessionException, TecDocException {
		
		InfoRegister[] registersInfo = null;
		if ((queryResults != null) && (queryResults.getResults() != null) && (!queryResults.getResults().isEmpty())) {
			...
			for (Iterator<?> it = queryResults.getResults().iterator(); it.hasNext();) {
				...
				InfoRegister wsRegisterInfo = ConsultRegister.consultRegisterInfo(bookId, scrOfic, user, axsf, axsf.getLocale());
