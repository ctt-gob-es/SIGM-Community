package ieci.tecdoc.sgm.rpadmin.exception;

import ieci.tecdoc.sgm.core.services.rpadmin.RPAdminException;

import java.util.Locale;

public class RPAdminDAOException extends RPAdminException {
	...
	public static final long IUSER_NOT_FOUND_IN_LDAP = EXC_GENERIC_EXCEPCION+112;

	private static String RESOURCE_FILE = "ieci.tecdoc.sgm.rpadmin.exception.RPAdminDAOExceptionMessages";


	public RPAdminDAOException(long errorCode) {
		this(errorCode, null);
	}

	public RPAdminDAOException(String message) {
		this(message, null);
	}

	public RPAdminDAOException(Throwable cause) {
		this(0, cause);
	}

	public RPAdminDAOException(long errorCode, String message, Throwable cause){
		super(message,cause);
		this.errorCode = errorCode;
		errorMessage = loadMessage(Locale.getDefault());
	}

   public RPAdminDAOException(long errorCode, Throwable cause) {
	   super(cause);
	   this.errorCode = errorCode;
	   errorMessage = loadMessage(Locale.getDefault());
   }

   public RPAdminDAOException(String message, Throwable cause) {
	   super(message, cause);
