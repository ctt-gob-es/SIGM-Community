...
public class ServiceTreeLdap extends ServiceTree {
    ...
    public ServiceTreeLdap(NaryTree tree, int maxChildrenLdap, int typeTree) throws Exception {
        super(tree);
        serviceLdap = new ServiceLdap(maxChildrenLdap, typeTree); 
        ...
        if (enc)
        {
	        ...
	        if (!nodoRaiz.getChildPrinted()){
		        if ( fin.isRoot()) // Es Nodo raiz --> "c=es"
		        	...
		        	List hijos = serviceLdap.GetLdapChildList (parent, entidad);
		            ...
		            while (it.hasNext())
		            {
		                ...
		                boolean hasChildren = serviceLdap.hasChildLdap(hijo.getValor(), entidad);
    ...
    private String getTitle(String dn) {
        ...
        if (!rootDn.equals(dn))
        {
        	  title = LdapUtils.getLdapTitleDn(dn); 
        }
        return title;
    }
