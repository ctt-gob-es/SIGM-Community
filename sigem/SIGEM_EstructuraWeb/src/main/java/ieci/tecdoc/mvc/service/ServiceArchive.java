...
public class ServiceArchive {
    ...
    private AccessDTO getAccessDTOLdap (HttpServletRequest request, String entidad) throws Exception {
        ...
        ServiceTree serviceTree = new ServiceTreeLdap(tree, maxChildrenLdap, Constantes.LDAP_TYPE_TREE_USER); // ????
        String dn = null;
        if (id != null) {
        ...
        String groupFilter = serviceLdap.getGroupFilter(entidad);
        ...
        Map attMap = serviceLdap.getAttributesMap(dn, entidad);

        String objectClassValue = attMap.get("objectclass").toString().toLowerCase();
        String guid = serviceLdap.getGuidAttributeName(entidad);
