...
public class UserDelete extends BaseAction{
    ...
    public ActionForward executeLogic(ActionMapping mapping, ActionForm form,
    	...
    	*/
        ...
        boolean userCanDeleteUser = true;
        if (user != null) {
        	userCanDeleteUser = user.getIsSystemXSuperUser()? true :  ( user.getUserProfile().intValue() ==  UserDefs.PROFILE_SUPERUSER ) && ( profile.getProfile() != UserDefs.PROFILE_SUPERUSER );
        }
    	...
    	try {
        	...
        	ActionMessage msg = new ActionMessage("message.user.darDeBaja");
