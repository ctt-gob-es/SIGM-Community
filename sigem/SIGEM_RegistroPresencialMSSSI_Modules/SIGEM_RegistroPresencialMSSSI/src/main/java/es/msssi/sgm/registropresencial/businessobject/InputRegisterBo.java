...
//	private static final String FLD1004 = "fld1004";
	 ...
	 */
	synchronized public static Integer saveOrUpdateFolder( UseCaseConf useCaseConf, Integer bookId, Integer fldid, List<?> files, InputRegisterBean inputRegisterBean, List<Interesado> inter, Map<?, ?> documents) throws RPGenericException, RPInputRegisterException {
		
		LOG.trace("Entrando en InputRegisterBo.saveOrUpdateFolder()");
		
		boolean updateDate = false;
		Integer varFldid = fldid;
		Integer modifySystemDate = Integer.valueOf(BookSession.invesicresConf( useCaseConf.getEntidadId()).getModifySystemDate());
		...
		newAxSF.setLiteralBookType(RBUtil.getInstance(useCaseConf.getLocale()).getProperty(Keys.I18N_BOOKUSECASE_NODE_INBOOK_NAME));
		newAxSF.setLocale(useCaseConf.getLocale());

		if (fldid == null) {
			newAxSF = mappingInputRegisterToAxSF(inputRegisterBean);
			if (inter != null && inter.size() > 0) {
				Interesado iInteresado = (Interesado) inter.get(0);
				...
				inputRegisterBean.setFld9((docIdentidad + " " + nombre + " " + pApellido + " " + sApellido).trim());
			} else {
				inputRegisterBean.setFld9(null);
			}
			
			try {
				varFldid = FolderSession.createNewFolder( useCaseConf.getSessionID(), bookId, newAxSF, inter, launchDistOutRegister, useCaseConf.getLocale(), useCaseConf.getEntidadId());
				...
				throw new RPInputRegisterException(
						RPInputRegisterErrorCode.CREATE_INPUT_REGISTER_ERROR, ErrorConstants.CREATE_INPUT_REGISTER_ERROR_MESSAGE, bookException);
			...
			if (inter != null && inter.size() > 0) {
				...
				if(StringUtils.isEmpty(docIdentidad)){
					...
					iInteresado.setDocIdentidad(nombreSplit[0]);
					nombre = nombre.replaceAll(nombreSplit[0], "").trim();
				}
			...
			try {
				FolderSession.updateFolder(useCaseConf.getSessionID(), bookId, fldid, newAxSF, inter, documents, updateDate, launchDistOutRegister, useCaseConf.getLocale(), useCaseConf.getEntidadId());
				...
				throw new RPGenericException(
						RPGenericErrorCode.PARAMETER_VALIDATION_ERROR, ErrorConstants.PARAMETERS_VALIDATION_ERROR_MESSAGE, validationException);
			} catch (BookException bookException) {
				LOG.error(ErrorConstants.UPDATE_INPUT_REGISTER_ERROR_MESSAGE, bookException);
				throw new RPInputRegisterException(
						RPInputRegisterErrorCode.UPDATE_INPUT_REGISTER_ERROR, ErrorConstants.GET_INPUT_REGISTER_ERROR_MESSAGE, bookException);
			} catch (SessionException sessionException) {
	...
	public static void crearCarpeta(UseCaseConf useCaseConf, Integer bookId, Integer folderId, String nombreCarpeta) {
		try {
			...
			DocumentoRegistroVO documentoRegistro = new DocumentoRegistroVO();
			
			IdentificadorRegistroVO idRegistro= new IdentificadorRegistroVO();
			idRegistro.setIdLibro(bookId.toString());
			idRegistro.setIdRegistro(folderId.toString());
			
			documentoRegistro.setIdRegistro(idRegistro);
			documentoRegistro.setName(nombreCarpeta);
			
			List<DocumentoRegistroVO> listaDocs = new ArrayList<DocumentoRegistroVO>();
			listaDocs.add(documentoRegistro);
		
			FolderFileSession.createDocuments(bookId, folderId, DocumentosHelper.getDocumentosISicres(listaDocs), user.getId(), useCaseConf.getEntidadId());
			
		} catch (BookException bookException) {
			LOG.error(ErrorConstants.UPDATE_INPUT_REGISTER_ERROR_MESSAGE, bookException);
			//throw new RPInputRegisterException( RPInputRegisterErrorCode.UPDATE_INPUT_REGISTER_ERROR, ErrorConstants.GET_INPUT_REGISTER_ERROR_MESSAGE, bookException);
			...
			LOG.error(ErrorConstants.INSERT_USERS_ERROR_MESSAGE, tecDocException);
			//throw new RPGenericException(RPGenericErrorCode.INSERT_USERS_ERROR_MESSAGE, ErrorConstants.SESSION_ERROR_MESSAGE, tecDocException);			
		}		
	}
	
	public static void modificarNombreCarpeta(UseCaseConf useCaseConf, Integer bookId, Integer folderId, String nombreViejoCarpeta, String nombreNuevoCarpeta) {
		try {
			FolderFileSession.modificarNombreCarpeta(bookId, folderId, nombreViejoCarpeta, nombreNuevoCarpeta, useCaseConf.getEntidadId());
	...
	public static void modificarNombreDocumento(UseCaseConf useCaseConf, Integer bookId, Integer folderId, int docId, String nombreViejoDocumento, String nombreNuevoDocumento) {
		try {
			FolderFileSession.modificarNombreDocumento(bookId, folderId, docId, nombreViejoDocumento, nombreNuevoDocumento, useCaseConf.getEntidadId());
	...
	public static String getFileExtension(String fileName, String extension) {
	...
	public static FolderDataSession subirDocumentos(UseCaseConf useCaseConf, Integer bookId, Integer folderId, Map<String, Object> documentos) {
		FolderDataSession documento = null;
		
		try {
			documento = FolderSession.addDocument(useCaseConf.getSessionID(), bookId, folderId, documentos, new AxSf(), useCaseConf.getLocale(), useCaseConf.getEntidadId());
			
		} catch (BookException bookException) {
			LOG.error(ErrorConstants.UPDATE_INPUT_REGISTER_ERROR_MESSAGE, bookException);
		} catch (SessionException e) {
			LOG.error("ERROR al subir un documento. " + e.getMessage(), e);
	...
	public static FolderDataSession subirDocumentosCarpeta(UseCaseConf useCaseConf, Integer bookId, Integer folderId, String nombreCarpeta, Map<String, Object> documentos) {
		FolderDataSession documento = null;

		try {
			documento = FolderSession.addPage(useCaseConf.getSessionID(), bookId, folderId, nombreCarpeta, documentos, new AxSf(), useCaseConf.getLocale(), useCaseConf.getEntidadId());
			
		} catch (BookException bookException) {
			LOG.error(ErrorConstants.UPDATE_INPUT_REGISTER_ERROR_MESSAGE, bookException);
		} catch (SessionException e) {
			LOG.error("ERROR al subir un documento a una carpeta. " + e.getMessage(), e);
		} catch (ValidationException e) {
			LOG.error("ERROR al subir un documento a una carpeta. " + e.getMessage(), e);
		}
		return documento;
	...
	private static AxSf mappingInputRegisterToAxSF( InputRegisterBean inputRegisterBean) {
		...
		if (inputRegisterBean.getFld16() != null && !"".equals(String.valueOf(inputRegisterBean.getFld16()))) {
			...
			newAxSF.addAttributeValue(FLD16, inputRegisterBean.getFld16()
					.getId());
		}
	 ...
	 *             si se ha producido un error genérico en el proceso.
	 */
	public InputRegisterBean copyInputRegisterBean(UseCaseConf useCaseConf, ScrRegstate book, Integer fdrid) throws RPGenericException, RPInputRegisterException {
		LOG.trace("Entrando en InputRegisterBo.copyInputRegisterBean()");
		AxSfIn axsf = null;
		InputRegisterBean inputRegisterBean = new InputRegisterBean();
		try {
			axsf = (AxSfIn) FolderSession.getBookFolder( useCaseConf.getSessionID(), book.getIdocarchhdr().getId(), fdrid, useCaseConf.getLocale(), useCaseConf.getEntidadId());
		} catch (ValidationException validationException) {
			...
			throw new RPInputRegisterException(
					RPInputRegisterErrorCode.GET_INPUT_REGISTER_ERROR, ErrorConstants.GET_INPUT_REGISTER_ERROR_MESSAGE, bookException);
		} catch (SessionException sessionException) {
		...
		inputRegisterBean.setFld11((axsf.getAttributeValueAsString(FLD11) != null) ? Integer.valueOf(axsf.getAttributeValueAsString(FLD11)) : null);
		inputRegisterBean.setFld12((Date) axsf.getAttributeValue(FLD12));
		inputRegisterBean.setFld13(axsf.getFld13());
		inputRegisterBean.setFld13Name((axsf.getFld13() != null) ? axsf.getFld13().getName() : null);
		...
		inputRegisterBean.setFld16Name((axsf.getFld16() != null) ? axsf.getFld16().getCode() : null);
		...
		inputRegisterBean.setFld21((axsf.getExtendedFields().get(FIELD21) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD21)).getText() : null);
		inputRegisterBean.setFld22((axsf.getExtendedFields().get(FIELD22) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD22)).getText() : null);
		inputRegisterBean.setFld18((axsf.getAxxf() != null) ? axsf.getAxxf().getText() : null);
		inputRegisterBean.setFld504((axsf.getAttributeValueAsString("fld504") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld504")) : null);
		inputRegisterBean.setFld505((axsf.getAttributeValueAsString("fld505") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld505")): null);
		inputRegisterBean.setFld506((axsf.getAttributeValueAsString("fld506") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld506")) : null);
		inputRegisterBean.setFld501((axsf.getExtendedFields().get(FIELD501) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD501)).getText() : null);
		inputRegisterBean.setFld502((axsf.getExtendedFields().get(FIELD502) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD502)).getText() : null);
	 ...
	 *             si se ha producido un error genérico en el proceso.
	 */
	public InputRegisterBean loadInputRegisterBean(UseCaseConf useCaseConf, ScrRegstate book, Integer fdrid) throws RPInputRegisterException, RPGenericException {
		...
		try {
			...
			InterestedBo interestedBo = new InterestedBo();
			interesados = interestedBo.getAllInterested(book.getIdocarchhdr().getId(), fdrid, useCaseConf);
			...
			throw new RPInputRegisterException( RPInputRegisterErrorCode.LOAD_INPUT_REGISTER_ERROR, ErrorConstants.GET_INPUT_REGISTER_ERROR_MESSAGE, bookException);
		} catch (SessionException sessionException) {
		...
		inputRegisterBean.setFld6Name(RBUtil.getInstance( useCaseConf.getLocale()).getProperty( "book.fld6." + axsf.getAttributeValueAsString(FLD6)));
		inputRegisterBean.setFld7(axsf.getFld7());
		inputRegisterBean.setFld7Name(axsf.getFld7Name());
		inputRegisterBean.setFld8(axsf.getFld8());
		inputRegisterBean.setFld8Name(axsf.getFld8Name());
		...
		inputRegisterBean.setFld21((axsf.getExtendedFields().get(FIELD21) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD21)).getText() : null);
		inputRegisterBean.setFld22((axsf.getExtendedFields().get(FIELD22) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD22)).getText() : null);
		inputRegisterBean.setFld503((axsf.getAttributeValueAsString("fld503") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld503")) : null);
		inputRegisterBean.setFld504((axsf.getAttributeValueAsString("fld504") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld504")) : null);
		inputRegisterBean.setFld505((axsf.getAttributeValueAsString("fld505") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld505")) : null);
		inputRegisterBean.setFld506((axsf.getAttributeValueAsString("fld506") != null) ? Integer.parseInt(axsf.getAttributeValueAsString("fld506")) : null);
		inputRegisterBean.setFld501((axsf.getExtendedFields().get(FIELD501) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD501)).getText() : null);
		inputRegisterBean.setFld502((axsf.getExtendedFields().get(FIELD502) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD502)).getText() : null);
		inputRegisterBean.setFld507((axsf.getExtendedFields().get(FIELD507) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD507)).getText() : null);

		if (null != axsf.getExtendedFields().get(FIELD1003)) {
			inputRegisterBean.setFld1003(Integer.parseInt(((AxXf) axsf.getExtendedFields().get(FIELD1003)).getText()));
		} else if (null != axsf.getAttributeValueAsString(FLD1003)) {
			inputRegisterBean.setFld1003(Integer.parseInt(axsf.getAttributeValueAsString(FLD1003)));
		} else {
			inputRegisterBean.setFld1003(null);
		}

		inputRegisterBean.setFld1004((axsf.getExtendedFields().get(FIELD1004) != null) ? ((AxXf) axsf.getExtendedFields().get(FIELD1004)).getText() : null);
		...
		ISicresAuditHelper.auditarAccesoRegistro( useCaseConf.getSessionID(), book.getIdocarchhdr().getId(), inputRegisterBean.getFdrid(), axsf, book.getIdocarchhdr().getName(), 1);
	 ...
	 *             si se ha producido un error genérico en el proceso.
	 */
	public Integer updateOnlyFolder(UseCaseConf useCaseConf, Integer bookId, Integer fldid, InputRegisterBean inputRegisterBean) throws RPGenericException, RPInputRegisterException {
		...
		try {
			FolderSession.updateFolder(useCaseConf.getSessionID(), bookId, fldid, newAxSF, inputRegisterBean.getInteresados(), null, updateDate, launchDistOutRegister, useCaseConf.getLocale(), useCaseConf.getEntidadId());
