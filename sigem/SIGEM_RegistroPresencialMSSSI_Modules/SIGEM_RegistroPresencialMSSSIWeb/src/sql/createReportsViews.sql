...
AS
    SELECT 1 AS IDBOOK, FDRID, TIMESTAMP, FLD1, FLD2, FLD3, FLD4, FLD5, OFIC.NAME AS FLD5_TEXT, 
        ...
        DIROFIC.EMAIL AS FLD5_EMAIL, FLD6, FLD7, coalesce(ORG.NAME,  CONCAT(CONCAT(ORG.CODE,'-'), ORG.NAME), '') AS FLD7_TEXT, 
        FLD8, coalesce(ORG.NAME,  CONCAT(CONCAT(ORG.CODE,'-'), ORG.NAME), '') AS FLD8_TEXT, FLD9, FLD10, FLD11, FLD12, 
        FLD13, ORG.CODE AS FLD13_TEXT, FLD14, FLD15, FLD16, 
        coalesce(CA.MATTER, CONCAT(CONCAT(CA.CODE,'-'), CA.MATTER), '') AS FLD16_TEXT, FLD17, FLD19, FLD20 
    FROM A1SF
...
AS 
    SELECT 2 AS IDBOOK, FDRID, TIMESTAMP, FLD1, FLD2, FLD3, FLD4, FLD5, OFIC.NAME AS FLD5_TEXT, 
        ...
        DIROFIC.EMAIL AS FLD5_EMAIL, FLD6, FLD7, coalesce(ORG.NAME,  CONCAT(CONCAT(ORG.CODE,'-'), ORG.NAME), '') AS FLD7_TEXT, 
        FLD8, coalesce(ORG.NAME,  CONCAT(CONCAT(ORG.CODE,'-'), ORG.NAME), '') AS FLD8_TEXT, FLD9, FLD10, FLD11, FLD12, 
        coalesce(CA.MATTER, CONCAT(CONCAT(CA.CODE,'-'), CA.MATTER), '') AS FLD12_TEXT, FLD13, FLD15 
    FROM A2SF 
