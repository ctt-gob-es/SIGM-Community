 ...
 * 
 */
public class ReportsOutputRegisterBo extends LazyDataModel<RowSearchOutputRegisterBean> implements IGenericBo, Serializable {
	 ...
	 */
	public List<RowSearchOutputRegisterBean> getRegistersForRelationReports( Integer bookId, SearchOutputRegisterBean sOutputRegister) {
		 ...
		 * fecha.setTime(searchOutputRegister.getFld2ValueDesde()); fecha.set(
		 * Calendar.HOUR_OF_DAY, 0); fecha.set( Calendar.MINUTE, 0); fecha.set(
		 * Calendar.SECOND, 0); Date fromDate = fecha.getTime();
		 * searchOutputRegister.setFld2ValueDesde(fromDate); fecha.set(
		 ...
		 * searchOutputRegister.setFld2ValueHasta(toDate); } else {
		 */
		// if (searchOutputRegister.getFld2ValueDesde() != null &&
		// !"".equals(searchOutputRegister.getFld2ValueDesde())
		...
		try {
			...
			LOG.error(ErrorConstants.LOAD_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, sessionException);
			Utils.redirectToErrorPage(null, sessionException, null);
			
		} catch (TecDocException tecDocException) {
			LOG.error(ErrorConstants.LOAD_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, tecDocException);
			Utils.redirectToErrorPage(null, tecDocException, null);
		}

		List<RowSearchOutputRegisterBean> registersList = new ArrayList<RowSearchOutputRegisterBean>();
		String orderBy = "";

		// MQE si se seleccionan demasiados tarda muchísimo y se va bloqueando.
...
//					}
	 ...
	 */
	private void buildXmlRelationsReportDocument( List<RowSearchOutputRegisterBean> registersList, boolean isOriginReport) {
	 ...
	 */
	private void fillFldForOriginNodes( List<RowSearchOutputRegisterBean> registersList) {
		...
		myXmlHandler.addNode("FLD8_FATHERNAME", registersList.get(0).getFld8() != null ? registersList.get(0).getFld8().getNameOrgFather() : "");
		myXmlHandler.addNode( "FLD8_ADDRESS", registersList.get(0).getFld8() != null ? reportDAO.getAddressOrg(registersList.get(0).getFld8().getId()) : "");
		...
		int numOrganoOrigen = registersList.get(0).getFld8() != null ? ((ScrOrg) registersList.get(0).getFld8()).getId().intValue() : 0;
		...
		for (int i = 0; i < finalRegistersListLength; i++) {
			numOrganoOrigenNew = registersList.get(i).getFld8() != null ? registersList.get(i).getFld8().getId() : 0;
			...
			if (numOrganoOrigen == numOrganoOrigenNew) {
				...
				myXmlHandler.addNode("FLD8_FATHERNAME", registersList.get(i).getFld8() != null ? registersList.get(i).getFld8().getNameOrgFather() : "");
				myXmlHandler.addNode("FLD8_ADDRESS", registersList.get(i).getFld8() != null ? reportDAO.getAddressOrg(registersList.get(i).getFld8().getId()) : "");
	 ...
	 *            Lista de Ids. de registros.
	 */
	private void fillFldForDestinationNodes( List<RowSearchOutputRegisterBean> registersList) {
		...
		// Se añaden los datos de los registros para cada oficina
		for (int i = 0; i < finalRegistersListLength; i++) {
			numOrganoOrigenNew = registersList.get(i).getFld7() != null ? registersList.get(i).getFld7().getId() : 0;
	 ...
	 */
	private void fillRegisterNode(RowSearchOutputRegisterBean rowSORBean, boolean isOriginReport) {
		
		myXmlHandler.addBeginXmlHandler("REGISTRO");
		myXmlHandler.addNode("FDRID", rowSORBean.getFdrid() != null ? rowSORBean.getFdrid() : "");
		myXmlHandler.addNode("FLD1", rowSORBean.getFld1() != null ? rowSORBean.getFld1() : "");
		myXmlHandler.addNode("FLD2", rowSORBean.getFld2() != null ? Utils.formatDateInString((Date) rowSORBean.getFld2()) : "");
		myXmlHandler.addNode("FLD3", rowSORBean.getFld3() != null ? rowSORBean.getFld3() : "");
		myXmlHandler.addNode("FLD4", rowSORBean.getFld4() != null ? Utils.formatDateInString((Date) rowSORBean.getFld4()) : "");
		myXmlHandler.addNode("FLD6", rowSORBean.getFld6() != null ? String.valueOf(rowSORBean.getFld6()) : "");
		
		if (isOriginReport) {
			myXmlHandler.addNode("FLD7", rowSORBean.getFld7() != null ? String.valueOf(((ScrOrg) rowSORBean.getFld7()).getId().intValue()) : "");
			
		} else {
			myXmlHandler.addNode("FLD8", rowSORBean.getFld8() != null ? String.valueOf(((ScrOrg) rowSORBean.getFld8()).getId().intValue()) : "");
		}
		
		myXmlHandler.addNode("FLD9", interestedBo.fillSenderFieldFromSenderListToRelationsReport(rowSORBean.getFdrid(), book.getIdocarchhdr().getId(), useCaseConf));
		myXmlHandler.addNode("FLD10", rowSORBean.getFld10() != null ? rowSORBean.getFld10() : "");
		myXmlHandler.addNode("FLD11", rowSORBean.getFld11() != null ? String.valueOf(rowSORBean.getFld11()) : "");
		myXmlHandler.addNode("FLD12", rowSORBean.getFld12() != null ? String.valueOf(((ScrCa) rowSORBean.getFld12()).getId().intValue()) : "");
		myXmlHandler.addNode("FLD13", rowSORBean.getFld13() != null ? rowSORBean.getFld13() : "");
		myXmlHandler.addNode("FLD14", rowSORBean.getFld14() != null ? rowSORBean.getFld14() : "");
		myXmlHandler.addNode("FLD7_TEXT", rowSORBean.getFld7Name() != null ? rowSORBean.getFld7Name() : "");
		myXmlHandler.addNode("FLD8_TEXT", rowSORBean.getFld8Name() != null ? rowSORBean.getFld8Name() : "");
		myXmlHandler.addEndXmlHandler("REGISTRO");
	}
	 ...
	 * @param registersList
	 *            Lista de Ids. de registros.
	 * @return file el informe.
	 * @throws Exception
	 *             Si se ha producido un error generando el informe.
	 */
	public StreamedContent buildJasperReport(ScrRegstate book, List<LinkedHashMap<String, Object>> reportResults, List<RowSearchOutputRegisterBean> registersList) throws Exception {
		...
		if (reportResults != null && reportResults.size() > 0) {
			if (reportResults.size() == 1) {
				...
				outputRegisterBean = outputRegisterBo.loadOutputRegisterBean( useCaseConf, book, (Integer) reportResults.get(0).get("FDRID"));

				registerData = Utils.mappingOutputRegisterToAxSF(outputRegisterBean);
				registerDocumentsBo = new RegisterDocumentsBo();

				numAcuses = registerDocumentsBo.countPageReport(useCaseConf, book.getIdocarchhdr().getId(), outputRegisterBean.getFdrid());
				...
				if (documentSaved != null) {
					...
					if (outputRegisterBean.getInteresados() != null && outputRegisterBean.getInteresados().size() > 0) {
						...
						Interesado iInteresado = outputRegisterBean.getInteresados().get(0);
						String docIdentidad = StringUtils.isNotEmpty(iInteresado.getDocIdentidad()) ? iInteresado.getDocIdentidad() : "";
						String nombreInteresado = StringUtils.isNotEmpty(iInteresado.getNombre()) ? iInteresado.getNombre() : "";
						...
						interesado.setNombre((docIdentidad + " " + nombreInteresado + " " + pApellido + " " + sApellido).trim());
						interesado.setIdTercero(iInteresado.getIdTercero());
						interesado.setRazonSocial(iInteresado.getRazonSocial());
						interesado.setRepresentante(iInteresado.getRepresentante());
						interesado.setTipo(iInteresado.getTipo());
						interesado.setTipodoc(iInteresado.getTipodoc());
					...
					outputRegisterBo.updateOnlyFolder(useCaseConf, book.getIdocarchhdr().getId(), outputRegisterBean.getFdrid(), outputBeanUpdate);
					registerDocumentsBo.updateFlag(useCaseConf, book.getIdocarchhdr().getId(), outputRegisterBean.getFdrid(), Long.valueOf(documentSaved.getId().getIdPagina()).intValue(), numAcuses);
				}
				// }
				...
				String zipName = KeysRP.OR_REPORT_CERTIFICATE_NAME + KeysRP.REPORT_CERTIFICATE_EXTENSION_ZIP;
			...
			if (OPTION_TYPE_RMD == searchOutputRegister.getReportTypeValue()) {
				...
				LOG.info("Vienen datos referentes a un informe de relaciones por destino. " + "Nombre informe: " + KeysRP.OR_REPORT_DESTINATION_RELATIONS_NAME + "; nombre plantilla: " + KeysRP.OR_REPORT_DESTINATION_RELATIONS_TEMPLATE_NAME + ", Xpath: " + REPORT_EXPRESSION_DESTINATION_RELATIONS);
				...
				LOG.info("Vienen datos referentes a un informe de relaciones por origen. " + "Nombre informe: " + KeysRP.OR_REPORT_ORIGIN_RELATIONS_NAME + "; nombre plantilla: " + KeysRP.OR_REPORT_ORIGIN_RELATIONS_TEMPLATE_NAME + ", Xpath: " + REPORT_EXPRESSION_ORIGIN_RELATIONS);
			}
	...
	private LinkedHashMap<String, Object> buildAndSignCertificateReport( LinkedHashMap<String, Object> report) throws Exception {
		...
		reportName = KeysRP.OR_REPORT_CERTIFICATE_NAME + regNumber + KeysRP.REPORT_CERTIFICATE_EXTENSION_FILE;
		reportTemplateName = KeysRP.OR_REPORT_CERTIFICATE_TEMPLATE_NAME;
		reportExpression = REPORT_EXPRESSION_CERT;
		
		LOG.info("Acuse de recibo a generar. Nombre informe: " + KeysRP.OR_REPORT_CERTIFICATE_NAME + "; nombre plantilla: " + KeysRP.OR_REPORT_CERTIFICATE_TEMPLATE_NAME + ", Xpath: " + REPORT_EXPRESSION_CERT);
	...
	public LinkedHashMap<String, Object> buildJasperReportReturnMap(Integer id, List<LinkedHashMap<String, Object>> reportResults) throws Exception {
