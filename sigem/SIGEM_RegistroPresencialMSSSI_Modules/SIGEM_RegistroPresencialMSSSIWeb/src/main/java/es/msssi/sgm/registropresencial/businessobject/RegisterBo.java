...
public class RegisterBo implements IGenericBo, Serializable {
	...
	if (map != null && !map.isEmpty()) {
		...
		result.getAttributesValues().put("libro",
			idocs.get(Integer.parseInt(key.getType())).getName());
	...
	case NUMBER3:
		...
		if (listaRegs != null && !listaRegs.isEmpty()) {
		    AsocRegsResults asocRegsResults = listaRegs.get(0);
     ...
     *            libro destino.
     */
    public void moveRegister(Integer fdridOriginRegister, Integer bookIdOrigin, Integer bookIdDestination) throws RPRegisterException {
    	...
    	try {
    		if (fdridOriginRegister != null && bookIdOrigin != null && bookIdDestination != null) {
    			...
    			moveRegisterDao.moveRegisterBook(fdridOriginRegister, bookIdOrigin, bookIdDestination);
    		}
    	} catch (Exception exception) {
    		LOG.error(ErrorConstants.MOVE_REGISTER_ERROR, exception);
    		throw new RPRegisterException(RPRegisterErrorCode.MOVE_REGISTER_ERROR, ErrorConstants.MOVE_REGISTER_ERROR, exception);
    	}
    }
    

    /**
     * Obtiene el histórico de documentos.
     * 
     * @param useCaseConf
     *            Usuario que realiza la petición.
     * @param book
     ...
     * @param numReg
     *            Número de registro.
     * @return result Listado de histórico de docuemntos.
     * 
     * @throws RPRegisterException
     *             si se ha producido algún error al tratar la información del
     *             registro.
     ...
     *             si se ha producido un error genérico en el proceso.
     */    
    public List<UpdHisDocFdrResults> getUpdHisDocResults(UseCaseConf useCaseConf, ScrRegstate book, int idRegistro, String numReg) throws RPGenericException, RPRegisterException {
    	
    	LOG.trace("Entrando en registerBo.getUpdHisFdrResults()");
    	
    	List<UpdHisDocFdrResults> result = null;
//    	UpdHisDocsFdrResults hist;
    	
    	try {
    		AxSf axsf = FolderSession.getBookFolder(useCaseConf.getSessionID(), book.getIdocarchhdr().getId(), idRegistro, useCaseConf.getLocale(), useCaseConf.getEntidadId());
    		result = FolderHistSession.getUpdHisDocResults(useCaseConf.getSessionID(), useCaseConf.getLocale(), book.getIdocarchhdr().getId(), idRegistro, axsf, numReg, useCaseConf.getEntidadId());

//    		// convertir los códigos en texto
//    		ValidationListAction val = null;
//    		if (FacesContext.getCurrentInstance().getExternalContext().getApplicationMap().get("validationListAction") == null) {
//    			val = new ValidationListAction();
//    			val.create();
//    		} else {
//    			val = (ValidationListAction) FacesContext.getCurrentInstance().getExternalContext().getApplicationMap().get("validationListAction");
//    		}
//    		
//    		for (int ind = 0; ind < result.size(); ind++) {
//    			hist = result.get(ind);
//    			
//    			hist.setValue(val.getTextOfValue(hist.getScrModifReg().getIdFld(), hist.getValue(), book.getIdocarchhdr().getType()));
//    			hist.setOldvalue(val.getTextOfValue(hist.getScrModifReg().getIdFld(), hist.getOldvalue(), book.getIdocarchhdr().getType()));
//    			result.set(ind, hist);
//    		}
    	} catch (ValidationException validationException) {
    		LOG.error(ErrorConstants.GET_HISTORICAL_REGISTER_ERROR_MESSAGE, validationException);
    		...
    		LOG.error(ErrorConstants.GET_HISTORICAL_REGISTER_ERROR_MESSAGE, bookException);
    		throw new RPRegisterException(RPRegisterErrorCode.GET_HISTORICAL_REGISTER_ERROR, ErrorConstants.GET_HISTORICAL_OR_REGISTER_ERROR_MESSAGE, bookException);
    	} catch (SessionException sessionException) {
    		LOG.error(ErrorConstants.GET_HISTORICAL_REGISTER_ERROR_MESSAGE, sessionException);
