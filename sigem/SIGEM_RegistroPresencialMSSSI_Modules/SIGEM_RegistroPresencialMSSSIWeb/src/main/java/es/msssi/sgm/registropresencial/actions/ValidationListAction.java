...
public class ValidationListAction extends GenericActions {
    ...
    public static final String HIBERNATEIUSERUSERHDR = HIBERNATEINVESDOCOBJPATH + "Iuseruserhdr";
    public static final String HIBERNATEIUSERDEPTHDR = HIBERNATEINVESDOCOBJPATH + "Iuserdepthdr";
    ...
    public void create() {
		...
		try {
		    ...
		    listCCAA = validationListBo.getListCCAA(useCaseConf);
		    listProv = validationListBo.getListProv(useCaseConf);
		    
		    hashTipoRegistro.add(new ItemBean( 1, "Entrada"));
		    hashTipoRegistro.add(new ItemBean( 2, "Salida"));
	
		    hashEstados.add(new ItemBean( 0, "Completo"));
		    hashEstados.add(new ItemBean( 1, "Incompleto"));
		    hashEstados.add(new ItemBean( STATECLOSE, "Cerrado"));
		    
		    for (int i = 1; i < NUMSTATESDIST; i++) {
		    	if (i < NUMBER6) {
		    		listStateDist.add(new ItemBean( i, RBUtil.getInstance( useCaseConf.getLocale()).getProperty( Keys.I18N_BOOKUSECASE_DISTRIBUTIONHISTORY_MINUTA_DIST_STATE + i)));
		    	}
		    }
		...
		// Si falla, redireccionamos a la página de error
			...
			LOG.error( ErrorConstants.GET_INFORMATION_LISTS_ERROR_MESSAGE, bookException);
			...
			LOG.error( ErrorConstants.GET_INFORMATION_LISTS_ERROR_MESSAGE, attributesException);
			Utils.redirectToErrorPage( null, attributesException, null);
		}
    }
     ...
     */
    public void setValidationListBo( ValidationListBo validationListBo) {
     ...
     *            Código de error a guardar.
     */
    public void setListTransportes( List<ScrTt> listTransportes) {
     ...
     *            Código de error a guardar.
     */
    public void setListOficinas( List<ScrOfic> listOficinas) {
     ...
     *            Código de error a guardar.
     */
    public void setListAsuntos( List<ScrCa> listAsuntos) {
     ...
     *            Código de error a guardar.
     */
    public void setHashEstados( List<ItemBean> hashEstados) {
     ...
     *            Código de error a guardar.
     */
    public void setHashTipoRegistro( List<ItemBean> hashTipoRegistro) {
     ...
     *            lista a guardar.
     */
    public void setListStateDist( List<ItemBean> listStateDist) {
     ...
     */
    public void setListDepartamentos( List<Iuserdepthdr> listDepartamentos) {
     ...
     */
    public List<ScrCa> completeAsunt( String query) {
    	...
    	for (ScrCa p : listAsuntos) {
    		if (Utils.converterToCS( p.getCode()).indexOf( queryCS) != -1 || Utils.converterToCS( p.getMatter()).indexOf( queryCS) != -1 || Utils.converterToCS( p.getCode() + " " + p.getMatter()).indexOf( queryCS) != -1) {
    			suggestions.add(p);
    		}
    	}
     ...
     */
    public List<ScrOrg> completeOrgDestino(	String query) {
    	...
    	try {
    		...
    		LOG.error( ErrorConstants.GET_ORGANISMS_LIST_ERROR_MESSAGE, e);
     ...
     * @return suggestions Lista de organismos según el texto insertado.
     */
    public List<ScrOrg> completeOrgOrigen( String query) {
    	List<ScrOrg> suggestions = new ArrayList<ScrOrg>();
    	UnitsBo unitsBo = new UnitsBo();
     ...
     * 
     * @param query
     *            Parte del nombre del interesado.
     * 
     * @return suggestions Lista de organismos según el texto insertado.
     */
    public List<Interesado> completeInteresado (String query) {
    	List<Interesado> suggestions = new ArrayList<Interesado>();
    	suggestions = getInteresados(query);
    	
    	return suggestions;
    }
    
    private List<Interesado> getInteresados(String query){
    	...
    	try {
			...
			terceroFisicoNIF.setValue(query.toUpperCase());
			...
			terceroJuridicoCIF.setField("upper(cif)");
			terceroJuridicoCIF.setOperator(OperatorEnum.ES_IGUAL);
			terceroJuridicoCIF.setValue(query.toUpperCase());
			...
			if(tercerosFisicos.size() + tercerosJuridicos.size() == 0){
				// En caso de no encontrar nada IGUAL a NIF o CIF se cambia el criterio de busqueda a que el NIF o CIF COMIENCE POR...
				...
				tercerosFisicos = terceroManager.findByCriteria(criterioInterFisico);
				tercerosJuridicos = terceroManager.findByCriteria(criteriaInterJuridico);
				...
				if(tercerosFisicos.size() + tercerosJuridicos.size() == 0){
					...
					terceroFisicoNIF.setOperator(OperatorEnum.COMIENZA_POR);
					terceroJuridicoCIF.setField("upper(name)");
					terceroJuridicoCIF.setOperator(OperatorEnum.COMIENZA_POR);
					...
					terceroFisicoNIF.setOperator(OperatorEnum.COMIENZA_POR);
					tercerosFisicos.addAll(terceroManager.findByCriteria(criterioInterFisico));
				}
			}
     ...
     */
    private ScrOfic getOficina( Integer id) {
    	ScrOfic office = null;
    	boolean officeFound = false;
    	for (ScrOfic p : listOficinas) {
    		if (p.getId().equals( id) && !officeFound) {
    			office = p;
    			officeFound = true;
    		}
    	}
     ...
     */
    private ScrOrg getOrganismo( String code) {
    	ScrOrg organism = null;
    	UnitsBo unitsBo = new UnitsBo();
    	
    	try {
    		organism = unitsBo.getOrg( useCaseConf, code);
     ...
     *         texto.
     */
    public String getTextOfValue( int idFld, String value, int type) {
    	
    	String result = null;

    	SimpleDateFormat format = new SimpleDateFormat( "dd/MM/yyyy HH:mm:ss");
    	SimpleDateFormat formatShort = new SimpleDateFormat( "dd/MM/yyyy");
    	SimpleDateFormat formatOld = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss.SS");
    		...
    		if (type == 1) { // ENTRADA
    			switch (idFld) {
	    			...
	    			case FIELD12:
	    				...
	    				// oficina de registro
	    			case FIELD5:
	    				ScrOfic ofic = getOficina(new Integer( value));
	    				result = ofic.getCode() + " " + ofic.getName();
	    				break;
	    			// estado
	    			case FIELD6:
	    				result = Utils.getItemList( hashEstados, new Integer( value));
	    				break;
	    			// organismo origen
	    			case FIELD7:
	    				ScrOrg org = getOrganismo(value);
	    				result = org.getCode() + " " + org.getName();
	    				break;
	    			// organismo destino
	    			case FIELD8:
	    				ScrOrg org8 = getOrganismo(value);
	    				result = org8.getCode() + " " + org8.getName();
	    				break;
	    			// tipo registro
	    			case FIELD11:
	    				if ("0".equals(value)) {
	    				...
	    				result = Utils.getItemList( hashTipoRegistro, new Integer( value));
	    				break;
	    			// transporte
	    			case FIELD14:
	    				...
	    				while (iterScrTT.hasNext() && result == null) {
	...
	private void getDeptsGroupsUsers( UseCaseConf useCaseConf) {
		...
		try {
		    ...
		    if (useCaseConf.getUseLdap().booleanValue()) {
				...
				listDepartamentos = ValidationSessionEx.getGroupsUsersLDAP( useCaseConf.getSessionID(), TYPEDEP, useCaseConf.getEntidadId());
				...
				listDepartamentos = ValidationListBo.getDeptsGroupsUsers( useCaseConf.getSessionID(), TYPEDEP, useCaseConf.getEntidadId(), null);
     ...
     */
    public static List<Iuseruserhdr> getUsersNotFromElectronicRegister( String sessionID, String entidad) throws SessionException, ValidationException {
    	Transaction tran = null;
    	...
    	try {
    		...
    		userList = getUsers( session, user.getId());
    		HibernateUtil.commitTransaction(tran);
    		return userList;
    ...
    public static List<Iuseruserhdr> getUsers( Session session, Integer userId) throws HibernateException {
    ...
    public void setListCCAA( List<ScrCCAA> listCCAA) {
     ...
     *            Código de error a guardar.
     */
    public void setListProv( List<ScrProv> listProv) {
