...
public class RegisterDocumentsBo implements IGenericBo {
     ...
     */
    public List<AxDoch> getDocuments(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr) throws RPRegisterException {
    	
    	List<AxDoch> docs = null;
    	
    	try {
    		AxSf axsf = FolderSession.getBookFolder(useCaseConf.getSessionID(), bookId, folderId, useCaseConf.getLocale(), useCaseConf.getEntidadId());
    		boolean permShowDocuments = SecuritySession.permisionShowDocuments(useCaseConf.getSessionID(), axsf);
    		
    		if (permShowDocuments || openFolderDtr) {
    			docs = FolderFileSession.getBookFolderDocsWithPages(useCaseConf.getSessionID(), bookId, folderId, useCaseConf.getEntidadId());
    		}
    	} catch (ValidationException validationException) {
    		LOG.error(ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, validationException);
    		throw new RPRegisterException(RPRegisterErrorCode.GET_REGISTER_DOCUMENTS_ERROR, ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, validationException);
    	} catch (BookException bookException) {
    		LOG.error(ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, bookException);
    		throw new RPRegisterException(RPRegisterErrorCode.GET_REGISTER_DOCUMENTS_ERROR, ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, bookException);
    	} catch (SessionException sessionException) {
    		LOG.error(ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, sessionException);
    		throw new RPRegisterException(RPRegisterErrorCode.GET_REGISTER_DOCUMENTS_ERROR, ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, sessionException);
    	}
    ...
    public List<Axdoch> getDocumentsBasicInfo(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr, Integer flag) throws RPRegisterException { 
...
//    					if (page.getPageSignedId() == null || (page.getPageSignedId() != null && page.getPageSignedId().equals(page.getId()))) {
//    						for (Axpageh pageSign : listPage) {
//    							if (pageSign.getPageSignedId() != null && !pageSign.getPageId().equals(page.getId()) && pageSign.getPageSignedId().equals(page.getId())) {
...
//    				}
    		...
    		throw new RPRegisterException(RPRegisterErrorCode.GET_REGISTER_DOCUMENTS_ERROR, ErrorConstants.GET_REGISTER_DOCUMENTS_ERROR_MESSAGE, exception);
     ...
     *             Si se ha producido un error al obtener los documentos anexos.
     */
    public List<Axdoch> getDocumentsBasicInfo(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr) throws RPRegisterException {
    	List<Axdoch> result = new ArrayList<Axdoch>();
     ...
     */
    public boolean saveDocuments(String sessionID, Integer bookID, Integer folderId, List<UploadedFile> filesToUpload, AxSf axsf, Locale locale, String entidad) throws RPRegisterException {
    	try {
    		int it = 0;
    		...
    		for (UploadedFile fileToUpload : filesToUpload) {
    			...
    			do {
    				...
    				if (supportedContentType.getContentType().equals(fileToUploadContentType)) {
    					LOG.info("Content Type del documento: " + fileToUploadContentType + "encontrado con :" + supportedContentType.getContentType());
    			...
    			if (isContentTypeFound) {
    				...
    				byte[] fileBytes = Utils.convertInputStreamToByeArray(fileToUpload.getInputstream());
    			...
    			FolderSession.addDocument(sessionID, bookID, folderId, documentMap, axsf, locale, entidad);
    		}
    		return isContentTypeFound;
    		
    	} catch (BookException bookException) {
    		LOG.error(ErrorConstants.UPLOADFILE_ERROR_MESSAGE, bookException);
    		throw new RPRegisterException(RPRegisterErrorCode.ADD_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPLOADFILE_ERROR_MESSAGE, bookException);
    		
    	} catch (SessionException sessionException) {
    		LOG.error(ErrorConstants.UPLOADFILE_ERROR_MESSAGE, sessionException);
    		throw new RPRegisterException(RPRegisterErrorCode.ADD_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPLOADFILE_ERROR_MESSAGE, sessionException);
    		
    	} catch (ValidationException validationException) {
    		LOG.error(ErrorConstants.UPLOADFILE_ERROR_MESSAGE, validationException);
    		throw new RPRegisterException(RPRegisterErrorCode.ADD_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPLOADFILE_ERROR_MESSAGE, validationException);
    		
    	} catch (IOException ioException) {
    		LOG.error(ErrorConstants.UPLOADFILE_ERROR_MESSAGE, ioException);
    		throw new RPRegisterException(RPRegisterErrorCode.ADD_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPLOADFILE_ERROR_MESSAGE, ioException);
    	}
    }
     ...
     *             - Si se ha producido un error al adjuntar el documento.
     */
    public DocumentoElectronicoAnexoVO saveStreamedContentDocuments(String sessionID, Integer bookID, Integer folderId, List<StreamedContent> filesToUpload, AxSf axsf, Locale locale, String entidad) throws RPRegisterException {
        return saveStreamedContentDocuments(sessionID, bookID, folderId, filesToUpload, axsf, locale, entidad, null);

    }
    	
    public DocumentoElectronicoAnexoVO saveStreamedContentDocuments(String sessionID, Integer bookID, Integer folderId, List<StreamedContent> filesToUpload, AxSf axsf, Locale locale, String entidad, Date fecha_firma) throws RPRegisterException {
    	...
    	try {
    		...
    		for (StreamedContent fileToUpload : filesToUpload) {
    			...
    			if (isContentTypeFound) {
    				...
    				//extension
    				String extension=fileToUpload.getName().substring(fileToUpload.getName().lastIndexOf(".") + 1, fileToUpload.getName().length());
    				...
    				if(null != fecha_firma){
    					datosFirmaDocumento.setSelloTiempo(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(fecha_firma));
    				}
    				...
    				doc = saveDocuments(sessionID, bookID, folderId, documentoElectronico, axsf, locale, entidad,usuario);
    			}
    		}
    ...
    public DocumentoElectronicoAnexoVO saveDocuments(String sessionID, Integer bookID, Integer folderId, DocumentoElectronicoAnexoVO documentoElectronico, AxSf axsf, Locale locale, String entidad, UsuarioVO usuario) throws RPRegisterException {
    	...
    	try {
    		...
    		throw new RPRegisterException(RPRegisterErrorCode.ADD_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPLOADFILE_ERROR_MESSAGE, exception);
    	}
    }
    
    public DocumentoElectronicoAnexoVO saveDocuments(String sessionID, Integer bookID, Integer folderId, DocumentoElectronicoAnexoVO documentoElectronico, AxSf axsf, Locale locale, String entidad) throws RPRegisterException {
    	try {
    		DocumentoElectronicoAnexoVO doc;
    		ConfiguracionCreateDocumentoElectronicoAnexoVO cfg=new ConfiguracionCreateDocumentoElectronicoAnexoVO();
			...
			doc = documentoElectronicoAnexoManager.create(documentoElectronico, cfg);
    ...
    public void deleteDocument (UseCaseConf useCaseConf, Integer bookId, int folderId, int iddoc)  throws RPRegisterException {
    	...
    	try {
    		...
    		throw new RPRegisterException(RPRegisterErrorCode.DELETE_REGISTER_DOCUMENTS_ERROR, ErrorConstants.DELETE_REGISTER_DOCUMENTS_ERROR_MESSAGE, exception);
    	}
    }
    
    
    public void deletePage (UseCaseConf useCaseConf, Integer bookId, int folderId, int iddoc, Integer fileId)  throws RPRegisterException {
    	...
    	try {
    		...
    		axPKByIdDoc.setType(String.valueOf(bookId));
    		axPKByIdDoc.setId(iddoc);
    		docsDao.deletePage(axPKByIdDoc, fileId.intValue());
     ...
     *             Si se ha producido un error al obtener los documentos anexos.
     */
    public Integer countPageReport(UseCaseConf useCaseConf, Integer bookId, int folderId) throws RPRegisterException {
     ...
     *             Si se ha producido un error al obtener los documentos anexos.
     */
    public void updateFlag(UseCaseConf useCaseConf, Integer bookId, int folderId, int idpage, Integer numAcuse) throws RPRegisterException {
    	...
    	try {
    		...
    		throw new RPRegisterException(RPRegisterErrorCode.UPDATE_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, bookException);
    		
    	} catch (SessionException sessionException) {
    		HibernateUtil.rollbackTransaction(tran);
    		LOG.error(ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, sessionException);
    		throw new RPRegisterException(RPRegisterErrorCode.UPDATE_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, sessionException);
    		
    	} catch (ValidationException validationException) {
    		LOG.error(ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, validationException);
    		throw new RPRegisterException(RPRegisterErrorCode.UPDATE_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, validationException);
    		
    	} catch (Exception exception) {
    		HibernateUtil.rollbackTransaction(tran);
    		LOG.error(ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, exception);
    		throw new RPRegisterException(RPRegisterErrorCode.GET_REGISTER_DOCUMENTS_ERROR, ErrorConstants.UPDATE_REGISTER_DOCUMENTS_ERROR_MESSAGE, exception);
    ...
    public List<Axdoch> getAllDocumentsYFolders(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr) throws RPRegisterException {
    	List<Axdoch> result = new ArrayList<Axdoch>();
    	result = getAllDocumentsYFolders(useCaseConf, bookId, folderId, openFolderDtr, null);
    	return result;    	
    }
    
    @SuppressWarnings("unchecked")
    public List<Axdoch> getAllDocumentsYFolders(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr, Integer flag) throws RPRegisterException {
    	List<Axdoch> result = new ArrayList<Axdoch>();
    	Transaction tran = null;
    	String entity = useCaseConf.getEntidadId();
...
//	    		    }
    ...
    public DocumentoElectronicoAnexoVO saveDocumentCompulsa(UseCaseConf useCaseConf, DocumentoElectronicoAnexoVO documentoElectronico, Map<String, Object> documentos) throws RPRegisterException {
    	try {
    		DocumentoElectronicoAnexoVO doc;
    		ConfiguracionCreateDocumentoElectronicoAnexoVO cfg=new ConfiguracionCreateDocumentoElectronicoAnexoVO();
			...
			doc = documentoElectronicoAnexoManager.createDocumentCompulsa(useCaseConf, documentoElectronico, cfg, documentos);
    ...
    public DocumentoElectronicoAnexoVO getDocumentoElectronicoById(UseCaseConf useCaseConf, IdentificadorDocumentoElectronicoAnexoVO identificadorDocumentoElectronicoAnexoVO){
    	DocumentoElectronicoAnexoVO doc;

    	MultiEntityContextHolder.setEntity(useCaseConf.getEntidadId());

		doc = documentoElectronicoAnexoManager.retrieve(identificadorDocumentoElectronicoAnexoVO);
    ...
    @SuppressWarnings("unchecked")
	public List<Axdoch> getAllPdfsNoFirmados(UseCaseConf useCaseConf, Integer bookId, int folderId, boolean openFolderDtr, Integer flag) throws RPRegisterException {
    	List<Axdoch> result = new ArrayList<Axdoch>();
    	Transaction tran = null;
    	String entity = useCaseConf.getEntidadId();
    	...
    	try {
    	    ...
    	    if (permShowDocuments || openFolderDtr) {
    	    	...
    	    	for (AxPKById axPKByIdDoc : docs) {
	    		    ...
	    		    for(Axpageh pagina : listPage){
	    		    	if(!esFirmado(pagina) && esPDF(pagina)){
	    		    		listPageTemp.add(pagina);
	    		    	}
	    		    }
		    		
	    		    if(!listPageTemp.isEmpty()){
	    		    	axdoch.setPages(listPageTemp);
	    		    	result.add(axdoch);
	    		    }    		    	
	    		}
	...
	private boolean esPDF(Axpageh pagina) {
		return (null != pagina.getName() && "PDF".equalsIgnoreCase(FileUtils.getExtension(pagina.getName())));
	}

	private boolean esFirmado(Axpageh pagina) {
		return (null != pagina.getFechaFirma() && StringUtils.isNotEmpty(new String(pagina.getFechaFirma())));
	}
}