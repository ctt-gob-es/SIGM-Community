...
//    private static final String DEFAULT_ENTITY = "001";
    private static final String DEFAULT_APP = "RPMSSSI";
    ...
    public void init( FilterConfig filterConfig) {
    	LOG.trace("Entrando en LoginCheckFilter.init(FilterConfig)" + " para iniciar el filtro de configuración");
...
//			    	ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_ENTIDAD, DEFAULT_ENTITY);
    			response.sendRedirect(AutenticacionBackOffice.obtenerUrlLogin( request, DEFAULT_APP));
    			exit = true;
...
//		    		ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_ENTIDAD, DEFAULT_ENTITY);    			
    			...
    			try {
    				if (!containsPathToIgnore(request.getRequestURI()) && !AuthenticationHelper.authenticate(request)) {
    					
    					LOG.info("LoginCheckFilter mySession: " + mySession);
    					LOG.info("LoginCheckFilter request.getRequestURI(): " + request.getRequestURI());
...
//							DEFAULT_ENTITY);
    			...
    			// Si falla, redireccionamos a la página de error
    				...
    				LOG.error(ErrorConstants.AUTHENTICATION_ERROR_MESSAGE + ". Código: " + rpGenericException.getCode().getCode() + " . Mensaje: " + rpGenericException.getShortMessage());
    				es.msssi.sgm.registropresencial.utils.Utils.redirectToErrorPage( rpGenericException, null, null);
    			} catch (RPBookException rpBookException) {
    				LOG.error(ErrorConstants.AUTHENTICATION_ERROR_MESSAGE + ".Código: " + rpBookException.getCode().getCode() + " .Mensaje: " + rpBookException.getShortMessage());
    				es.msssi.sgm.registropresencial.utils.Utils.redirectToErrorPage( rpBookException, null, null);
    			}
    		}
    ...
    private boolean containsPathToIgnore( String requestURI) {
    	...
    	while (tokenizer.hasMoreElements() && !excludePath) {
    		String ignorePath = tokenizer.nextToken();
    		
    		if (requestURI.indexOf(ignorePath) != -1) { 
    			excludePath = true;
    			LOG.info("Path: " + ignorePath + " excluído");
    		}
    	}
    ...
    private void init( ServletContext context) {
    	...
    	LOG.trace("Entrando en LoginCheckFilter.init(ServletContext)" + " para iniciar el contexto de la aplicación");
    	...
    	try {
    		...
    		es.msssi.sgm.registropresencial.utils.Utils.redirectToErrorPage( null, null, namingException);
    	}
    }
}