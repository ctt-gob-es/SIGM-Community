 ...
 * @author cmorenog
 */
public class SearchInputRegisterFormAction extends GenericActions implements
		Serializable {
	private static final long serialVersionUID = 1L;
	
	private static final Logger LOG = Logger.getLogger(SearchInputRegisterFormAction.class.getName());
	...
	public void lastRegister() {
		((SearchInputRegisterBo) inputRegisterBo).setSearchInputRegister(new SearchInputRegisterBean());
		((SearchInputRegisterBo) inputRegisterBo).setLastRegister(true);
	}
		...
		if (selectedResults != null && selectedResults.length > 0) {
			...
			try {
				...
				if (listOut.size() > 0) {
					...
					FacesMessage message = new FacesMessage( FacesMessage.SEVERITY_INFO, "Cerrar Registros", result);
					RequestContext.getCurrentInstance().showMessageInDialog(message);
				}
			} catch (RPRegisterException e) {
			...
			ValidationBo.showDialog("Cerrar", new FacesMessage( FacesMessage.SEVERITY_WARN, "Cerrar", "Debe seleccionar mínimo un registro"));
		}
	}
		...
		for (RowSearchInputRegisterBean selectedResultBean : selectedResults) {
			try {
				if (!("S".equals(selectedResultBean.getFld1001()))) {
					...
					inputRegisterBeanBo.updateOnlyFolder(useCaseConf, book.getIdocarchhdr().getId(), selectedResultBean.getFdrid(), inputRegisterBean);
				}
	...
	public void moverReg() throws SessionException {
		...
		for (RowSearchInputRegisterBean selectedResultBean : selectedResults) {
			...
			try {
				registerBo = new RegisterBo();
				registerBo.moveRegister(selectedResultBean.getFdrid(), book.getIdocarchhdr().getId(), bookDestination);
				FacesMessage message = new FacesMessage( FacesMessage.SEVERITY_INFO, "Copiado", "Se ha copiado el registro correctamente");
				RequestContext.getCurrentInstance().showMessageInDialog(message);
				
			} catch (RPRegisterException rPRegisterException) {
				LOG.error(ErrorConstants.MOVE_REGISTER_ERROR + ". Código: " + rPRegisterException.getCode().getCode() + " . Mensaje: " + rPRegisterException.getShortMessage());
