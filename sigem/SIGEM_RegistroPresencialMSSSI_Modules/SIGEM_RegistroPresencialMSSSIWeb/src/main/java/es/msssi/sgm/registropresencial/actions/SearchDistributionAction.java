...
public class SearchDistributionAction extends GenericActions implements Serializable {
	...
	public SearchDistributionAction() throws SessionException {
		...
		int pageSize = Integer.parseInt(Configurator.getInstance().getProperty( ConfigurationKeys.KEY_DESKTOP_DEFAULT_PAGE_TABLE_RESULTS_SIZE));
		searchDistributionBo.load(1, pageSize, null, null, null);
		limpiar();
		initDistribution();
	}
			...
			if (selectedResults == null || selectedResults.length == 0) {
				...
				if (orgIds != null && orgIds.size() > 0) {
					...
					if(1 == typeDestinoRedis){						
						...
						this.setSelectdestinoRedisUsuarios(Utils.defaultUser(this.listUsuarios, orgIds.get(0)));						
	 ...
	 */
	public void setSearchDistributionBo( LazyDataModel<RowSearchDistributionBean> searchDistributionBo) {
	 ...
	 *            valor del campo a guardar.
	 */
	public void setSelectedResult(RowSearchDistributionBean selectedResult) {
	 ...
	 *            valor del campo a guardar.
	 */
	public void setSelectedResults(RowSearchDistributionBean[] selectedResults) {
	...
	public void initDistribution() {
		...
		try {
			countResult = ((SearchDistributionBo) searchDistributionBo).countSearchDistributionRegister();
			
		} catch (RPDistributionException e) {
			LOG.error(ErrorConstants.GET_COUNT_DISTRIBUTION_ERROR_MESSAGE + ". Código: " + e.getCode().getCode() + " . Mensaje: " + e.getShortMessage());
			Utils.redirectToErrorPage(e, null, null);
			
		} catch (RPGenericException e) {
			LOG.error(ErrorConstants.SAVE_INPUT_REGISTER_ERROR_MESSAGE + ". Código: " + e.getCode().getCode() + " . Mensaje: " + e.getShortMessage());
			Utils.redirectToErrorPage(e, null, null);
		}
	}
	 ...
	 * */
	public void onRowSelectNavigateBottomOpen() throws BookException, SessionException, ValidationException {
	 ...
	 *             Si se produce un error al tratar la información del libro.
	 * */
	public void onRowSelectNavigateOpen(SelectEvent event) throws BookException, SessionException, ValidationException {
	 ...
	 *             Si se produce un error al tratar la información del libro.
	 * */
	private void goToOpenRegister(RowSearchDistributionBean result) throws BookException, SessionException, ValidationException {
		
		BooksBo booksBo = new BooksBo();
		ScrRegstate book = null;
		
		try {
			if (FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(KeysRP.J_BOOK) != null) {
				book = (ScrRegstate) FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(KeysRP.J_BOOK);

				if (book.getIdocarchhdr().getId() == null || (book.getIdocarchhdr().getId() != null && !book.getIdocarchhdr().getId().equals(result.getIdArch()))) {

					book = booksBo.getBook(useCaseConf.getSessionID(), result.getIdArch());
					FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put(KeysRP.J_BOOK, book);
			...
			LOG.error(ErrorConstants.GET_BOOK_ERROR_MESSAGE + ". Código: " + rpBookException.getCode().getCode() + " . Mensaje: " + rpBookException.getShortMessage());
			Utils.redirectToErrorPage(rpBookException, null, null);
			
		} catch (RPGenericException rpGenericException) {
			LOG.error(ErrorConstants.GET_BOOK_ERROR_MESSAGE + ". Código: " + rpGenericException.getCode().getCode() + " . Mensaje: " + rpGenericException.getShortMessage());
		...
		try {
			ParamBookBean param = permissionsBo.getPermission(useCaseConf, FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(KeysRP.J_PARAMBOOK), result.getIdArch(), true);
			FacesContext.getCurrentInstance().getExternalContext().getSessionMap().put(KeysRP.J_PARAMBOOK, param);
		...
		FacesContext.getCurrentInstance().getExternalContext().getFlash().put("distribucion", true);
		Map<String, Object> parameter = new HashMap<String, Object>();
		parameter.put("registerSelect", selectedResults[0].getIdFdr());
		...
		if (selectedResults != null && selectedResults.length > 0) {
			...
			try {
				if ("1".equals(tipoAceptacion)) {
					...
					distributionBo.accept(useCaseConf, selectedResults, Integer.valueOf(-1), tipoRegistros);
					
				} else {
					// List<ScrDistreg> listDistr =
					distributionBo.accept(useCaseConf, selectedResults, libroSelecAceptar, tipoRegistros);
				}

			} catch (RPDistributionException e) {
				LOG.error(ErrorConstants.ACCEPT_DISTRIBUTION_ERROR_MESSAGE, e);
				FacesMessage message = new FacesMessage( FacesMessage.SEVERITY_INFO, "Aceptar Distribución", e.getMessage());
				RequestContext.getCurrentInstance().showMessageInDialog(message);
			}

		} else {
			ValidationBo.showDialog("Aceptar", new FacesMessage( FacesMessage.SEVERITY_WARN, "", "Debe seleccionar mínimo un registro"));
		}
	}
	...
	public void rechazarDist() {
		...
		try {
			// List<ScrDistreg> listDistr =
			distributionBo.reject(useCaseConf, selectedResults, motivoRechazo);
			
		} catch (RPDistributionException e) {
			LOG.error(ErrorConstants.REJECT_DISTRIBUTION_ERROR_MESSAGE, e);
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Rechazar Distribución", e.getMessage());
			RequestContext.getCurrentInstance().showMessageInDialog(message);
	...
	public void isSelect() {
		if (selectedResults == null || selectedResults.length == 0) {
			ValidationBo.showDialog("Error", new FacesMessage( FacesMessage.SEVERITY_WARN, "", "Debe seleccionar mínimo un registro"));
	...
	public void isSelectAccept() {
		...
		if (selectedResults == null || selectedResults.length == 0) {
			...
			if (countInput > 0 && countOutput > 0) {
				ValidationBo.showDialog( "Error", new FacesMessage( FacesMessage.SEVERITY_WARN, "", "No puede aceptar registros de distintos tipos. Seleccione registros de entrada o de salida"));
			} else {
				reinitDis();
	...
	public void cambiarDestDist() {
		...
		try {
			distributionBo.change(useCaseConf, selectedResults, destinoCambio, ((SearchDistributionBo) searchDistributionBo).getSearchDistributionRegister().getType());
			
		} catch (RPDistributionException e) {
			LOG.error(ErrorConstants.CHANGE_DISTRIBUTION_ERROR_MESSAGE, e);
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Cambio de destino", e.getMessage());
			RequestContext.getCurrentInstance().showMessageInDialog(message);
	...
	public void redistribuirDist() {
		...
		try {
			...
			distributionBo.redistribution(useCaseConf, selectedResults, ((SearchDistributionBo) searchDistributionBo).getSearchDistributionRegister().getType(), motivoRedireccion, typeDestinoRedis, userId);
			
		} catch (RPDistributionException e) {
			LOG.error(ErrorConstants.REDISTRIBUTION_DISTRIBUTION_ERROR_MESSAGE, e);
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Redistribuir Distribución", e.getMessage());
			RequestContext.getCurrentInstance().showMessageInDialog(message);
	...
	public void archivarDist() {
		...
		if (selectedResults != null && selectedResults.length > 0) {
			...
			try {
				// List<ScrDistreg> listDistr =
				distributionBo.archive(useCaseConf, selectedResults);
				
			} catch (RPGenericException e) {
				LOG.error(ErrorConstants.ARCHIVE_DISTRIBUTION_ERROR_MESSAGE, e);
				FacesMessage message = new FacesMessage( FacesMessage.SEVERITY_INFO, "Archivar Distribución", e.getMessage());
				RequestContext.getCurrentInstance().showMessageInDialog(message);
			...
			ValidationBo.showDialog("Archivar", new FacesMessage( FacesMessage.SEVERITY_WARN, "", "Debe seleccionar mínimo un registro"));
		}
	}
	 ...
	 *            valor del campo a guardar.
	 */
	public void setDestinoCambio(ScrOrg destinoCambio) {
	 ...
	 *            valor del campo a guardar.
	 */
	public void setMotivoRedireccion(String motivoRedireccion) {
	...
	public void updateOrigin() {
		...
		if (FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("UNITSDIALOG") != null) {
			...
			((SearchDistributionBo) searchDistributionBo).getSearchDistributionRegister().setFld7(unidad);
		}
	}
	...
	public void updateDestination() {
		...
		if (FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("UNITSDIALOG") != null) {
			unidad = (ScrOrg) FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("UNITSDIALOG");
			((SearchDistributionBo) searchDistributionBo).getSearchDistributionRegister().setFld8(unidad);
		}
	}
