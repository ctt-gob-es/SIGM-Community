...
public class LdapUsersTable
{
   ...
   /**
    * Construye una expresión de búsqueda para la tabla base de usuarios a
    * partir del nombre ldap.
    *
    * @param fullName Nombre completo del usuario ldap.
    * @return La expresión mencionada.
    */

   public String getLoadBaseByFullName(String fullName)
   {
      String qual;

      qual = "WHERE " + getBaseTableName() + "." +
             UasDaoLdapUserTbl.CD_LDAPFULLNAME.getName() + "='" + fullName + "'";

      if (_logger.isDebugEnabled())
         _logger.debug("getLoadBaseByFullName: " + qual);
