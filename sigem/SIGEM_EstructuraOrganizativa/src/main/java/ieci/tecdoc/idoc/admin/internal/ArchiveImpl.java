...
public class ArchiveImpl implements Archive {
		...
		try {
			...
			if (existsFdrs(entidad)) {
				if (archUpdInfo.isModifyDefFlds())
					AdminException
							.throwException(ArchiveErrorCodes.EC_ARCH_WITH_FDRS);
	...
	public void updateParentId(int parentId, int archiveId, String entidad)
		...
		try {
			...
			if (!isValidParentId(parentId, entidad))
				AdminException
						.throwException(ArchiveErrorCodes.EC_PARENT_NO_ID);
	...
	public Integer insertXNextIdDocValues(DbInputStatement statement,
		...
		statement
				.setLongInteger(index++, Defs.NEXT_ID_XNID_TABLE_TYPE_IDOC_DOC);
		statement.setLongInteger(index++, _id);
		statement.setLongInteger(index++, 1);
	...
	private void loadArchiveHdr(DbConnection dbConn) throws Exception {
		...
		try {
			...
			if (!DynamicFns.select(dbConn, table.getLoadArchIdQual(_id),
					tableInfo, rowsInfo)) {
				AdminException
						.throwException(ArchiveErrorCodes.EC_ARCH_NOT_EXISTS);
			}
	...
	private void getUpdFldsIdxsDef(ArchiveFldsImpl flds, ArchiveIdxsImpl idxs)
		...
		if (flds == null || idxs == null)
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_PARAM_NO_VALID);
				...
				for (j = 0; j < fldsId.size(); j++) {
					...
					for (h = 0; h < delFlds.size(); h++) {
						if (id.equals(delFlds.get(h)))
							AdminException
									.throwException(ArchiveErrorCodes.EC_ARCH_IDXS_DEL_FIELDS);
					}
				}
	...
	private String getVldDetValue(boolean isUpdate) throws Exception {
		...
		// campos
		sb.append("\"").append(ver).append("\"|").append(Defs.NULL_ID).append("|0");
		...
		for (int i = 0; i < flds.count(); i++) {
			...
			case IeciTdType.LONG_INTEGER:
				...
				sbVal.append(",").append(fld.getType()).append(",\".\",\"\",0|0,").append(Defs.NULL_ID);
				numValidaciones++;
				break;
			case IeciTdType.DATE_TIME:
				sbVal.append("|").append(fld.getId()).append("|").append(ArchiveDefs.IDOC_FLD_FMT_TYPE_DT);
				sbVal.append(",").append(fld.getType()).append(",\"-\",\"\",\"\",1,0|0,").append(Defs.NULL_ID);
				numValidaciones++;
				break;
			case IeciTdType.TIMESTAMP:
				sbVal.append("|").append(fld.getId()).append("|").append(ArchiveDefs.IDOC_FLD_FMT_TYPE_DT);
				sbVal.append(",").append(fld.getType()).append(",\"-\",\":\",\" \",1,1|0,").append(Defs.NULL_ID);
				numValidaciones++;
				break;
			}
		}
	...
	private void checkCanDeleteArchive(String entidad) throws Exception {
		...
		try {
			...
			if (count > 0) {
				AdminException
						.throwException(ArchiveErrorCodes.EC_ARCH_HAS_FOLDERS);
			}
			...
			if (count > 0) {
				AdminException
						.throwException(ArchiveErrorCodes.EC_ARCH_HAS_VALIDATION_TABLE);
			}
		} catch (Exception e) {
			_logger.error(e);
			throw e;
	...
	private void checkArchiveExists(DbConnection dbConn) throws Exception {
		...
		try {
			...
			if (count > 0) {
				...
				if (count > 0)
					AdminException
							.throwException(ArchiveErrorCodes.EC_ARCH_EXITS);
			}
		} catch (Exception e) {
			_logger.error(e);
			throw e;
	...
	private void validateArchive(boolean update, String entidad)
		...
		if ((remarks != null && remarks.indexOf("\"") >= 0)
			...
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_REMARKS_EXIST_QUOTES);
		}
		...
		if (!isValidIdxsDef(flds, idxs)) {
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_IDXS_BAD_FIELDS);
		}
		...
		if (!isValidTxtFldsDef(flds)) {
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_FLD_TXT_BAD_LEN);
		}

		// se valida que los campos documentales sean SHORT_TEXT ó LONG_TEXT
		if (!isValidFtsFldsDef(flds)) {
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_FTS_BAD_FIELDS);
		}
		...
		if (update && _updInfo.isModifyDefFlds() && existsValidation(entidad)) {
			AdminException
					.throwException(ArchiveErrorCodes.EC_ARCH_EXISTS_FLDS_WITH_VALIDATION);
		}
