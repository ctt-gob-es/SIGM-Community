...
<xsl:include href="../templates_comunes.xsl" /><xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:template match="/"  xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			validar[2] = new Array('asunto','<xsl:value-of select="$lang.asunto"/>');
			validar[3] = new Array('numExpediente', '<xsl:value-of select="$lang.numexp"/>');
			
			validar[4] = new Array('nif_repre','<xsl:value-of select="$lang.nif_repre"/>');

			validar[5] = new Array('nombre_repre','<xsl:value-of select="lang.nombre_repre"/>');
			
			validar[6] = new Array('expone','<xsl:value-of select="$lang.expone"/>');
			validar[7] = new Array('solicita','<xsl:value-of select="$lang.solicita"/>');


			//Array con los datos especificos del formilario -> -> ('id_campo','tag_xml')
			//----------------------------------------------
			var especificos = new Array(23);
			...
			especificos[12] = new Array('nif_repre','nif_repre');
			especificos[13] = new Array('nombre_repre','nombre_repre');
			especificos[14] = new Array('domicilio_repre','domicilio_repre');
			especificos[15] = new Array('ciudad_repre','ciudad_repre');
			especificos[16] = new Array('c_postal_repre','c_postal_repre');
			especificos[17] = new Array('region_repre','region_repre');
			especificos[18] = new Array('movil_repre','movil_repre');
			especificos[19] = new Array('d_email_repre','d_email_repre');
			
			especificos[20] = new Array('asunto','asunto');
			
			especificos[21] = new Array('expone','expone');
			especificos[22] = new Array('solicita','solicita');
			...
			function getAsuntoNumexp(numExpediente){
				window.open('tramites/001/SUBSANACION_JUSTIFICACION/buscaAsuntoNumExp.jsp?valor='+numExpediente+';001','','width=1,height=1,left=10000,top=10000');
			}
