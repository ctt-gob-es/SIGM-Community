	...
	<xsl:template match="/"  xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			function getDatosPersona(nif){
				//if (validarCampoNIF()){
					window.open('tramites/001/CERT_PADRON/buscaPersonaPadron.jsp?valor='+nif+';130408','','width=3,height=3');
				//}
			}
		...
		<div class="cuadro" style="">
			...
			<xsl:variable name="fileTiposCert" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosVldtblConSustitutoByOrden('VLDTBL_CERT_PADRON','001')"/>
			...
			<!-- CAMBIAR ID_ENTIDAD DEPENDIENDO DEL AYUNTAMIENTO -->
			<xsl:variable name="fileTiposFamiliar" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosVldtblConSustitutoByOrden('VLDTBL_CERT_PADRON_FAMILIAR','001')"/>
		...
		<xsl:call-template name="TEXTO_COMPARECE_COMUN" />
			...
			<input type="hidden">
				...
				<xsl:attribute name="value">130408</xsl:attribute>
			</input>
