...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.mensajeRepresentante" select="'No tiene ningún representado dado de alta en Comparece para este NIF/NIE o bien su solicitud de alta de representación está siendo tramitada en el departamento correspondiente.&lt;br&gt;Si no ha realizado dicha solicitud cierre el navegador, acceda a Comparece y rellene el formulario &lt;b&gt;Solicitud de alta, modificación y baja de datos de usuarios en Comparece para AYUNTAMIENTOS, ASOCIACIONES Y EMPRESAS&lt;/b&gt;.'"/>
	...
	<xsl:template match="/"  xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			function getDatosObligado(nif){
				window.open('tramites/001/SUBSANACION_JUST_REPRESENT/buscaObligado.jsp?valor='+nif+';001','','width=3,height=3');
			}
			...
			function getAsuntoNumexp(numExpediente){
				window.open('tramites/001/SUBSANACION_JUST_REPRESENT/buscaAsuntoNumExp.jsp?valor='+numExpediente+';001','','width=700,height=300,resizable=1');
			}
			function getDatosObligadoEntidad(nif){
				window.open('tramites/001/SUBSANACION_JUST_REPRESENT/buscaObligadoEntidad.jsp?valor='+nif+';001','','width=3,height=3');
			}
			function getDatos(anio){
				...
				window.open('tramites/001/SUBSANACION_JUST_REPRESENT/dameExpedientes.jsp?valor='+dni+';'+anio.value+';001','','width=3,height=3,left=10000,top=10000');
			}

			function getRepresentados(){
				window.open('tramites/001/SUBSANACION_JUST_REPRESENT/dameRepresentantes.jsp?valor='+document.getElementById('documentoIdentidad').value+';001','','width=3,height=3,left=10000,top=10000');
			}
