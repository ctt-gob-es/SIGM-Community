...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'SOLICITUD DE ALTA A LOS CONVENIOS CULTURALES'"/>
	...
	<xsl:variable name="lang.anexarInfo11" select="'Para los artistas individuales:'"/>
	<xsl:variable name="lang.anexarInfo1a" select="'a) Ficha de inscripción según modalidades, que se encuentra en el siguiente enlace:'"/>
	<xsl:variable name="lang.anexarInfo1a2Enlace" select="'http://www.dipucr.es/index.php/documentos-cultura/convenios-culturales/normativa'"/>
	<xsl:variable name="lang.anexarInfo1a3" select="'deben enviarla en formato Word para poder editarla (dicha ficha será la que aparezca en la página Web).'"/>
	<xsl:variable name="lang.anexarInfo1b" select="'b) Los nacidos en la provincia de Ciudad Real:'"/>
	<xsl:variable name="lang.anexarInfo1b2" select="'Fotocopia del DNI.'"/>
	<xsl:variable name="lang.anexarInfo1c" select="'c) Los nacidos fuera de la provincia de Ciudad Real, residentes en la misma:'"/>
	<xsl:variable name="lang.anexarInfo1c2" select="'Certificado de Empadronamiento, donde conste que llevan residiendo en la provincia un mínimo de 3 años, anteriores al momento de la inscripción.'"/>

	<xsl:variable name="lang.anexarInfo12" select="'Para los grupos y asociaciones:'"/>
	<xsl:variable name="lang.anexarInfo2a" select="'a) Ficha de inscripción según modalidades, que se encuentra en el siguiente enlace:'"/>
	<xsl:variable name="lang.anexarInfo2a2Enlace" select="'http://www.dipucr.es/index.php/documentos-cultura/convenios-culturales/normativa'"/>
	<xsl:variable name="lang.anexarInfo2a3" select="'deben enviarla en formato Word para poder editarla (dicha ficha será la que aparezca en la página Web).'"/>
	<xsl:variable name="lang.anexarInfo2b" select="'b) Copia de la tarjeta de identificación fiscal, con su correspondiente C.I.F. (Código de Identificación Fiscal), que acredite que el domicilio fiscal corresponde a la provincia de Ciudad Real.'"/>
	<xsl:variable name="lang.anexarInfo2c" select="'c) Copia del apartado de los estatutos que identifique la finalidad y su carácter sin ánimo de lucro (por lo cual quedaría exenta de IVA).'"/>

	<xsl:variable name="lang.anexarInfo2" select="'2.- Para adjuntar los ficheros, pulse el botón examinar. Seleccione el fichero que desee anexar a la solicitud. '"/>
	<xsl:variable name="lang.anexarInfo21" select="'Recuerde que debe anexar copia de la Memoria o proyecto al que se refiere la presente solicitud y de cualquier otro que considere conveniente.'"/>
	<xsl:variable name="lang.anexarInfo22" select="'Si el documento está en soporte papel, debe escanearlo con carácter previo.'"/>
	<xsl:variable name="lang.anexarInfo23" select="'Los originales quedan a disposición de la Diputación Provincial durante toda la tramitación del expediente.'"/>
	...
	<xsl:template match="/"  xmlns:java="http://xml.apache.org/xslt/java">
		...
		<br/>
   		...
   		<div class="cuadro" style="">	
			...
			<xsl:variable name="convocatoria" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosConsulta('Convocatoria', 'select numexp, asunto from spac_expedientes where codprocedimiento IN ( select valor from DPCR_EXP_ALTA_CONVENIOS_CULT) and fcierre is null and estadoadm=#PR#','001')"/>
...
document.getElementById('solicita').value = 'Que se conceda la más alta subvención posible.';
			...
			<div class="submenu">
   			...
   			<div class="cuadro" style="">
				<label class="gr">
			   		<xsl:attribute name="style">position: relative; width:650px; text-align:justify;</xsl:attribute>
		   			<xsl:value-of select="$lang.anexarInfo1"/>
					<br/>
					<li>
						<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>     
						<b><xsl:value-of select="$lang.anexarInfo11"/></b>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1a"/>
						</span>
						<br/>						
						<a target="_blank">
							<xsl:attribute name="href"><xsl:value-of select="$lang.anexarInfo1a2Enlace"/></xsl:attribute>
							<xsl:attribute name="style">padding-left:25px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1a2Enlace"/>
						</a>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1a3"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1b"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:25px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1b2"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1c"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:25px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo1c2"/>
						</span>
					</li>
					<br/>
					<li>
						<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>     
						<b><xsl:value-of select="$lang.anexarInfo12"/></b>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo2a"/>
						</span>
						<br/>						
						<a target="_blank">
							<xsl:attribute name="href"><xsl:value-of select="$lang.anexarInfo1a2Enlace"/></xsl:attribute>
							<xsl:attribute name="style">padding-left:25px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo2a2Enlace"/>
						</a>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo2a3"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo2b"/>
						</span>
						<br/>
						<span>
							<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
							<xsl:value-of select="$lang.anexarInfo2c"/>
						</span>
					</li>
		   			<br/>
					...
					<span>
						<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
			   			<xsl:value-of select="$lang.anexarInfo21"/>
		   			</span>
					<br/>
					<span>
						<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
			   			<xsl:value-of select="$lang.anexarInfo22"/>
		   			</span>
					<br/>
					<span>
						<xsl:attribute name="style">padding-left:10px; text-align:justify;</xsl:attribute>
			   			<xsl:value-of select="$lang.anexarInfo23"/>
					</span>
				</label>
			<br/><br/>
