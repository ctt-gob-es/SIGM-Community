...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'Formulario de solicitud de Anulación de Liquidación de IBI por Error en el Sujeto Pasivo'"/>
	...
	<xsl:variable name="lang.expone2" select="', que habiendo recibido las/s liquidación/es de IBI que a continuación se indicarán correspondientes al municipio de: '"/>
	<xsl:variable name="lang.expone3" select="', y dado que se ha apreciado error en la determinación del sujeto pasivo por la causa que se expresa:'"/>
	<xsl:variable name="lang.motivo1" select="'1 - Vivienda de Protección Oficial (IBI-URBANA)'"/>
	<xsl:variable name="lang.motivo2" select="'2 - Empresas de Construcción y Promoción Inmobiliaria (IBI-URBANA)'"/>
	...
	<xsl:variable name="lang.notas_motivos" select="'Opción 4: Indicar cuál:'"/>
	...
	<xsl:variable name="lang.solicita2" select="', que previa las comprobaciones oportunas, procedan a anular la/s liquidación/es indicadas y, en el caso que proceda (seleccionar la opción):'"/>

	<xsl:variable name="lang.opcion1" select="'A emitir liquidación sólo por los años en los que es titular del inmueble.'"/>
	<xsl:variable name="lang.opcion2" select="'A la devolución del/los año/s'"/>
	...
	<xsl:variable name="lang.notas3" select="'3.- En los casos de adquisición o venta deberá aportarse copia de escritura de compraventa o nota simpre del registro o resolución del catastro.'"/>
	<xsl:variable name="lang.notas4" select="'4.- En el caso de que la solicitud venga referida a liquidaciones a nombre de otro obligado tributario, deberá acreditarse la representación legal o voluntaria (ver modelo en la página) del mismo.'"/>
	...
	<xsl:template match="/" xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			especificos[123] = new Array('iban','iban');

			especificos[124] = new Array('texto_legal_comun','texto_legal_comun');
			especificos[125] = new Array('texto_datos_personales_comun','texto_datos_personales_comun');
			...
			function verificacionesEspecificas() {
				...
				if(document.getElementById('opcion').value == '2'){
					if(document.getElementById('ccc1').value==''){
						alert('Debe introducir los años de devolución y los datos de la cuenta corriente');
			...
			function getDatosObligado(nif){
				window.open('tramites/001/REC_ESP/buscaObligado.jsp?valor='+nif+';001','','width=3,height=3');
			}
			function getDatosRepresentante(nif){
				window.open('tramites/001/REC_ESP/buscaRepresentante.jsp?valor='+nif+';001','','width=3,height=3');
			}				
		...
		<xsl:variable name="fileMotBene" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosTablaValidacionConSustituto('REC_VLDTBL_MOT_ERROR_SP_IBI','001')"/>
