...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.notas_motivos1" select="'Se deberá aportar original de la/s liquidación/es ingresada/s.'"/>
	<xsl:variable name="lang.notas_motivos2" select="'Opción 5: Indicar cual:'"/>
	<xsl:variable name="lang.liquidaciones" select="'LIQUIDACIONES PAGADAS CON ANTERIORIDAD*'"/>
	...
	<xsl:variable name="lang.notas3" select="'3.- En el caso de que la solicitud venga referida a liquidaciones a nombre de otro obligado tributario, deberá acreditarse la representación legal o voluntaria (descargar modelo) del mismo.'"/>
	...
	<xsl:template match="/">
		<script language="Javascript">
			...
			especificos[89] = new Array('iban','iban');

			especificos[90] = new Array('texto_legal_comun','texto_legal_comun');
			especificos[91] = new Array('texto_datos_personales_comun','texto_datos_personales_comun');
			...
			function getDatosObligado(nif){
				window.open('tramites/001/REC_03/buscaObligado.jsp?valor='+nif+';001','','width=3,height=3');
			}
			function getDatosRepresentante(nif){
				window.open('tramites/001/REC_03/buscaRepresentante.jsp?valor='+nif+';001','','width=3,height=3');
			}
