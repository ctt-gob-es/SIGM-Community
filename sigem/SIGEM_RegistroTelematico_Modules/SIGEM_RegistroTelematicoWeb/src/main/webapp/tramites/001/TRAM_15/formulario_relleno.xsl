...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.datosSolicitud" select="'Datos de la Reclamación'"/>
	...
	<xsl:template match="/">
		...
		<xsl:variable name="desc" select="Solicitud_Registro/Datos_Firmados/Datos_Especificos/Descripcion"/>		
        ...
        <xsl:otherwise>
			<div class="col" style="height:55px">
				...
				<label class="gr" style="position: relative; width:400px;">
					<xsl:value-of select="Solicitud_Registro/Datos_Firmados/Datos_Especificos/Descripcion"/>
				</label>
				<br/>
			</div>
		</xsl:otherwise>
