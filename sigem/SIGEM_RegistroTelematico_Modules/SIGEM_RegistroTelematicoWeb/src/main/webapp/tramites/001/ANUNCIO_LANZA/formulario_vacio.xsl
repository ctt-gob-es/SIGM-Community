...
<xsl:output encoding="ISO-8859-1" method="html"/>
	<xsl:variable name="lang.titulo" select="'Remisión de anuncios al Diario Lanza'"/>
	...
	<xsl:variable name="lang.avisoDoc" select="'Es obligatorio adjuntar un documento con el texto deseado para el anuncio, en formato DOC (Microsoft Word) o ODT (OpenOffice Text).'"/> 
	<xsl:variable name="lang.avisoParticular" select="'Si usted es un particular, inserte sus datos personales.'"/>
	
	<xsl:variable name="lang.organoAsignado" select="'Órgano al que se dirige: Registro del Diario Lanza Digital'"/>
	...
	<xsl:variable name="lang.anexarInfo3" select="'(Máximo 20 mbs total)'"/>
	...
	<xsl:template match="/">
		<script language="Javascript">
			...
			validar_NO[0] = new Array('LANZA_D1','<xsl:value-of select="$lang.documento1"/>');
			validar_NO[1] = new Array('LANZA_D2','<xsl:value-of select="$lang.documento2"/>');
			...
			especificos[11] = new Array('solicitarEnvio','Solicitar_Envio');
			especificos[12] = new Array('direccionElectronicaUnica','Direccion_Electronica_Unica');

			especificos[13] = new Array('texto_legal_comun','texto_legal_comun');
			especificos[14] = new Array('texto_datos_personales_comun','texto_datos_personales_comun');
			...
			function getDatosTercero(nif){
				window.open('tramites/001/ANUNCIO_LANZA/buscaTercero.jsp?valor='+nif+';001','','width=3,height=3');
			}
	   	...
	   	<div class="cuadro" style="">
			...
			<div class="col">
				...
				<input type="file">
					...
					<xsl:attribute name="name">LANZA_D1</xsl:attribute>
					<xsl:attribute name="id">LANZA_D1</xsl:attribute>
					<xsl:attribute name="value"></xsl:attribute>
				</input>
				<input type="hidden">
					<xsl:attribute name="name">LANZA_D1_Tipo</xsl:attribute>
					<xsl:attribute name="id">LANZA_D1_Tipo</xsl:attribute>
			...
			<div class="col">
				...
				<input type="file">
					<xsl:attribute name="style">position: relative; width:500px; </xsl:attribute>
					<xsl:attribute name="name">LANZA_D2</xsl:attribute>
					<xsl:attribute name="id">LANZA_D2</xsl:attribute>
					<xsl:attribute name="value"></xsl:attribute>
				</input>
				<input type="hidden">
					<xsl:attribute name="name">LANZA_D2_Tipo</xsl:attribute>
					<xsl:attribute name="id">LANZA_D2_Tipo</xsl:attribute>
