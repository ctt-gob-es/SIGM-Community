...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'CONVOCATORIA DE SUBVENCIONES Y ACTIVIDADES A E.E.L.L.'"/>
	...
	<xsl:template match="/"  xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			especificos[30] = new Array('texto_legal_comun','texto_legal_comun');
			especificos[31] = new Array('texto_datos_personales_comun','texto_datos_personales_comun');
...
//				document.getElementById('domicilioNotificacion').value = document.getElementById('domicilio_repre').value;
			...
			function compruebaSolicitud(valueConvocatoria){
				...
				if(valueAyto == '' || valueAyto == '-------------------------'){
					...
					window.open('tramites/001/CONVSUB_01/compruebaSol.jsp?valor='+document.getElementById('cif').value+';001;'+valueConvocatoria,'','width=3,height=3,top='+(screen.height/2-100)+',left='+(screen.width/2-100));
				}
			}
		</script>
