...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'CONVOCATORIA DE SUBVENCIONES Y ACTIVIDADES PARA PERSONAS FÍSICAS'"/>
...
//				document.getElementById('domicilioNotificacion').value = document.getElementById('domicilio_repre').value;
		...
		<h1><xsl:value-of select="$lang.titulo"/></h1>
   		...
   		<div class="cuadro" style="">
			...
			<xsl:variable name="convocatoria" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosConsulta('Convocatoria', 'select numexp, asunto from spac_expedientes where codprocedimiento IN ( select valor from DPCR_CONV_EXP_CONV_CIU) and fcierre is null and estadoadm=#PR#','001')"/>
