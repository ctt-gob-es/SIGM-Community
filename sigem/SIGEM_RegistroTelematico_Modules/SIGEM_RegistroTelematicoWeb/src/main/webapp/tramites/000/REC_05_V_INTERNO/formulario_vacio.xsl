...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.expone2" select="', que considerando que tiene derecho al disfrute del beneficio fiscal en el IBI que se indica correspondientes al municipio de: '"/>
	...
	<xsl:template match="/" xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			function verificacionesEspecificas() {
				...
				if(document.getElementById('otro_motivo').value == ''){
					alert('Debe introducir el motivo de la bonificación');
					document.getElementById('otro_motivo').focus();
					return false;
				}
			...
			function getDatosObligado(nif){
				window.open('tramites/000/REC_05_V_INTERNO/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}
			function getDatosRepresentante(nif){
				window.open('tramites/000/REC_05_V_INTERNO/buscaRepresentante.jsp?valor='+nif+';000','','width=3,height=3');
			}			
		</script>
