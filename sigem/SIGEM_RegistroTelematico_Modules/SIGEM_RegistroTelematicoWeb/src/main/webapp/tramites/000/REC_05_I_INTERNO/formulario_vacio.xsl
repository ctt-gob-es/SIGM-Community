...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.expone2" select="', que considerando que tiene derecho al disfrute del beneficio fiscal en el IBI por Vivienda de Protección Oficial (IBI-URBANA) correspondientes al municipio de: '"/>
	<xsl:variable name="lang.notas_motivos1" select="'Se deberá aportar copia de la cédula de calificación definitiva y de las escrituras de compra venta.'"/>
	...
	<xsl:template match="/" xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			validar[8] = new Array('ref1_1','<xsl:value-of select="$lang.referencia"/>');
			validar[9] = new Array('ref2_1','<xsl:value-of select="$lang.referencia"/>');
			validar[10] = new Array('ref3_1','<xsl:value-of select="$lang.referencia"/>');
			validar[11] = new Array('ref4_1','<xsl:value-of select="$lang.referencia"/>');
			validar[12] = new Array('sit_1','<xsl:value-of select="$lang.situacion"/>');

			//Array con los datos especificos del formulario -> -> ('id_campo','tag_xml')
			//----------------------------------------------
			var especificos = new Array(153);
			...
			function verificacionesEspecificas() {				
				...
				var i = 1;
				while(i!=21){
					var sit = document.getElementById('sit_'+i);
					if(sit.value != null){
						if(sit.value!=''){
							...
							if((ref1.value == null || ref1.value=='') || (ref2.value == null || ref2.value=='') || (ref3.value == null || ref3.value=='')||(ref4.value == null || ref4.value=='')){
								alert('Es obligatorio indicar la Referencia Castral');
								ref1.focus();
								return false;								
							}
						}
			...
			function getDatosObligado(nif){
				window.open('tramites/000/REC_05_I_INTERNO/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}
			function getDatosRepresentante(nif){
				window.open('tramites/000/REC_05_I_INTERNO/buscaRepresentante.jsp?valor='+nif+';000','','width=3,height=3');
			}			
		</script>
		...
		<xsl:variable name="fileMotBene" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosTablaValidacionConSustituto('REC_VLDTBL_MOT_BENEFICIO_IBI','000')"/>
