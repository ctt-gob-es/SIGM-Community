...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'CONVOCATORIA DE SUBVENCIÓN A E.E.L.L'"/>
...
//				document.getElementById('domicilioNotificacion').value = document.getElementById('domicilio_repre').value;
			...
			function getDatosObligado(nif){
				window.open('tramites/000/CONVSUB_01/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}
			...
			function compruebaSolicitud(valueConvocatoria){
				...
				if(valueAyto == '' || valueAyto == '-------------------------'){
					...
					window.open('tramites/000/CONVSUB_01/compruebaSol.jsp?valor='+document.getElementById('cif').value+';000;'+valueConvocatoria,'','width=3,height=3,left=10000,top=10000');
				}
			}
		</script>
		...
		<br/>
   		...
   		<div class="cuadro" style="">	
			...
			<xsl:variable name="convocatoria" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosConsulta('Convocatoria', 'select numexp, asunto from spac_expedientes where codprocedimiento IN ( select valor from DPCR_CONV_EXP_CONV) and fcierre is null and estadoadm=#PR#','000')"/>
