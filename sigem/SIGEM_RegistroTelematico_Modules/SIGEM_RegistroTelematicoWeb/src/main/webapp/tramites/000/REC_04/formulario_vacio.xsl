...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:variable name="lang.titulo" select="'Formulario de Interposición de Recurso de Reposición.'"/>

	<xsl:variable name="lang.datosSolicitud" select="'Datos del Recurso'"/>
	...
	<xsl:template match="/">
		<script language="Javascript">
			...
			function getDatosObligado(nif){
				window.open('tramites/000/REC_04/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}

			function getDatosRepresentante(nif){
				window.open('tramites/000/REC_04/buscaRepresentante.jsp?valor='+nif+';000','','width=3,height=3');
			}
		</script>
			...
			<br/>
   			<div class="col">
				...
				<input type="file">
					...
					<xsl:attribute name="name">REC-04-D1</xsl:attribute>
					<xsl:attribute name="id">REC-04-D1</xsl:attribute>
					<xsl:attribute name="value"></xsl:attribute>
				</input>
				<input type="hidden">
					<xsl:attribute name="name">REC-04-D1_Tipo</xsl:attribute>
					<xsl:attribute name="id">REC-04-D1_Tipo</xsl:attribute>
