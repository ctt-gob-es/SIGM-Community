...
<%
	...
	String ficheroSolAyto = XmlCargaDatos.getDatosConsulta("SolicitudesAyto", "select a.numexp, case when (select trim(a.numexp || # - # ||d.asunto|| # - # ||a.asunto) from  dpcr_sol_conv_sub c, spac_expedientes d where c.numexp = a.numexp and c.numexp_padre = d.numexp) is null then trim(a.numexp || # - # ||a.asunto) else (select trim(a.numexp || # - # ||d.asunto|| # - # ||a.asunto) from  dpcr_sol_conv_sub c, spac_expedientes d where c.numexp = a.numexp and c.numexp_padre = d.numexp) end from  spac_expedientes a where upper(a.nifciftitular) like upper(#%"+ nifCif +"%#) and a.numexp like #DPCR"+anio+"/%# and codprocedimiento not in (#DPCR-001#,#SOL-AYTO-GEN#,#ACC-TEL-EXP#) and nombreprocedimiento not like #%BOP - Solicitud de inserción de anuncio%# and nombreprocedimiento not like #%Datos del Tercero y Gestión de Representantes%# and nombreprocedimiento not like #%Comunicación administrativa electrónica Registro (Carta digital)%#",entidad);
