...
<xsl:output encoding="ISO-8859-1" method="html"/>
	...
	<xsl:template match="/">
		...
		<xsl:if test="Solicitud_Registro/Datos_Firmados/Datos_Especificos/Organo_Destinatario">
			<div class="col">
				<label class="gr" style="position: relative; width:150px;">
					<xsl:value-of select="$lang.organoAsignado"/>:	
				</label>
				<label class="gr" style="position: relative; width:500px;">
					<xsl:value-of select="Solicitud_Registro/Datos_Firmados/Datos_Especificos/Organo_Destinatario"/>
				</label>
				<br/>
			</div>
		</xsl:if>
