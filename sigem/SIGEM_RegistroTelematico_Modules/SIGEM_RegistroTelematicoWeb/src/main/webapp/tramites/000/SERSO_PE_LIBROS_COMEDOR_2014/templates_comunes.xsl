...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/" xmlns:java="http://xml.apache.org/xslt/java" name="DATOS_FAMILIA">
		...
		<div class="cuadro" style="">	
			...
			<xsl:variable name="convocatoria" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosConsulta('Convocatoria', 'select numexp, asunto from spac_expedientes where codprocedimiento=#PCD-221# and fcierre is null and estadoadm=#PR#','000')"/>
				...
				<script>
					function cambiaColumnas(tipoAyuda){
						if(tipoAyuda == 'LIBROS'){
							...
							document.getElementById("idcolumna").style.width='5%';
							document.getElementById("nombrecolumna").style.width='45%';
							document.getElementById("edadcolumna").style.width='10%';
							document.getElementById("cursocolumna").style.width='0%';
							document.getElementById("costecolumna").style.width='40%';
...
if(document.getElementById('total').value=='')
	document.getElementById('total').value=0;
			...
			<div class="col">
				...
				</input>DESAHUCIADO O EN PROCESO
				<br/>
				<label>      - </label>
				OTRA(Especificar):
				<br/>
	...
	<xsl:template name="TABLA_LIBROS_COMEDOR">
		...
		<xsl:variable name="lang.costeMensual" select="'Coste mensual'"/>
		<xsl:variable name="lang.mesInicio" select="'Mes de inicio del comedor'"/>
		<xsl:variable name="lang.mesesSolComedor" select="'Meses solicitados'"/>
		...
		<div class="col">
			...
			<table style="font-size:12px; margin:5 auto 20 auto;" cellspacing="0" border="1">
				<tr>
					...
					<td style="width:15%;background-color:#dee1e8;text-align:center" id="costecolumna">
						<label class="gr">
		...
		<div class="col" id="datosLibros">
			...
			<input type = "radio">
				...
				<xsl:attribute name="onclick">document.getElementById('pagado').value='SI';</xsl:attribute>
			</input>
			<br/>
			<label>
			...
			<input type = "radio">
				...
				<xsl:attribute name="name">rdPagado</xsl:attribute>
				<xsl:attribute name="id">rdPagado</xsl:attribute>
				<xsl:attribute name="onclick">document.getElementById('pagado').value='NO';</xsl:attribute>
			</input>
			<input type = "hidden">
		...
		<div class="col" id="datosComedor">			
			...
			<label class="gr">
				<xsl:attribute name="style">width:30%;</xsl:attribute>
				<xsl:value-of select="$lang.mesesSolComedor"/>:
			</label>
			<input type="">
				<xsl:attribute name="style">width:80%;</xsl:attribute>
