...
//				document.getElementById('domicilioNotificacion').value = document.getElementById('domicilio_repre').value;
			...
			function getDatosObligado(nif){
				window.open('tramites/000/CONVSUB_ASO/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}
		...
		<br/>
   		...
   		<div class="cuadro" style="">	
			...
			<xsl:variable name="convocatoria" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosConsulta('Convocatoria', 'select numexp, asunto from spac_expedientes where codprocedimiento IN ( select valor from DPCR_CONV_EXP_CONV_ASO) and fcierre is null and estadoadm=#PR#','000')"/>
