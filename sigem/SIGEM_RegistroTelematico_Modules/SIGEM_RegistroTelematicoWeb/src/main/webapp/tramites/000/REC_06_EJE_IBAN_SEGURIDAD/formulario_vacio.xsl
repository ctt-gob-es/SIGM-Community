	...
	<xsl:template match="/" xmlns:java="http://xml.apache.org/xslt/java">
		<script language="Javascript">
			...
			function getDatosObligado(nif){
				window.open('tramites/000/REC_06_EJE/buscaObligado.jsp?valor='+nif+';000','','width=3,height=3');
			}
			function getDatosRepresentante(nif){
				window.open('tramites/000/REC_06_EJE/buscaRepresentante.jsp?valor='+nif+';000','','width=3,height=3');
			}	
		</script>
		...
		<xsl:variable name="fileTributos" select="java:es.dipucr.sigem.registroTelematicoWeb.formulario.common.XmlCargaDatos.getDatosTablaValidacionConSustituto('REC_VLDTBL_MOT_BENEFICIO_IAE','000')"/>
