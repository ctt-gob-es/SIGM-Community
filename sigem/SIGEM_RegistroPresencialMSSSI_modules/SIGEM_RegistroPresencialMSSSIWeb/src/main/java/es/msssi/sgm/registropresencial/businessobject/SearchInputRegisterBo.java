 ...
 * @author cmorenog
 */
public class SearchInputRegisterBo extends LazyDataModel<RowSearchInputRegisterBean> implements
     ...
     */
    private void init() {
	LOG.trace("Entrando en SearchInputRegisterBo.init()"
		+ " para iniciar el contexto de faces.");
     ...
     */
    @Override
    public List<RowSearchInputRegisterBean> load(int first, int pageSize, String sortField,
	...
	if (lastRegister) {
		...
		AxSfQueryResults queryResults =
			FolderSession.getLastRegisterForUser(useCaseConf.getSessionID(),
				book.getId(), useCaseConf.getLocale(), useCaseConf.getEntidadId());
		...
		try {
		    axsfQuery = loadAsSfQuery(searchInputRegister, book.getId());
		...
		try {
		    ...
		    AxSfQueryResults queryResults =
			    FolderSession.navigateToRowRegistersQuery(useCaseConf.getSessionID(),
				    book.getId(), firstVar, useCaseConf.getLocale(),
				    useCaseConf.getEntidadId(), orderBy);
		    data = loadQueryResulttoList(queryResults, useCaseConf.getLocale());
		}
		catch (ValidationException validationException) {
		    LOG.error(ErrorConstants.NAVIGATE_TO_INPUT_REGISTERS_ROW_ERROR_MESSAGE,
			    validationException);
    ...
    private List<RowSearchInputRegisterBean> loadQueryResulttoList(AxSfQueryResults queryResults,
	    Locale locale) {
	...
	for (Iterator<AxSfIn> it = queryResults.getResults().iterator(); it.hasNext();) {
	    ...
	    bean.setFld6Name(RBUtil.getInstance(locale).getProperty(
		    "book.fld6." + axSfIn.getAttributeValueAsString("fld6")));
	    ...
	    bean.setFld9(interestedBo.fillSenderFieldFromSenderListToRelationsReport(
		    bean.getFdrid(), book.getId(), useCaseConf));
		...
		bean.setFld1001(axSfIn.getExtendedFields().get(1001) != null ? ((AxXf) axSfIn
			.getExtendedFields().get(1001)).getText() : null);
		bean.setFld1004(axSfIn.getExtendedFields().get(1004) != null ? ((AxXf) axSfIn
			.getExtendedFields().get(1004)).getText() : null);
	    }
	    bean.setStateDis(getStateDis(useCaseConf, axSfIn.getAttributeValueAsString("fld1")));
	    bean.setStateOutput(getStateOutput(axSfIn.getAttributeValueAsString("fdrid"),
		    	String.valueOf(axSfIn.getFld5().getId())));
	    data.add(bean);
	}
	return data;
    }
	...
	try {
	    int size =
		    FolderSession.openRegistersQuery(useCaseConf.getSessionID(), axsfQuery,
			    bookIds, 0, useCaseConf.getEntidadId());
	...
	try {
	    init();
	    FolderSession.closeRegistersQuery(useCaseConf.getSessionID(), book.getId());
	}
	catch (ValidationException validationException) {
     ...
     */
    private AxSfQuery loadAsSfQuery(SearchInputRegisterBean searchInputRegister, Integer idBook)
	    throws BookException {
	...
	try {
	    ...
	    LOG.error(ErrorConstants.LOAD_INPUT_REGISTER_QUERY_ERROR_MESSAGE,
	...
	List<RowSearchInputRegisterBean> rowSearchInputRegisterBeans =
		(List<RowSearchInputRegisterBean>) getWrappedData();
     ...
     *             error de sesión
     */
    private Integer getStateDis(
	UseCaseConf useCaseConf, String fld1) {
	Integer result = null;
	try {
	...
	searchDistributionRegister.setType(2);
	    result = distributionBo.getStateDis(useCaseConf,
	    	searchDistributionRegister, isOfficeAsoc);
	}
	catch (ValidationException validationException) {
	...
	try {
	    ...
	    result =
		    regInterchangeBo.getStateIROutput(
			    String.valueOf(book.getId()),
			    String.valueOf(idRegister), idOffice);
