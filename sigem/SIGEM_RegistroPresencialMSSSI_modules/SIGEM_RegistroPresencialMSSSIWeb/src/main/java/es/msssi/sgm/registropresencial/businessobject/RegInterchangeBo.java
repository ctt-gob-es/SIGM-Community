...
import es.msssi.sgm.registropresencial.utils.SicresXMLManagerImpl;
...
public class RegInterchangeBo implements IGenericBo {
     ...
     *
     * @param useCaseConf Usuario que realiza la petición.
     * @param bookId      Id del libro.
     * @param folderId    Id de registro.
     * @return fldid Id de registro guardado o actualizado.
     * @throws RPGenericException           si se ha producido un error genérico en el proceso.
     * @throws RPRegistralExchangeException si se ha producido algún error al recuperar el destinatario
     *                                      por defecto.
     */
     ...
     *
     * @param destinationRegInterchange Datos sobre le destino y el registro a enviar.
     * @param usuario                   Nombre completo del usuario que realiza el envío.
     * @param usuarioContacto           Nombre completo del usuario que realiza el envío.
     * @param idOficina                 Id de oficina.
     * @return idInterchange Id del intercambio registral.
     * @throws RPRegistralExchangeException si se ha producido algún error al realizar el envío.
     */
        ...
        try {
            ...
            if (documentSaved != null) {
                ...
                if (libro.getIdocarchhdr().getType() == 1) {
                    ...
                    outputRegisterBo.updateOnlyFolder(useCaseConf, libro.getIdocarchhdr().getId(),
     ...
     * @param unidadDestino
     */
    private void addSicresXMLInputRegister(String idRegistro, ScrRegstate libro, String idOficina,
                                           String usuario, UseCaseConf useCaseConf, String fld1, UnidadTramitacionIntercambioRegistralVO unidadDestino) {
        InputRegisterBo inputRegisterBo = null;
        InputRegisterBean inputRegisterBean;
        SicresXMLManagerImpl sicresXMLManagerImpl = new SicresXMLManagerImpl();
        List<StreamedContent> filesToUpload = new ArrayList<StreamedContent>();
        String xml = null;
        StreamedContent file;
        RegisterDocumentsBo registerDocumentsBo = null;
        try {
            ...
            LOG.info("Cargando datos del registro " + idRegistro);
            inputRegisterBean =
                    inputRegisterBo.loadInputRegisterBean(useCaseConf, libro,
                            Integer.valueOf(idRegistro));
            xml = sicresXMLManagerImpl.createXMLFicheroIntercambio(inputRegisterBean, true, unidadDestino, usuario, useCaseConf, libro);
            SignResponse signResponse = ReportsSignature.signXMLSicres(xml.getBytes());
            file = new DefaultStreamedContent(new ByteArrayInputStream(signResponse.getSignData()),
                    "application/xml", "sicres-firmado.xsig");
            filesToUpload.add(file);
            ...
            LOG.error("Error al generar el xml: ", exception);
            Utils.redirectToErrorPage(null, null, exception);
        }
    }
     ...
     * @param unidadDestino
     */
    private void addSicresXMLOutputRegister(String idRegistro, ScrRegstate libro, String idOficina,
                                            String usuario, UseCaseConf useCaseConf, String fld1, UnidadTramitacionIntercambioRegistralVO unidadDestino) {
        OutputRegisterBo outputRegisterBo = null;
        ...
        try {
            ...
            outputRegisterBean =
                    outputRegisterBo.loadOutputRegisterBean(useCaseConf, libro,
                            Integer.valueOf(idRegistro));
            xml = sicresXMLManagerImpl.createXMLFicheroIntercambio(outputRegisterBean, true, unidadDestino, usuario, useCaseConf, libro);
            SignResponse signResponse = ReportsSignature.signXMLSicres(xml.getBytes());
            file = new DefaultStreamedContent(new ByteArrayInputStream(signResponse.getSignData()),
                    "application/xml", "sicres-firmado.xsig");
            filesToUpload.add(file);
     ...
     * @return Listado de objetos.
     * @throws RPRegistralExchangeException Error en la operación.
     */
    ...
    getOutboxIR(Integer estado, Integer idOficina, Integer idLibro)
        ...
        try {
        	...
        	// Si se busca por enviados por error y en el outBoxList el registro está repetido varias veces,
            // nos quedamos el estado del último intercambio
        	if (estado != null && new Integer(3).equals(estado)) 
            {
            	
            	outBoxList =
                        intercambioManager.getBandejaSalidaIntercambioRegistralEnviadosConError(idOficina,
                                idLibro);
            	outBoxList = getOutBoxStateLast(outBoxList, estado);
            	
            	List<BandejaSalidaItemVO> resBandejaSalidaItemVO = new ArrayList<BandejaSalidaItemVO>(); 
            	
            	for(BandejaSalidaItemVO bandejaSalidaItemVO: outBoxList) {
            	
            		try {
            			List<IntercambioRegistralSalidaVO> lista =
                                intercambioManager.getHistorialIntercambioRegistralSalidaResumen(String.valueOf(idLibro), 
                                		String.valueOf(bandejaSalidaItemVO.getIdRegistro()),
                                		String.valueOf(idOficina));
                        if (lista != null && lista.size() > 0) 
                        {
                        	for(IntercambioRegistralSalidaVO intercambioRegistralSalidaVO: lista) 
                        	{
                        		if(intercambioRegistralSalidaVO.getEstadosIntercambioRegistralSalida().get(0).getIdEstado() == estado) {
                        			resBandejaSalidaItemVO.add(bandejaSalidaItemVO);
                        		}
                        	}
                        }
            	...
            	outBoxList = getOutBoxStateLast(resBandejaSalidaItemVO, estado);
            }
        	else
        	{
     ...
     *
     * @param selectedRegisters lista de registros.
     * @param book              libro.
     * @param user              usuario.
     * @param idOficina         id de la oficina.
     * @param codOficina        codigo oficina.
     ...
     * @param tipoRechazo       tipo del rechazo.
     * @param user              usuario.
     * @param idOficina         id de la oficina.
     * @param codOficina        codigo oficina.
     * @return lista lista de registros que han fallado.
     ...
     * @param unidadTramitacionDestino unidad a la que va dirigido.
     * @param user                     usuario.
     * @param idOficina                id de la oficina.
     * @param codOficina               codigo oficina.
     * @return lista lista de registros que han fallado.
     ...
     *
     * @param idIntercambio id del asiento.
    ...
    isIntercambioRegistralNotEdit(String idLibro, String idRegistro, String idOficina)
        ...
        try {
            ...
            for (IntercambioRegistralSalidaVO ir : lista) {
                if (ir.getIdEstado() == 1 || ir.getIdEstado() == 2 || ir.getIdEstado() == 5 || ir.getIdEstado() == 4
     ...
     *
     * @param idOficina              id de la oficina
     * @param numeroRegistroOriginal numero de registro original
        ...
        try {
            ...
            criterioEstado =
                    new CriterioVO(CriterioEnum.ASIENTO_ESTADO, OperadorCriterioEnum.IN,
                            new Integer[]{EstadoAsientoRegistralEnum.RECIBIDO.getValue(),EstadoAsientoRegistralEnum.VALIDADO.getValue()});
    ...
    getStateIROutput(String idLibro, String idRegistro, String idOficina)
        ...
        try {
            ...
            if (lista != null && lista.size() > 0) {
            	for(IntercambioRegistralSalidaVO intercambioRegistralSalidaVO: lista) {
            		return intercambioRegistralSalidaVO.getEstadosIntercambioRegistralSalida().get(0).getIdEstado();
            	}
            }
    ...
    protected List<BandejaSalidaItemVO> getOutBoxStateLast(List<BandejaSalidaItemVO> outBoxList, int estado)
    {
		List<String> registros = new ArrayList<String>();
    	for(BandejaSalidaItemVO bandejaSalidaItemVO: outBoxList) {
    		registros.add(bandejaSalidaItemVO.getNumeroRegistro());
    	}
    	
    	HashSet<String> hashSet = new HashSet<String>();
    	hashSet.addAll(registros);
		registros.clear();
		registros.addAll(hashSet);
        
		boolean delete;
		for(String registro: registros) {
			delete = false;
			for(int i = 0; i < outBoxList.size(); i++) {
				BandejaSalidaItemVO bandejaSalidaItemVO = (BandejaSalidaItemVO)outBoxList.get(i);
	    		if(bandejaSalidaItemVO.getNumeroRegistro().equalsIgnoreCase(registro)) {
	    			if(delete) {
	    				outBoxList.remove(i);
	    				i = 1;
	    			}
		...
		List<BandejaSalidaItemVO> newOutBoxList = new ArrayList<BandejaSalidaItemVO>();
		EstadoIntercambioRegistralSalidaEnumVO estadoIR = null;
		for(BandejaSalidaItemVO bandejaSalidaItemVO: outBoxList) {
			if(bandejaSalidaItemVO.getEstado() != null) { 
				estadoIR = bandejaSalidaItemVO.getEstado();
				if(estadoIR.getValue() == estado)	{
					newOutBoxList.add(bandejaSalidaItemVO);
				}
			}	
		}
