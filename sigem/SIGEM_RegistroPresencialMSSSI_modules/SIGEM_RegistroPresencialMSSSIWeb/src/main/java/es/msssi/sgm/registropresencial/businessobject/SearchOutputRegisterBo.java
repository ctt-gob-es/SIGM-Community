 ...
 * @author cmorenog
 */
public class SearchOutputRegisterBo extends LazyDataModel<RowSearchOutputRegisterBean> implements
     ...
     */
    private void init() {
	LOG.trace("Entrando en SearchOutputRegisterBo.init()"
     ...
     */
    @Override
    public List<RowSearchOutputRegisterBean>
	load(
	...
	if (lastRegister) {
		...
		AxSfQueryResults queryResults =
		    FolderSession.getLastRegisterForUser(
			useCaseConf.getSessionID(), book.getId(), useCaseConf.getLocale(),
			useCaseConf.getEntidadId());
		this.setRowCount(queryResults.getPageSize());
		data = loadQueryResulttoList(
		...
		try {
		    ...
		    LOG.error(
			ErrorConstants.LOAD_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, bookException);
		}
		...
		try {
		    ...
		    AxSfQueryResults queryResults =
			FolderSession.navigateToRowRegistersQuery(
			    useCaseConf.getSessionID(), book.getId(), firstVar,
		    ...
		    LOG.error(
			ErrorConstants.NAVIGATE_TO_OUTPUT_REGISTERS_ROW_ERROR_MESSAGE,
			validationException);
	...
	for (Iterator<AxSfOut> it = queryResults.getResults().iterator(); it.hasNext();) {
		...
		bean.setFld1004(axSfOut.getExtendedFields().get(1004) != null ? ((AxXf) axSfOut
			.getExtendedFields().get(1004)).getText() : null);
	    }
	    bean.setStateDis(getStateDis(useCaseConf, axSfOut.getAttributeValueAsString("fld1")));
	    bean.setStateOutput(getStateOutput(axSfOut.getAttributeValueAsString("fdrid"),
		    	String.valueOf(axSfOut.getFld5().getId())));
	...
	try {
	    ...
	    LOG.error(
		ErrorConstants.OPEN_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, bookException);
	...
	try {
	    ...
	    LOG.error(
		ErrorConstants.OPEN_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, validationException);
	    Utils.redirectToErrorPage(
	    ...
	    LOG.error(
		ErrorConstants.OPEN_OUTPUT_REGISTER_QUERY_ERROR_MESSAGE, sessionException);
	    Utils.redirectToErrorPage(
    ...
    @SuppressWarnings("unchecked")
    @Override
    public RowSearchOutputRegisterBean getRowData(
	String rowKey) {
	LOG.trace("Entrando en SearchOutputRegisterBo.getRowData()");
	RowSearchOutputRegisterBean result = null;
	List<RowSearchOutputRegisterBean> rowSearchOutputRegisterBeans =
	    (List<RowSearchOutputRegisterBean>) getWrappedData();
	...
	while (iterator.hasNext() &&
	    ...
	    if (rowSearchOutputRegisterBean.getFdrid().equals(
		new Integer(
		    rowKey))) {
     ...
     *            valor del campo a guardar.
     */
    public void setLastRegister(
	boolean lastRegister) {
