...
public class ReportsOutputRegisterAction extends GenericActions {
	...
	if (facesContext.getViewRoot().getViewMap().get(
	    "searchOutputRegisterAction") != null) {
	    selectedResults =
		((SearchOutputRegisterFormAction) facesContext.getViewRoot().getViewMap().get(
		    "searchOutputRegisterAction")).getSelectedResults();
	    if (selectedResults != null &&
		selectedResults.length > 0) {
		buildCertificateReport();
	    }
	    else {
		ValidationBo.showDialog("Informe",
		    new FacesMessage(
	...
	try {
	    ...
	    if (facesContext.getViewRoot().getViewMap().get(
		"outputRegisterAction") != null) {
		outputRegisterBean =
		    ((OutputRegisterAction) facesContext.getViewRoot().getViewMap().get(
			"outputRegisterAction")).getOutputRegisterBean();
		...
		if (facesContext.getViewRoot().getViewMap().get(
		    "searchOutputRegisterAction") != null) {
		    selectedResults =
			((SearchOutputRegisterFormAction) facesContext.getViewRoot().getViewMap()
			    .get(
				"searchOutputRegisterAction")).getSelectedResults();
		    if (selectedResults != null &&
			selectedResults.length > 0) {
		...
		if (ultimoAcuse == null ){
        		...
        		if (facesContext.getViewRoot().getViewMap().get(
        		    ...
        		    ((OutputRegisterAction) facesContext.getViewRoot().getViewMap().get(
                				"outputRegisterAction")).setOutputRegisterBean(outputRegisterBean);
        		    ((OutputRegisterAction) facesContext.getViewRoot().getViewMap().get(
       				"outputRegisterAction")).getRegisterAttachedDocuments();
            		   ((OutputRegisterAction) facesContext.getViewRoot().getViewMap().get(
       				"outputRegisterAction")).getListUpdates();   
		  ...
		  //si existe el acuse se recupera
		    ...
		    page.setLoc("pdf");
		    page.setName("AcuseReciboRegistroSalida"+fld1+".pdf");
	...
	try {
		...
		bookId, searchOutputRegister);
	 /*   registersList =
		(List<RowSearchOutputRegisterBean>) reportsBo
