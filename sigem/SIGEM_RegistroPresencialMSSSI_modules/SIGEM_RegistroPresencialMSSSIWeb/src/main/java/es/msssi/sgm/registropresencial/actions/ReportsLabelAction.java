...
private byte[] fileByte = null;
	...
	try {
	    ...
	    if (book.getIdocarchhdr().getType() == 1){
        		...
        		inputRegisterBean =
        			((InputRegisterAction) facesContext.getViewRoot().getViewMap()
        				.get("inputRegisterAction")).getInputRegisterBean();
        		...
        		fileByte = reportsBo.buildLabelJasperReport(reportResults, book,
        			String.valueOf(inputRegisterBean.getFdrid()), useCaseConf, inputRegisterBean.getFld1004());
		...
		if (facesContext.getViewRoot().getViewMap().get("outputRegisterAction") != null) {
        		    outputRegisterBean =
            			((OutputRegisterAction) facesContext.getViewRoot().getViewMap()
            				.get("outputRegisterAction")).getOutputRegisterBean();
            		...
            		reportResults = buildLabel(outputRegisterBean);
                	    ...
                	    fileByte = reportsBo.buildLabelJasperReport(reportResults, book,
                    			String.valueOf(outputRegisterBean.getFdrid()), useCaseConf, outputRegisterBean.getFld1004());
