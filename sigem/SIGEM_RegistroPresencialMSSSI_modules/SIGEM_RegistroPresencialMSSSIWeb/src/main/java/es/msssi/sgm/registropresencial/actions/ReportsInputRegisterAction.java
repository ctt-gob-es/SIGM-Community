...
public class ReportsInputRegisterAction extends GenericActions implements Serializable {
	...
	if (facesContext.getViewRoot().getViewMap().get(
	    "searchInputRegisterAction") != null) {
	    selectedResults =
		((SearchInputRegisterFormAction) facesContext.getViewRoot().getViewMap().get(
		    "searchInputRegisterAction")).getSelectedResults();
		...
		FacesMessage message =
		    new FacesMessage(
			FacesMessage.SEVERITY_WARN, "Informe",
	...
	try {
	    ...
	    if (facesContext.getViewRoot().getViewMap().get(
		"inputRegisterAction") != null) {
		inputRegisterBean =
		    ((InputRegisterAction) facesContext.getViewRoot().getViewMap().get(
			"inputRegisterAction")).getInputRegisterBean();
		LOG.info("Llega solo un Id.: " +
		...
		if (facesContext.getViewRoot().getViewMap().get(
		    "searchInputRegisterAction") != null) {
		    selectedResults =
			((SearchInputRegisterFormAction) facesContext.getViewRoot().getViewMap()
			    .get(
				"searchInputRegisterAction")).getSelectedResults();
		    if (selectedResults != null &&
			selectedResults.length > 0) {
		...
		if (ultimoAcuse == null ){
        		reportResults = reportDAO.getRegisterCertReports(
        		    fdridList, bookId, true);
        		file = reportsBo.buildJasperReport(
        			(ScrRegstate) facesContext.getExternalContext().getSessionMap().get(
        				    KeysRP.J_BOOK), reportResults, null);
        		if (facesContext.getViewRoot().getViewMap().get(
			"inputRegisterAction")!= null){
        		    inputRegisterBean.setFld1004(reportsBo.getAcuseJson());
        		   ((InputRegisterAction) facesContext.getViewRoot().getViewMap().get(
        				"inputRegisterAction")).setInputRegisterBean(inputRegisterBean);
        		   ((InputRegisterAction) facesContext.getViewRoot().getViewMap().get(
   				"inputRegisterAction")).getRegisterAttachedDocuments();
        		   ((InputRegisterAction) facesContext.getViewRoot().getViewMap().get(
   				"inputRegisterAction")).getListUpdates();
        		}
        		if (facesContext.getExternalContext().getSessionMap().get(
        			"reportsLabelAction")!= null){
		  ...
		  //si existe el acuse se recupera
		    ...
		    page.setLoc("pdf");
		    page.setName("AcuseReciboRegistroEntrada"+fld1+".pdf");
	...
	try {
	    ...
	    registersList = reportsBo.getRegistersForRelationReports(
		bookId, searchInputRegister);
		...
		if (sizeRegister >= KeysRP.NUM_REGISTERINFORM){
		    FacesContext.getCurrentInstance().addMessage(
			    ...
			    new FacesMessage(
				FacesMessage.SEVERITY_WARN, "Generación de informe ",
				"El resultado del informe son "+ KeysRP.NUM_REGISTERINFORM +" o mas registros," +
				" debe reducir la búsqueda seleccionando mas criterios de búsqueda"));
		}  else {
        
        		file = reportsBo.buildJasperReport(
        			(ScrRegstate) facesContext.getExternalContext().getSessionMap().get(
        				    KeysRP.J_BOOK), null, registersList);
        		FacesContext.getCurrentInstance().addMessage(
        		    null,
        		    new FacesMessage(
        			FacesMessage.SEVERITY_WARN, "Generación de informe ",
        			"Se ha generado el informe correctamente."));
     ...
     */
    private Integer getAndValidateBook()
	throws SessionException {
	Integer idBook = null;
	...
	RowSearchInputRegisterBean[] selectedResults) {
	...
	if (FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(
		...
		(ScrOrg) FacesContext.getCurrentInstance().getExternalContext().getSessionMap()
		    .get(
			"UNITSDIALOG");
	    getSearchInputRegister().setFld7Value(
		unidad);
	}
    }
	...
	if (FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get(
		...
		(ScrOrg) FacesContext.getCurrentInstance().getExternalContext().getSessionMap()
		    .get(
			"UNITSDIALOG");
	    getSearchInputRegister().setFld8Value(
		unidad);
	}
    }
