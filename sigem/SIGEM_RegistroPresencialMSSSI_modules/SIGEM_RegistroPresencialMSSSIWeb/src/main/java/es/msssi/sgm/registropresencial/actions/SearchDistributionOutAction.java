 ...
 * @author cmorenog
 * */
public class SearchDistributionOutAction extends GenericActions implements Serializable {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger(SearchDistributionOutAction.class.getName());
    ...
    /** Flag que establece un registro distribuido con estado Rechazado como PROCESADO **/
    private Integer PROCESAR_REGISTRO_DISTRIBUIDO_RECHAZADO = 1;
    
    /** Flag que establece un registro distribuido con estado Rechazado como NO PROCESADO **/
    private Integer NO_PROCESAR_REGISTRO_DISTRIBUIDO_RECHAZADO = 0;
     ...
     */
    public SearchDistributionOutAction() throws SessionException {
    ...
    /**
     * Método que marca como Procesadas las distribuciones que han sido rechazadas
     */
    public void pasarProcesados() 
    {
    	init();
    	...
    	try
    	{
    	    ...
    	    // Se debe seleccionar al menos una distribución del worklist
    	    errors = validateCount();
    		if(errors) 
    			return;
    		
    		// Sólo se permiten No procesar las distribuciones rechazadas
    		errors = estadosPermitidos(ISDistribution.STATE_RECHAZADO); 
    		if(errors) 
    			return;
    		
    		
    	    distributionBo.processReject(useCaseConf, selectedResults, PROCESAR_REGISTRO_DISTRIBUIDO_RECHAZADO);
    		...
    		ErrorConstants.REJECT_DISTRIBUTION_ERROR_MESSAGE, e);
    	    FacesMessage message = new FacesMessage(
    		FacesMessage.SEVERITY_INFO, "Procesar Distribución Rechazada", e.getMessage());
    	    RequestContext.getCurrentInstance().showMessageInDialog(
    		message);
    	}
    	catch (RPGenericException e) {
    ...
    /**
     * Métod que marca como no procesadas las distribuciones rechazadas 
     * que han sido seleccionadas del worklist
     */
    public void borrarProcesados() 
    {
    	init();
    	...
    	try
    	{
    		...
    		distributionBo.processReject(useCaseConf, selectedResults, 
    				NO_PROCESAR_REGISTRO_DISTRIBUIDO_RECHAZADO);
    		...
    		ErrorConstants.REJECT_DISTRIBUTION_ERROR_MESSAGE, e);
    	    FacesMessage message = new FacesMessage(
    		FacesMessage.SEVERITY_INFO, "No Procesar Distribución Rechazada", e.getMessage());
    	    RequestContext.getCurrentInstance().showMessageInDialog(
    		message);
    	}
    	catch (RPGenericException e) {
    ...
    /**
     * Método que comprueba que se haya seleccionado al menos una distribución del worklist
     * @return boolean - True (Se produce un error de validación) / False (Todo correcto) 
     */
    ...
    /**
     * Método que comprueba que distribuciones son pemitidas
     * @return boolean - True (Se produce un error de validación) / False (Todo correcto)
     */
    protected boolean estadosPermitidos(Integer state) {
    	
    	boolean estadosPermitidos = true;
    	for (RowSearchDistributionBean row : selectedResults) {
    		if(row.getState() != state) { 
    			estadosPermitidos = false;
    			break;
    		}
    	}
    	...
    	if(!estadosPermitidos) {
	    	ValidationBo.showDialog("", new FacesMessage(FacesMessage.SEVERITY_WARN, "",
	    			"Esta opción sólo está disponible para las Distribuciones rechazadas."));
			return true;
    	}
