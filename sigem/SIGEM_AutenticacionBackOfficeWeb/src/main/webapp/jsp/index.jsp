...
<head>
	...
	<%
		...
		if (idEntidad == null) idEntidad = new String("");
	...
	<script language="Javascript">
		...
		function redirige() {
			...
			<% if (sso) { %>
				...
				if(!errorCargaActive){
					url = '<%=request.getContextPath()%>/obtenerUsuarioSSO.do?<%=ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_ENTIDAD%>=<%=idEntidad%>&<%=ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_APLICACION%>=<%=idAplicacion%>&<%=ConstantesGestionUsuariosBackOffice.PARAMETRO_SSO_USER_DN%>=' + sUserDn;
				}
			<%} else { %>
				url = '<%=request.getContextPath()%>/seleccionEntidad.do?<%=ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_ENTIDAD%>=<%=idEntidad%>&<%=ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_APLICACION%>=<%=idAplicacion%>';
			<% } %>
