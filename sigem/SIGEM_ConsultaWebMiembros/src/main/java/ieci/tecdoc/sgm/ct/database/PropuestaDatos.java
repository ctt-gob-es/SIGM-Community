...
public class PropuestaDatos extends PropuestaImpl implements Serializable{
	...
	public void cargarPropuestas(String entidad, String numexp)throws ConsultaExcepcion {
		  ...
		  logger.error("sQueryPropuestas"+sQueryPropuestas);**/
		  
		  //Para cada propuesta
		  StringBuffer sQueryProp = new StringBuffer("select descripcion, infopag_rde, infopag, ffirma, id " +
		  		"from spac_dt_documentos " +
		  		"where (faprobacion IS not null or extension='pdf' or extension='zip') and numexp=? order by descripcion");
		  logger.error("sQueryProp"+sQueryProp);
		  ...
		  try {
			  ...
			  while (rsetIntegrante.next()){				  
				  ...
				  String numexpPropuestaRela = rsetIntegrante.getString(1);
				  ...
				  logger.warn("sPropuestaUrgencia "+sPropuestaUrgencia);
				  
				  ResultSet rsetPropuestaUrgencia = stmtPropUrgencia.executeQuery(sPropuestaUrgencia.toString());
				  
				  logger.warn("rsetPropuestaUrgencia "+rsetPropuestaUrgencia);
				  if(rsetPropuestaUrgencia.next()){
					  logger.warn("tiene ");
					  propuesta.setExtracto(extracto);
				  }
				  else{
					  logger.warn("no tiene ");
				  ...
				  while (rsetPropuesta.next()) {
					  ...
					  String nombrePropuesta = rsetPropuesta.getString(1).replaceAll("'", "");
					  nombrePropuesta = nombrePropuesta.replaceAll("\"", "");
			  ...
			  throw new ConsultaExcepcion(ConsultaCodigosError.EC_CARGAR_INTERESADO_EXPEDIENTE, e);
