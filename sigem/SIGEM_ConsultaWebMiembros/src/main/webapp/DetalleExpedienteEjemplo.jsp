...
<head>
	...
	function abrirVentana(url){
	   var opciones="left=100,top=100,width=800,height=600,status=no,menubar=no,scrollbars=yes,status=no,resizable=yes";
									...
									<tr>
										...
										<td class="borde">
											...
											<%	}%>
												...
												<p class="detalle">
													<bean:message key="aportar" />
													<%
														String paramsUrlAportacion = "?tramiteId=SUBSANACION" 
																				   + "&SESION_ID=" + session.getAttribute("SESION_ID")
																				   + "&ENTIDAD_ID=" + session.getAttribute("ENTIDAD_ID")
																				   + "&" + Misc.LANG + "=" + lang
																				   + "&" + Misc.COUNTRY + "=" + country;
														
														if (s.getIdDocumento() != null) {
															
															paramsUrlAportacion += "&idDocumento=" + s.getIdDocumento();
														}
													%>
													<a href='<%=request.getScheme() + "://"+request.getServerName()+":"+request.getServerPort()+"/"%><bean:write name="DetalleExpediente" property="URLAportacion" /><%=paramsUrlAportacion%>&numExpediente=<bean:write name="DetalleExpediente" property="numeroExpediente" />&numRegistro=<bean:write name="DetalleExpediente" property="numeroRegistro" />' target="_blank">
														<bean:message key="enlace_registro" />
													</a>
												</p>
											<%
												bSubs = true; 
											...
											<%	}%>
												...
												<p class="detalle">
													<bean:message key="consultar_notificacion" />
													<a href='<bean:write name="DetalleExpediente" property="URLNotificacion" />?deu=<%=s.getDEU()%>' target="_blank">
														<bean:message key="enlace_notificacion" />
													</a>
												</p>
											<%
												bNotifs = true; 
											...
											<%	}%>
												...
												<p class="detalle"><bean:message key="fecha" />: <%=s.getFecha()%></p>
												<p class="detalle">
													<bean:message key="proceder_pago" />
													<a href='<%=request.getScheme() + "://"+request.getServerName()+":"+request.getServerPort()+"/"%><bean:write name="DetalleExpediente" property="URLPago" />?idEntidadEmisora=<%=s.getEntidadEmisoraId()%>&idTasa=<%=s.getAutoliquidacionId()%>&importe=<%=s.getImporte()%>&SESION_ID=<%=session.getAttribute("SESION_ID")%>&ENTIDAD_ID=<%=session.getAttribute("ENTIDAD_ID")%>&<%=Misc.LANG%>=<%=lang%>&<%=Misc.COUNTRY%>=<%=country%>' target="_blank">
														<bean:message key="enlace_pago" />
													</a>
												</p>
												<p class="detalle">
													<bean:message key="aportar_justificante" />
													<%
														String paramsUrlAportacion = "?tramiteId=SUBSANACION" 
																				   + "&SESION_ID=" + session.getAttribute("SESION_ID")
																				   + "&ENTIDAD_ID=" + session.getAttribute("ENTIDAD_ID")
												...
												if (ss != null && ss.count()>0){
													...
													if (guidHito.equals(_guid)){
														...
														<%  if (guidHito.equals(guidHitoAct)){ %> 
															<p class="detalle">
																<bean:message key="aportar" />
																<a href='<%=request.getScheme() + "://"+request.getServerName()+":"+request.getServerPort()+"/"%><bean:write name="DetalleExpediente" property="URLAportacion" />?tramiteId=SUBSANACION&numExpediente=<bean:write name="DetalleExpediente" property="numeroExpediente" />&SESION_ID=<%=session.getAttribute("SESION_ID")%>&ENTIDAD_ID=<%=session.getAttribute("ENTIDAD_ID")%>&numRegistro=<bean:write name="DetalleExpediente" property="numeroRegistro" />&<%=Misc.LANG%>=<%=lang%>&<%=Misc.COUNTRY%>=<%=country%>' target="_blank">
																	<bean:message key="enlace_registro" />
																</a>
															</p>
														<%																
												...
												if (nn != null && nn.count()>0){
													...
													if (guidHito.equals(_guid)){
														...
														<%  if (guidHito.equals(guidHitoAct)){ %> 
															<p class="detalle">
																<bean:message key="consultar_notificacion" />
																<a href='<bean:write name="DetalleExpediente" property="URLNotificacion" />?deu=<%=n.getDEU()%>' target="_blank">
																	<bean:message key="enlace_notificacion" />
																</a>
															</p>
														<%																
												...
												if (pp != null && pp.count()>0){
													...
													if (guidHito.equals(_guid)){
														...
														<%  if (guidHito.equals(guidHitoAct)){ %> 
															<p class="detalle">
																<bean:message key="proceder_pago" />
																<a href='<%=request.getScheme() + "://"+request.getServerName()+":"+request.getServerPort()+"/"%><bean:write name="DetalleExpediente" property="URLPago" />?idEntidadEmisora=<%=p.getEntidadEmisoraId()%>&idTasa=<%=p.getIdentificador()%>&importe=<%=p.getImporte()%>&SESION_ID=<%=session.getAttribute("SESION_ID")%>&ENTIDAD_ID=<%=session.getAttribute("ENTIDAD_ID")%>&<%=Misc.LANG%>=<%=lang%>&<%=Misc.COUNTRY%>=<%=country%>' target="_blank">
																	<bean:message key="enlace_pago" />
																</a>
															</p>
														<%															
