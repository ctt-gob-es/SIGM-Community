...
// Comprueba si el objeto est� oculto
...
function takeElement(action, variable, message, aviso,accept, cancel,messageConfirm) {
	...
	if (data == "") {
		...
		//El usuario debe confirmar la operaci�n
...
function deactivateNeedToConfirm() {
	...
	if (typeof ispac_needToConfirm != 'undefined') {
		// Variable global a la p�gina (sin var) que se utiliza en restoreNeedToConfirm
...
function setAnio(anio){
	...
	var splitParametros = parametros.split('&');

	if(splitParametros[0] == ''){
		url = url + '?anio=' + anio;
	}
	else{
		url = url + '?';

		for (var i = 0; i<splitParametros.length; i++){
			var splitParam = splitParametros[i].split('=');

			url = url + splitParam[0] + '=';

			if(splitParam[0] == 'anio'){
				url = url + anio;
			}
			else{
				url = url + splitParam[1];
			}
			if(i<splitParametros.length-1){
				url = url + '&';
			}
		}
	}
