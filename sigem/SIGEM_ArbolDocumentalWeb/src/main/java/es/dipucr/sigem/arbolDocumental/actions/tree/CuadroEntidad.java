package es.dipucr.sigem.arbolDocumental.actions.tree;
...
public class CuadroEntidad implements TreeModel  {
	...
	PropertyMessageResources p;
	
	public CuadroEntidad (HttpServletRequest request, int entityId, int regId, ISessionAPI sessionApi, String language){
		this.entityId = entityId;
		this.regId = regId;
		this.sessionApi = sessionApi;
		this.language = language;
		this.p = (PropertyMessageResources) request.getAttribute(Globals.MESSAGES_KEY);
	}
	        ...
	        if (modelItem.isEntityProcedimiento()) {
	        	...
	        	if (isShowExtendedInfo()) {
		            ...
		            children.add(createElementoCuadro(
		            		...
		            		p.getMessage( "procedure.tree.inputflows"), 
		            		modelItem, language));
	
		        	// Flujo de salida
		            children.add(createElementoCuadro(
		            		ElementoCuadro.TYPE_STAGE_OUTPUT_FLOWS, -2,
		            		p.getMessage( "procedure.tree.outputflows"), 
		            		modelItem, language));
	        	}
