package es.dipucr.sigem.arbolDocumental.core.web;
...
/**
 ***
 * Servlet para la inicializacion de carga de recursos de skins en las
 * aplicaciones de sigem
 * 
 ...
 * aplicacion ejemplo: /SIGEM_AplicacionWeb por lo que el archivo en el proyecto
 * war deberá ir bajo la ruta
 * 
 * /src/main/resources/SIGEM_AplicacionWeb/skinEntidad_numeroEntidad
 * 
 * La aplicacion resolvera el fichero concatenando
 * subdirectorio/skinEntidad/nombreDeFichero para ello usara la clase
 * <code>SigemConfigFilePathResolver</code>
 * 
 * Si La petición será algo asi:
 * http://localhost:8080/SIGEM_AplicacionWeb/resourceServlet/logos/logo.gif y la
 * entidad en la que estamos logados es la "1" ira a buscar la ruta en:
 ...
 *
 */
public class ResourceDispatcherServletArbolDocumental extends HttpServlet {

	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(ResourceDispatcherServletArbolDocumental.class);
	...
	protected void doWork(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			logger.error(
					"Error cargando recurso a traves de ResourceDispatcherServletArbolDocumental",
					ex);
		}
	}
	...
	protected String getLocation(HttpServletRequest req,
		...
		SigemConfigFilePathResolver pathResolver = SigemConfigFilePathResolver
				.getInstance();
	 ...
	 * /pruebaWar/logoServlet/images/logo.gif" devolveria como filename
	 * "images/logo.gif"
	 * 
	 * @param req
	 * @return
	...
	protected String getFileName(HttpServletRequest req) {
		...
		// "/pruebaWar/logoServlet/images/logo.gif"
	...
	/**
	 * metodo que retorna el identificador de identidad para el usuario
	 * actualmente logado, para ello debe existir en la request o en la session
	 * el parámetro
	 * <code>ConstantesGestionUsuariosBackOffice.PARAMETRO_KEY_SESION_USUARIO</code>
	 * que actualmente tiene el valor "keySesionUsuario"; Previamente comprueba
	 * que haya en sesion un atributo con "idEntidad" o "idEntidadAlmacenada" y
	 * si existe lo usará
	 * 
	 * @param req
	 * @return
	...
	protected String getIdEntidad(HttpServletRequest req) {
		...
		if (StringUtils.isEmpty(result)) {
			...
			try {
				oServicio = LocalizadorServicios
						.getServicioAdministracionSesionesBackOffice();
			} catch (SigemException e) {
				logger.fatal(
						"Imposible cargar LocalizadorServicios.getServicioAdministracionSesionesBackOffice() -",
						e);
			...
			String key = req
					.getParameter(ConstantesGestionUsuariosBackOffice.PARAMETRO_KEY_SESION_USUARIO);
			if (StringUtils.isEmpty(key)) {
				key = (String) (req.getSession()
			...
			if (StringUtils.isEmpty(key)) {
				...
				req.getSession()
						.setAttribute(
								ConstantesGestionUsuariosBackOffice.PARAMETRO_ID_ENTIDAD_ALMACENADA,
								result);
			}
		}
	...
	protected String getIdEntidadFromContext(HttpServletRequest req) {
		...
		String idEntidadArbol = (String) req.getSession().getAttribute(ConstantesSesionUser.ID_ENTIDAD);

		if (!StringUtils.isEmpty(idEntidadArbol)) {
			result = idEntidadArbol;
		}
		else if (!StringUtils.isEmpty(idEntidadAlmacenada)) {
