package es.dipucr.sigem.arbolDocumental.tag;
...
public class ListTreeDocumentosTag extends TagSupport {
	...
	//[eCenpri-Manu Ticket#316] + ALSIGM3 Texto en el árbol de ayuda.
	private String keyInfoMensajeTitulo = "info.mensaje.titulo";
	private String keyInfoMensajeTexto= "info.mensaje.texto";
	
	public ListTreeDocumentosTag() {
		super();
	}
    ...
    //[eCenpri-Manu Ticket#316] + ALSIGM3 Texto en el árbol de ayuda.
    private void printMensaje(HttpServletRequest request) throws IOException {
    	
    	StringBuilder htmlMensaje = new StringBuilder();
    	
    	String texto = getMessage(request, keyInfoMensajeTexto);
    	texto = texto.replaceAll("<br><br>","</p><p class=\"infoTexto\">");
    	
    	// Texto del mensaje
    	htmlMensaje.append("<div class=\"cuadroInfo\">")
    			   .append("<p class=\"infoTitulo\">").append(getMessage(request, keyInfoMensajeTitulo)).append("</p>")
    			   .append("<hr><br>")
    			   .append("<p class=\"infoTexto\">").append(texto).append("</p>")
    			   .append("</div>&nbsp;&nbsp;");
    	
		out.println(htmlMensaje.toString());
    }
 		...
 		if(treeBean.getItem()!=null) {
			...
			if(format.getFieldType().equals("LINK")){
				...
				if (bSelect.booleanValue()) {
					...
					out.println("<a href='javascript:showFrame(\"" + format.getUrlTarget() + "\",\"" +
							format.getUrl() +
							"&" + format.getId() + "=" + treeBean.getItem().get(format.getFieldLink()) +"\", \"\", \"\", \"\", false);'" + 
							" class='" + format.getStyleClass() + "' " +
							">");
				}
 			}
