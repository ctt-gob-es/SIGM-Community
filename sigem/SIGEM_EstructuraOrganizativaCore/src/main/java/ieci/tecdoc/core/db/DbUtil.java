...
*/
   ...
   public static String getNativeDateTimeSyntax(int engine, Date data, boolean onlyDate) throws Exception
	...
	/* 
	 * @SF-SEVILLA  Revision para posgress
	 * 02-may-2006 / antmaria
	 */
           ...
           boolean onlyDate)
   {
        ...
        str = "TO_TIMESTAMP('" + sdf.format(data) + "','" + pattern + "')";
   ...
   /**
    * Concede a la tabla cuyo nombre se pasa por parametro los permisos
    * indicados en la variable perms al usuario que se encuentra definido
    * en el fichero IeciTecDocConfig.xml, entre <![CDATA[<userGranted></userGranted>]]>
    * Debe de existir una conexion abierta.
    * @param perms
    * @param tblName Nombre de la tabla
    * @throws Exception
    */
	public static  void grantPermsOnTable(int perms,String tblName) throws Exception {
		String userGranted = DbConnectionConfig.getUserGranted();
		DbGrantStatement stmt = null;

		stmt = new DbGrantStatement();
		try {
			stmt.create(perms,tblName,userGranted);
			stmt.execute();	
			stmt.release();
		}catch (Exception ex){
			DbGrantStatement.ensureRelease(stmt,ex);
			throw ex;
		}
