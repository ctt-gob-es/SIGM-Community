...
import ieci.tecdoc.sgm.tram.ws.server.Binario;
...
public final class SingleSign {
	...
	byte[] getData() throws IOException {
		...
		if(this.dataSource.startsWith("http"))
		{
			...
			ret = DataDownloader.downloadData(this.dataSource);
			Logger.getLogger("es.gob.afirma").info( //$NON-NLS-1$
				"Descarga desde '" + this.dataSource + "' terminada, descargados " + ret.length + " octetos" 
			);
		}
		else if(this.dataSource.startsWith("webservice")){
			
			String codent,guid,datasource="";
			
			codent = this.getExtraParams().getProperty("codent");
			guid = this.getExtraParams().getProperty("guid");
			datasource = this.getExtraParams().getProperty("datasource");
			Logger.getLogger("es.gob.afirma").info( //$NON-NLS-1$
					"Iniciando descarga de datos desde webservice, entidad: " + codent + ", path: "+guid 
			);
			
			TramitacionWebServiceProxy repositorio = new TramitacionWebServiceProxy();	
			repositorio.setEndpoint(datasource);	
			Binario bin = repositorio.getFicheroTemp(codent, guid);			
			ret = bin.getContenido();
			Logger.getLogger("es.gob.afirma").info( //$NON-NLS-1$
					"Descarga terminada, descargados " + ret.length + " octetos"
			);
