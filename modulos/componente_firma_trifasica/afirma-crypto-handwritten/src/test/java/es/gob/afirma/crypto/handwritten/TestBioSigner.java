...
//		);
		...
		new BioSigner().sign(
			...
			"CABECERA DE FIRMA", // Plantilla HTML //$NON-NLS-1$
			new Rectangle(30, 50, 400, 200),
			true
		);
	}
	...
	public void signatureFinished(final SignatureResult sr) {
		...
		final byte[] image = sr.getSignatureJpegImage();
		try {
			final OutputStream os = new FileOutputStream(File.createTempFile("IMG_", ".jpg")); //$NON-NLS-1$ //$NON-NLS-2$
			os.write(image);
			os.flush();
			os.close();
