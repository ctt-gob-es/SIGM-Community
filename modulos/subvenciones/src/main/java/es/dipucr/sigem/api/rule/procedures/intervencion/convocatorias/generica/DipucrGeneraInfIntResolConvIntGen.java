...
public class DipucrGeneraInfIntResolConvIntGen extends DipucrAutoGeneraDocIniTramiteRule {
    ...
    public void setSsVariables(IClientContext cct, IRuleContext rulectx) {
        ...
        try {
            ...
            IItemCollection expRelacionadosCollection = cct.getAPI().getEntitiesAPI().queryEntities(Constants.TABLASBBDD.SPAC_EXP_RELACIONADOS, "WHERE NUMEXP_PADRE='" + numexp + "'");
            ...
            IItemCollection expedientesCollection = cct.getAPI().getEntitiesAPI().queryEntities(Constants.TABLASBBDD.SPAC_EXPEDIENTES, "WHERE NUMEXP IN (" +query+") AND ESTADOADM='RS' ORDER BY IDENTIDADTITULAR");
               ...
               while (expedientesIterator.hasNext()){
                   ...
                   Iterator<?> resolucionIterator = cct.getAPI().getEntitiesAPI().getEntities("DPCR_SOL_CONV_SUB", expediente.getString("NUMEXP")).iterator();
                   if( resolucionIterator.hasNext()){
                       IItem resolucion = (IItem) resolucionIterator.next();
                    
                    String importeSolString = resolucion.getString("SUBVENCION");
                    if(StringUtils.isNotEmpty(importeSolString)){
                        ...
                        if(importeSolStringSplit.length == 1){
                            importeSolString = importeSolStringSplit[0];
                        }
                        if(importeSolStringSplit.length == 2){
                            importeSolString = importeSolStringSplit[0] + "." + importeSolStringSplit[1];
                        } else if(importeSolStringSplit.length > 2){
                            importeSolString = "";
                        ...
                        importeTotal += Double.parseDouble(importeSolString);
                    } else{                   
                           importeTotal += 0;
                       }
                   } else{                   
        ...
        try{
            if ("%TABLA1%".equals(refTabla)){
                ...
                if (xSearchInterface != null){
                    ...
                    setHeaderCellText(xTable, "B1", "SUBVENCIÓN SOLICITADA");                
                    setHeaderCellText(xTable, "C1", "PROYECTO / ACTIVIDAD");                
                    setHeaderCellText(xTable, "D1", "SUBVENCIÓN CONCEDIDA");                
                ...
                //    NumberFormat df = NumberFormat.getCurrencyInstance();
                       ...
                       while (expedientesIterator.hasNext()){
                        ...
                        String importeSolString = solicitud.getString("PRESUPUESTO");
                        String importeConcString = solicitud.getString("SUBVENCION");                        
                                            
                        setCellText(xTable, "A" + (i+1), ayuntamiento);
                        setCellText(xTable, "B" + (i+1), importeSolString);
                        setCellText(xTable, "C" + (i+1), proyecto);
                        setCellText(xTable, "D" + (i+1), importeConcString);                    
                     }
                }
