...
public class DipucrDatosResolAprobConvDiscapacitadosEELL extends DipucrAutoGeneraDocIniTramiteRule {
    ...
    public void setSsVariables(IClientContext cct, IRuleContext rulectx) {
        try {
            ...
            IItemCollection expedientesCollection = cct.getAPI().getEntitiesAPI().queryEntities(Constants.TABLASBBDD.SPAC_EXPEDIENTES, "WHERE NUMEXP IN (" +query+") AND ESTADOADM='RS' ORDER BY IDENTIDADTITULAR");
               ...
               while (expedientesIterator.hasNext()){
                   ...
                   if( resolucionIterator.hasNext()){
                       ...
                       importeTotal += parseDouble(expediente.getString("NUMEXP"), resolucion.getString("IMPORTE"));
                       importeDiscapacitados += parseDouble(expediente.getString("NUMEXP"), resolucion.getString("IMPORTE2"));
                   } else{
            ...
            cct.setSsVariable("IMPORTE_DISCAPACITADOS", ""    + new DecimalFormat("#,##0.00").format(importeDiscapacitados));
            cct.setSsVariable("IMPORTE_TOTAL", ""    + new DecimalFormat("#,##0.00").format(importeDiscapacitados+importeTotal));
        } catch (ISPACException e) {
    ...
    private double parseDouble(String numexp, String valor){
        ...
        try{
            ...
            LOGGER.error(ConstantesString.LOGGER_ERROR + " al recuperar el importe de la solicitud: " + numexp + ", " + e.getMessage(), e);
            valorDouble = 0;
        }
        return valorDouble;
    }
        ...
        try{
            if ("%TABLA1%".equals(refTabla)){
                ...
                if (xSearchInterface != null) {
                    ...
                    setHeaderCellText(xTable, "B1", "PROYECTO / ACTIVIDAD");                
                    setHeaderCellText(xTable, "C1", "IMPORTE SOLICITUD");
                    setHeaderCellText(xTable, "D1", "SOLICITUD SUBVENCIÓN DISCAPACITADOS");
                    setHeaderCellText(xTable, "E1", "SUBVENCIÓN PREASIGNADA");
                    setHeaderCellText(xTable, "F1", "TOTAL SUBVENCIÓN");
                       ...
                       while (expedientesIterator.hasNext()){
                        ...
                        if(expResolucion.hasNext()){
                            ...
                            importe += parseDouble(expediente.getString("NUMEXP"), resolucion.getString("IMPORTE"));
                            discapacitados += parseDouble(expediente.getString("NUMEXP"), resolucion.getString("IMPORTE2"));
                        } else{
                        ...
                        setCellText(xTable, "B" + (i+1), proyecto);
                        setCellText(xTable, "C" + (i+1), presupuesto);                        
                        setCellText(xTable, "D" + (i+1), "" + new DecimalFormat("#,##0.00").format(discapacitados));
                        setCellText(xTable, "E" + (i+1), "" + new DecimalFormat("#,##0.00").format(importe));
                        setCellText(xTable, "F" + (i+1), "" + new DecimalFormat("#,##0.00").format(discapacitados+importe));
                     }
                }
                ...
                if (xSearchInterface != null){
                    ...
                    setHeaderCellText(xTable, "D1", "MEMORIA DENEGACIÓN");        
