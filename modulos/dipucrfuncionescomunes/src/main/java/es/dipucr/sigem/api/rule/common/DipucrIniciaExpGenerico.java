...
public class DipucrIniciaExpGenerico  implements IRule{
	
	public static final Logger logger = Logger.getLogger(DipucrIniciaExpGenerico.class);
...
//						
						...
						ResultSet expedienteSolicitudCTProcIterator = cnt.executeQuery(sql).getResultSet();
						if(expedienteSolicitudCTProcIterator.next()){
							...
							if(expedienteGenericoIterator.hasNext()){
								...
								Iterator<IItem> respGenericaIterator = respGenericoCollection.iterator();
				...
				if(StringUtils.isNotEmpty(numExpSolicitudEspecifica)){
