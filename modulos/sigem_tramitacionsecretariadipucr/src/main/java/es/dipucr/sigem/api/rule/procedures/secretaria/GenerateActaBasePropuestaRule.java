...
public class GenerateActaBasePropuestaRule implements IRule {

	private OpenOfficeHelper ooHelper = null;
	private static final Logger logger = Logger
			.getLogger(GenerateActaBasePropuestaRule.class);
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {
		try {
			...
			if(!STR_constitutiva.equals("")){
				...
				// Borrador
				List listPropuesta = SecretariaUtil.getPropuestas(rulectx,
						entitiesAPI);
				...
				if (vPropuesta != null && vPropuesta.size()>1) {
					...
					int tamano = listPropuesta.size() + 1;
					...
					if (!STR_urgencias.equals("")) {
						...
						List listUrgencia = SecretariaUtil.getUrgencias(rulectx,
								entitiesAPI);
						...
						DocumentosUtil.generarDocumento(rulectx, STR_urgencias,
								null);
					...
					if (!STR_nombreRuegos.equals("")) {
						...
						if (organo.equals("PLEN")) {
							...
							cct.setSsVariable("ORDENRUEGOS", String.valueOf(tamano + 2));
						}
		
						DocumentosUtil.generarDocumento(rulectx,
								STR_nombreRuegos, null);
						strInfoPag = DocumentosUtil.getInfoPagByDescripcion(rulectx.getNumExp(), rulectx,
								STR_nombreRuegos);
			...
			DocumentosUtil.generaYAnexaDocumento(rulectx, tpdoc, STR_nombreTramite, file, extensionEntidad);
			file.delete();
