...
function errorDocumentos(id, fileName, descriperror) {
...
function mostrarAnexos(id, fileName) {
	...
	$('#uploadedAnexos').append('<li id="' + idDoc + '"><a class="fileName"><span>' + fileName + '</span></a>'+
								'<select type="text" id="signline_type_0" name="signline_type_0" value="" maxlength="15"' +
							   	 'onchange="f(\'docId=' + id + '&type=annex&docType=\' + this.value);">' + 
							   	 	documentTypesHTML +
							   	'</select>'+
...
function cargarTiposDocumento() {
	...
	$.ajax({url: 'redactionInvitation/getDocumentTypesInvReq',
     	dataType: 'json',
     	data: "",
     	async: false,
     	success:
...
function borrarDocumento(idDoc, queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'redactionInvitation/deleteDocumentInvReq',
     	dataType: 'html',
     	data: queryString,
     	async: false,
     	success:
...
function actualizarTipoDocumento(queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'redactionInvitation/updateDocumentTypeInvReq',
     	dataType: 'html',
     	data: queryString,
     	async: false,
     	success:
...
function cargarListaUsuarios(model) {
	...
	if(model!=undefined && model!=null && model.userSelectionList!=null && model.userSelectionList!=undefined){
		$.each(model.userSelectionList, function (i, item) {
			pickListHTML += '<option value="' + item.fullNameWithProvince + '"';
...
function loadUserPickList(queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'redactionInvitation/selectUsersInvReq',
     	dataType: 'json',
     	data: queryString,
     	async: false,
     	success:
...
function prepareUserPickList() {
	...
	loadUserPickList("signers=" + encodeURIComponent(window.btoa(signersVal)) + "&signLinesConfig=" + loadSignLineConfig() +"&signLinesAccion=" + loadSignLineAcciones() +
					 "&userNameFilter=&userTypeFilter=&userSeatFilter="+"&firstInvocation=true");
...
function addSignLine() {
	...
	auxhtml = auxhtml + '<li>' +
			...
			'</select>' +
	   		'<div class="overflowh" style="position: relative;">' +
				'<input id="signer_' + i + '" class="ui-autocomplete-input signer_input" style="width: 60%;" type="text" th:field="*{signer}" value="" name="signer_' + i + '" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">' +
	...
	$('#signer_' + i).autocomplete({
		source: "redactionInvitation/autocompleteSigners",
	...
	for (var i = 0; i < signers.length; i++){
		if (i == 0) {
			...
			auxhtml = auxhtml +'<option selected = "selected">Firma</option>' +
				...
				'</select>' +
				'<div class="overflowh" style="position: relative;">' +
					'<input id="signer_' + i + '" class="ui-autocomplete-input signer_input" style="width: 60%;" type="text" value="' + signers[i] + '" name="signer_' + i + '" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">' +
					'<a href="javascript:void(0);" onclick="deleteSignLine(' + i + ');" style="padding: 5px 0; position: absolute; right: 0px;"><span class="mf-icon mf-icon-delete-16" th:text="#{selectUsers}"></span></a>'; 
...
$(document).ready(function() {
	...
	//Se crea el diálogo de la ventana modal para invitaciones
	$("#invitation_modal").dialog({
	    autoOpen: false,
	    resizable: false,
	    //height:450,
	    width: 500,
                		...
                		auxhtml = auxhtml + '<li>' +
													...
													'</select>' +
											   		'<div>' +
														...
														'</a>' +
														'<input id="signer_0" class="ui-autocomplete-input signer_input" style="width: 60%;" type="text" value="" name="signer_0" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">';
                		auxhtml = auxhtml +             '<select id="actionSigner_' + i + '" name="actionSigner_' + i + '" class="actionSigner" >' ;
											        			$.each(listaAccionesFirmante, function (j, item) {
											        				if (item.id==1){
											        					auxhtml = auxhtml +'<option value="'+item.id+'" selected="selected">'+item.name+'</option>';
                		...
                		users.each(function (i, item) {
                			...
                			if (i == 0) {
	                			...
	                			auxhtml = auxhtml +'<li>' +
															...
															'</select>' +
													   		'<div class="overflowh">' +
																'<input id="signer_' + i + '" class="ui-autocomplete-input signer_input" style="width: 60%;" type="text" value="" name="signer_' + i + '" autocomplete="off" role="textbox" aria-autocomplete="list" aria-haspopup="true">';
                				auxhtml = auxhtml +				'<select id="actionSigner_' + i + '" name="actionSigner_' + i + '" class="actionSigner" >' ;
											                			$.each(listaAccionesFirmante, function (j, item) {
	...
	$('#signer_0').autocomplete({
		source: "redactionInvitation/autocompleteSignersInvReq",
		// Función que se ejecuta al seleccionar un elemento
		select: function( event, ui ) {
			selected = "";
			var signers = $('#signlines').find('.signer_input');
	...
	$('#redactionInvitationForm').bind('submit', function(event) {
		jsAjaxStatus.startAjax();
    });
