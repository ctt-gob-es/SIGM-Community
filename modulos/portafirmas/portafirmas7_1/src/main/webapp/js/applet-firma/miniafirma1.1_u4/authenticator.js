...
function end_authentication(callback_method)
{
	...
	try {
		var dataB64 = clienteFirma.getBase64FromText (text, null);
		signature = clienteFirma.sign(dataB64, algorithm, format, null);

	} catch (e) {
		if (clienteFirma.getErrorType() == 'es.gob.afirma.core.AOCancelledOperationException') {
			error = "operationCancelled";			
		} else if (clienteFirma.getErrorType() == 'es.gob.afirma.keystores.common.AOCertificatesNotFoundException') {
			error = "No se ha encontrado certificado en el almacen";
		} else {
//			alert("Por favor, copie salida de la consola de Java en este punto");
			error = clienteFirma.getErrorMessage();
		}
	}
