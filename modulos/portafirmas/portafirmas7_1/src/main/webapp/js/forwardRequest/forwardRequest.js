...
function errorDocumentos(id, fileName) {
	$('#warning').html('El documento ' + fileName + ' excede el tamaño máximo permitido');
	$('#warning').show();
}
...
function errorAnexos(id, fileName) {
	$('#warning').html('El anexo ' + fileName + ' excede el tamaño máximo permitido');
	$('#warning').show();
}
...
function cargarTiposDocumento() {
	...
	$.ajax({url: 'forwardRequest/getDocumentTypes',
     	dataType: 'json',
     	data: "",
     	async: false,
     	success:
...
function borrarDocumento(idDoc, queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'forwardRequest/deleteDocument',
     	dataType: 'html',
     	data: queryString,
     	async: false,
     	success:
...
function actualizarTipoDocumento(queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'forwardRequest/updateDocumentType',
     	dataType: 'html',
     	data: queryString,
     	async: false,
     	success:
...
function cargarListaUsuarios(model) {
	...
	if(model!=undefined && model!=null && model.userSelectionList!=null && model.userSelectionList!=undefined){
		$("#userPickList").pickList({
	        sourceListLabel:    "Usuarios",
	        targetListLabel:    "Firmantes",
...
function loadUserPickList(queryString) {
	jsAjaxStatus.startAjax();
	$.ajaxSetup({cache: false});
	$.ajax({url: 'forwardRequest/selectUsers',
     	dataType: 'json',
     	data: queryString,
     	async: false,
     	success:
...
function addSignLine() {
	...
	$('#signer_' + i).autocomplete({
		source: "forwardRequest/autocompleteSigners",
...
$(document).ready(function() {
	...
	$('#forwardRequestForm').bind('submit', function(event) {
		jsAjaxStatus.startAjax();
    });
