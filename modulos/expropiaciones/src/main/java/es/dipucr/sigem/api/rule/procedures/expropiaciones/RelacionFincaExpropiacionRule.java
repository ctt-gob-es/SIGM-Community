...
public class RelacionFincaExpropiacionRule implements IRule{
	
	private static final Logger logger = Logger.getLogger(RelacionFincaExpropiacionRule.class);
		...
		try
    	{
			...
			if (it.hasNext()) {
				...
				if(!StringUtils.isEmpty(numexpExp) && numexpExp != null){
					...
					registro.set("RELACION", "Finca/Expropiacion");
					registro.store(cct);
				}
