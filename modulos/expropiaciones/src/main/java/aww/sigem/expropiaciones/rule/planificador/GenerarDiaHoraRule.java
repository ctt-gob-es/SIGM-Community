 ...
 */
public class GenerarDiaHoraRule implements IRule {
	
	/** Logger de la clase. */
	private static final Logger logger = 
		Logger.getLogger(GenerarDiaHoraRule.class);
	...
	public boolean validate(IRuleContext rulectx) throws ISPACRuleException {
		try {
			...
			if (itExp.hasNext()) {
				...
				String fechas = itemExp.getString("ACTAS_PREVIAS_DIAS_SELECCIONAD");
				if (fechas==null){
					rulectx.setInfoMessage("EL campo Días seleccionados Actas Previas de Expediente de Expropiacion Forzosa no debe estar vacío.");
					return false;
				}
			}
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {
		try {
			//logger.warn("Ejecutando regla GenerarDiaHoraRule");
			...
			if(fechas != null ) {
				...
				while(itFincas.hasNext()) {
					...
					//String expFinca = (String) itFincas.next();
	...
	///////////////////////////////////			
				...
				for(int i = 0; i<listaFechas.length; i++) {
					...
					while((minutosHoy < (minutosDia + margenDia)) && itListaOrden.hasNext()) {
						...
						// Se elimina el municipio de la lista.
						...
						//logger.warn("minutosHoy " + minutosHoy);
						// Calculo la cita
				...
				while(itCitas.hasNext()) {
					...
					if (itFinca.hasNext()) {
						...
						itemFinca.set("LEVANTAMIENTO_FECHA", cita[1]);
						itemFinca.set("LEVANTAMIENTO_HORA", cita[2]);
			...
			//logger.warn("Fin de ejecución regla GenerarDiaHoraRule");
			return null;
