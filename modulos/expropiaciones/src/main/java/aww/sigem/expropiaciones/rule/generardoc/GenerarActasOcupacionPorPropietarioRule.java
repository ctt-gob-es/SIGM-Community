 ...
 */
public class GenerarActasOcupacionPorPropietarioRule implements IRule {
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {
		try {
			//logger.warn("Ejecutando regla GenerarActasOcupacionPorPropietario");
			...
			if (documentTypeId != 0) {
				...
				IItemCollection tpDocsTemplatesCollection2 = procedureAPI
				.getTpDocsTemplates(documentTypeId);
				
				Iterator itdoc = tpDocsTemplatesCollection2.iterator();
				
				while (itdoc.hasNext()){
					IItem tpDocsTemplate2 = (IItem) itdoc.next();
					
					int templateId2 = tpDocsTemplate2.getInt("ID");
					//logger.warn("Obteniendo todos los templateId:" + templateId2);
				}
			...
			try {
				...
				while (itExpropiados.hasNext()) {			 
					...
					expropiado+=item.getString("NOMBRE");
					
					expropiado+=" ";
					expropiado+=item.getString("DIRNOT");
					expropiado+=" ";
					expropiado+=item.getString("LOCALIDAD");
					expropiado+=" (";
					expropiado+=item.getString("C_POSTAL");
					expropiado+=") ";
					expropiado+=item.getString("CAUT");
					...
					String strQueryExpropiado = "WHERE NUMEXP_EXPROPIADO = '" + numExpTit + "'";
					IItemCollection collectionPago = entitiesAPI.queryEntities("EXPR_FINCA_EXPROPIADO_PAGO", strQueryExpropiado);
					Iterator fincasExp = collectionPago.iterator();
					while (fincasExp.hasNext()){
						IItem itemPago = (IItem)fincasExp.next();
						//logger.warn("fincas propiedad: " + itemPago.getString("NUMEXP"));	
						//logger.warn("finca actual: " + numExp);
						if (itemPago.getString("NUMEXP").equals(numExp)){
							cantidadPago = itemPago.getString("CANTIDADPAGO"); 
							cct.setSsVariable("CANTIDAD", cantidadPago);
							cantidad = true;
						}
						else{
							itemPago = (IItem)fincasExp.next();
						}
					}
			...
			//logger.warn("Fin de ejecución regla GenerarActasOcupacionPorPropietario");
		} catch (Exception e) {
