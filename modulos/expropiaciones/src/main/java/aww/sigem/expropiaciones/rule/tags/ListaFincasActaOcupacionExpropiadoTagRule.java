 ...
 */
public class ListaFincasActaOcupacionExpropiadoTagRule implements IRule {
	
	/** Logger de la clase. */
	private static final Logger logger = 
		Logger.getLogger(ListaFincasActaOcupacionExpropiadoTagRule.class);
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {
		try {
			////logger.warn("Ejecutando regla ListaFincasActaOcupacionExpropiadoTagRule");
			...
			//String clausulas =  "ORDER BY MUNICIPIO, NUM_POLIGONO ASC, NUM_PARCELA ASC";
			String clausulas = "ORDER BY LEV_OCUPACION_FECHA, LEV_OCUPACION_HORA ASC";
			...
			while (itFincas.hasNext()) {
				...
				fila.add(item.getString("LEV_OCUPACION_FECHA") + " # " + item.getString("LEV_OCUPACION_HORA"));
				//Código que extrae una lista de propietarios
				fila.add(propietariosFinca(entitiesAPI,item.getString("NUMEXP"),titulos));				
			...
			String textoTabla = TablaUtil.formateaTabla(titulos, contenido, 60, true, 2, true);
			
			//logger.warn("Tabla dibujada:\n" + textoTabla);
			//logger.warn("Fin regla ListaFincasActaOcupacionExpropiadoTagRule");
			return textoTabla;
		} catch (Exception e) {
...
private String propietariosFinca(IEntitiesAPI entitiesAPI, String numExpFinca, List titulos) throws Exception{
		...
		while (itExpropiados.hasNext()) {			 
			...
			if(itExpropiados.hasNext()) {
				expropiados+=" ";	
				int [] anchos = TablaUtil.calcularAnchos(titulos, 60, true, 2, true);				
