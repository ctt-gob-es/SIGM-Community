...
public class AsignarDiaHoraOcupacionPorMunicipioRule implements IRule {
	/** Logger de la clase. */
	private static final Logger logger = Logger.getLogger(AsignarDiaHoraOcupacionPorMunicipioRule.class);
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {
		try {
			//logger.warn("Ejecutando regla AsignarDiaHoraOcupacionPorMunicipioRule");
			
			/*
			 *  Hora inicio primer municipio: 9:30
			 *  Intervalo entre municipios (normalmente serán 60 min).
			 */
			...
			if(fechas != null ) {
				...
				for(int i = 0; i<listaFechas.length; i++) {
					...
					while((minutosHoy < (minutosDia + margenDia)) && itListaOrden.hasNext()) {
						...
						String municipio = FuncionesUtil.eliminarEspacios(filaLista.substring(0,filaLista.indexOf(",")).trim());
