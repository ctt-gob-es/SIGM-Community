 ...
 */
public class ListaFincasDecretoConsignaTagRule implements IRule {
	
	/** Logger de la clase. */
	private static final Logger logger = 
		Logger.getLogger(ListaFincasDecretoConsignaTagRule.class);
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {
		try {
			//logger.warn("Ejecutando regla ListaFincasDecretoConsignaTagRule");
			...
			while (itFincas.hasNext()) {
				...
				List listaExpedientesPropietarios = ConsignaTransferenciaUtil.listaExpedientesPropietariosConsigna(entitiesAPI,item.getString("NUMEXP"));
				...
				while(itPropietarios.hasNext()){					
					...
					fila.add(PropietariosUtil.datosPropietarioDecreto(entitiesAPI, expPropietario));					
			...
			fila.add("");
			fila.add("");
			TotalDecretoConsignaTagRule totalConsigna = new TotalDecretoConsignaTagRule();
			fila.add(totalConsigna.execute(rulectx));
			contenido.add(fila);
