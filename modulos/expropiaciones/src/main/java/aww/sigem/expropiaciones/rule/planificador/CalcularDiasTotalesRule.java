 ...
 */
public class CalcularDiasTotalesRule implements IRule {
	
	/** Logger de la clase. */
	private static final Logger logger = 
		Logger.getLogger(CalcularDiasTotalesRule.class);
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {		
		try {
			//logger.warn("Ejecutando regla CalcularDiasTotalesRule");
			...
			//logger.warn("intervalo " + intervalo);
			/*
			...
			//logger.warn("Numero de Fincas " +collectionFincas.toList().size());
			while(itFincas.hasNext()) {
				...
				// Aunque la finca no tenga propietarios, se le asignan 5 minutos.
				...
				//logger.warn("minutosTotal: " + minutosTotal);
			}	
			...
			if (itemExp != null) {
				itemExp.set("ACTAS_PREVIAS_DIAS_NECESARIOS", numeroDias);	
				//logger.warn("Numero de dias " + numeroDias);
				itemExp.store(rulectx.getClientContext());
			} else throw new ISPACRuleException("Se ha producido un error. EXPEDIENTE EXPROPIACION FORZOSA No encontrado");

			//logger.warn("Fin de ejecución regla CalcularDiasTotalesRule");
			return null;
