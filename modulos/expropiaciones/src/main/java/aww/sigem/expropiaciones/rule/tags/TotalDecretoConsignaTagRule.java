 ...
 */
public class TotalDecretoConsignaTagRule implements IRule {
	
	/** Logger de la clase. */
	private static final Logger logger = 
		Logger.getLogger(TotalDecretoConsignaTagRule.class);
	...
	public Object execute(IRuleContext 	rulectx) throws ISPACRuleException {
		try {
			//logger.warn("Ejecutando regla TotalDecretoConsignaTagRule");
			...
			while (itFincas.hasNext()) {
				...
				while(itPropietarios.hasNext()){					
					...
					valor = ConsignaTransferenciaUtil.cantidadPagoPropietarioFinca(entitiesAPI, item.getString("NUMEXP"), expPropietario);
					total += valor;						
				}									
			}		
			
			//logger.warn("Total:" + total);			
			//logger.warn("Fin regla TotalDecretoConsignaTagRule");
