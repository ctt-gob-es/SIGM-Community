...
public class DipucrInsertarParticipanteInteresadoAsesoramientoRule implements IRule{
	
	private static final Logger logger = Logger.getLogger(DipucrInsertarParticipanteInteresadoAsesoramientoRule.class);
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {
		...
		try {
			...
			IItemCollection represen = entitiesAPI.queryEntities(Constants.TABLASBBDD.ASES_REPRES, sQuery);
			...
			if(iReresent.hasNext()){
				...
				if(itRepre.getString("JUZGADO") != null) juzgado = itRepre.getString("JUZGADO");
			}
			logger.debug("Ayuntamiento = "+ayuntamiento+" Juzgado = "+juzgado);
			
			String idInteresadoAyuntamiento = accsRegistro.getIdParticipanteJuridico("AYUNTAMIENTO DE "+ayuntamiento.toUpperCase());
			String idInteresadoJuzgado = accsRegistro.getIdParticipanteJuridico(juzgado.toUpperCase());
			...
			IThirdPartyAdapter terceroJuzgado = servicioTerceros.lookupById(idInteresadoJuzgado);
            ...
            try{
            	...
            	if(terceroJuzgado != null && terceroJuzgado.getIdExt()!= null){
            		logger.debug("Juzgado");
            		ParticipantesUtil.insertarParticipanteById(rulectx, rulectx.getNumExp(), idInteresadoJuzgado, ParticipantesUtil._TIPO_INTERESADO, ParticipantesUtil._TIPO_PERSONA_JURIDICA, "");
            	}
            	else{
            		nuevoParticipante = entitiesAPI.createEntity(Constants.TABLASBBDD.SPAC_DT_INTERVINIENTES, rulectx.getNumExp());	        
			        nuevoParticipante.set("ROL", "INT");
			        nuevoParticipante.set("NOMBRE", juzgado.toUpperCase());
			        nuevoParticipante.set("DIRNOT", "Heras del Cerrillo, s/n");		        
	            	nuevoParticipante.store(cct);
            	}
            	cct.endTX(true);
            }
            catch(Exception e){
            	logger.error("Error al guardar el participante con id :"+terceroAyuntamiento.getIdExt()+" o "+terceroJuzgado.getIdExt() + ", en el expediente: " + rulectx.getNumExp() + ". " + e.getMessage(), e);
            	cct.endTX(false);
            }	        
		} catch (ISPACException e) {		
			logger.error("Error al guardar el paticipante en el expediente: " + rulectx.getNumExp() + ". " + e.getMessage(), e);
		}
