...
/**
 * [eCenpri-Felipe #477]
 * Regla que carga los terceros a partir del excel
 * @author Felipe
 * @since 10.11.2011
 */
public class CargarTercerosExcelRule implements IRule 
{
	/** Logger de la clase. */
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {
		...
		try{
			...
			List tercero = null;
			String tipoPersona, cifnif, nombre, ape1, ape2, direccion, cpostal, 
				municipio, provincia, tfnoFijo, tfnoMovil, email, strTipoDoc;
			int tipoDocumento = Integer.MIN_VALUE;
			...
			for(int i = 1; i < listTerceros.size(); i++){
				...
				//Vemos si ya existe
				IThirdPartyAdapter [] terceros = thirdpartyAPI.lookup(cifnif);
				if (terceros.length > 0){
					listErrores.add(crearError(linea, cifnif, "Ya existe el tercero en la BBDD"));
					continue;
				}
				
				//Tipo persona
				tipoPersona = (String) tercero.get(0);
				if (StringUtils.isEmpty(tipoPersona)){
					listErrores.add(crearError(linea, cifnif, "El campo TIPO PERSONA es obligatorio"));
					continue;
				}
				
				if (!tipoPersona.equals("F") && !tipoPersona.equals("J")){
					listErrores.add(crearError(linea, cifnif, "El campo TIPO PERSONA debe ser 'F' o 'J'"));
					continue;
				}
				
				//Tipo documento
				strTipoDoc = (String) tercero.get(1);
				if (StringUtils.isEmpty(strTipoDoc)){
					listErrores.add(crearError(linea, cifnif, "El campo TIPO DOCUMENTO es obligatorio"));
					continue;
				}
				
				try{
					tipoDocumento = Double.valueOf(strTipoDoc).intValue();
				}
				catch (Exception e) {
					listErrores.add(crearError(linea, cifnif, "El campo TIPO DOCUMENTO debe ser un entero comprendido entre 1 y 5"));
					continue;
				}
				
				if (tipoDocumento < 1 || tipoDocumento > 5){
					listErrores.add(crearError(linea, cifnif, "El campo TIPO DOCUMENTO debe estar comprendido entre 1 y 5"));
					continue;
				}
				
				//Si es persona jurídica, sólo acepta los tipos de doc. 1 y 5
				if (tipoPersona.equals("J") && !(tipoDocumento == 1 || tipoDocumento == 5)){
					listErrores.add(crearError(linea, cifnif, "Las personas jurídicas 'J' sólo aceptan " +
							"los tipos de documento 1(CIF) y 5(Otros)"));
					continue;
				}
				
				//Si es persona física, acepta todos los tipos de doc. excepto el 1
				if (tipoPersona.equals("F") && tipoDocumento == 1){
					listErrores.add(crearError(linea, cifnif, "Las personas físicas 'F' no aceptan el " +
							"tipo de documento 1(CIF)"));
					continue;
				}
				...
				if (tipoPersona.equals("F") && StringUtils.isEmpty(ape1)){
				...
				//Teléfono móvil
				tfnoMovil = (String) tercero.get(10);
				if (!StringUtils.isEmpty(tfnoMovil)){
					try{
						tfnoMovil = String.valueOf(Double.valueOf(tfnoMovil).intValue());
					}
					catch (Exception e) {
						listErrores.add(crearError(linea, cifnif, "El campo TFNO.MÓVIL debe ser de tipo numérico"));
						continue;
					}
				}
				
				//Teléfono móvil
				tfnoFijo = (String) tercero.get(11);
				if (!StringUtils.isEmpty(tfnoFijo)){
					try{
						tfnoFijo = String.valueOf(Double.valueOf(tfnoFijo).intValue());
					}
					catch (Exception e) {
						listErrores.add(crearError(linea, cifnif, "El campo TFNO.FIJO debe ser de tipo numérico"));
						continue;
					}
				}
				
				//Email
				email = (String) tercero.get(12);
			...
			logger.error("Error no controlado en la carga de los terceros. Línea " + linea + ". " + e.getMessage(), e);
			throw new ISPACRuleException
				("Error no controlado en la carga de los terceros. Línea " + linea + ". " + e.getMessage(), e);
		}
		return true;
	}
