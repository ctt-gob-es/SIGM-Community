...
public class DipucrNotificacionAcuerdoPlanesProvinciales2 implements IRule{
	
	private static final Logger logger = Logger.getLogger(DipucrNotificacionAcuerdoPlanesProvinciales2.class);
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {
		try{
			...
			logger.info("INICIO - DipucrNotificacionAcuerdoPlanesProvinciales2");			
			// APIS
			...
			if (participantes!=null && participantes.toList().size()>=1) {
        		...
        		while (it.hasNext()){
        			...
        			if ((((String)taskTpDoc.get("CT_TPDOC:NOMBRE")).trim().toUpperCase()).equals(("Notificación 2ª Ayuntamientos Aprobación Obra").trim().toUpperCase())){        				
...
//							        	plan = planService.getPlanProvincial(nombre, "2011");
			...
			cct.endTX(true);
			logger.info("FIN - DipucrNotificacionAcuerdoPlanesProvinciales2");
		} catch(Exception e) {
	...
	public void setSsVariables(IClientContext cct, IRuleContext rulectx) {
		try {
			String anio = "", anioAnterior = "", fechaTercerPleno = "", fechaPrimerBop = "", fechaPrimerPleno = "";
			...
			if(it.hasNext()){
				...
				fechaPrimerPleno = item.getString("PRIMERPLENO");
			...
			cct.setSsVariable("FECHAPRIMERPLENO", fechaPrimerPleno);
