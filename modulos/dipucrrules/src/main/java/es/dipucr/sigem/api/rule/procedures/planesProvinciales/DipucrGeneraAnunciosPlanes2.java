...
public class DipucrGeneraAnunciosPlanes2 implements IRule {

	private static final Logger logger = Logger.getLogger(DipucrGeneraAnunciosPlanes2.class);
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException {		
		try{
			logger.info("INICIO - DipucrGeneraAnunciosPlanes2");			
			...
			logger.info("FIN - DipucrGeneraAnunciosPlanes2");
	...
	private void generaAnuncioPlanCooperacion(IRuleContext rulectx, String anio) throws ISPACRuleException {
		try{
    		...
    		while (it.hasNext()){
    			...
    			if ((((String)taskTpDoc.get("CT_TPDOC:NOMBRE")).trim().toUpperCase()).equals(("Propuesta 2º Anuncio").trim().toUpperCase())){
    		...
    		if (documentTypeId != 0){
	        	...
	        	if(tpDocsTemplatesCollection==null || tpDocsTemplatesCollection.toList().isEmpty()){
	        		...
	        		while (docs.hasNext() && !encontrado){
	        			...
	        			if(((String)tpDocsTemplate.get("NOMBRE")).trim().toUpperCase().equals("BOP - Anuncio 2º Plan Cooperación".trim().toUpperCase())){
		...
		try{
    		...
    		if (documentTypeId != 0){
	        	...
	        	if(tpDocsTemplatesCollection==null || tpDocsTemplatesCollection.toList().isEmpty()){
	        		...
	        		while (docs.hasNext() && !encontrado){
	        			IItem tpDocsTemplate = (IItem)docs.next();
	        			if(((String)tpDocsTemplate.get("NOMBRE")).trim().toUpperCase().equals("BOP - Anuncio 2º Plan Complementario".trim().toUpperCase())){
	        				templateId = tpDocsTemplate.getInt("ID");
	        				encontrado= true;
	        			}
	        		}
...
//		    		el Nombre del municipio = 32
	...
	public void setSsVariables(IClientContext cct, IRuleContext rulectx) {
		try {
			String anio = "", anioAnterior = "", fechaTercerPleno = "", fechaPrimerBop = "";
			...
			if(it.hasNext()){
				...
				fechaTercerPleno = item.getString("TERCERPLENO");
			...
			if(fechaTercerPleno == null) fechaTercerPleno = "";
			...
			cct.setSsVariable("FECHATERCERPLENO", fechaTercerPleno);
	...
	public void deleteSsVariables(IClientContext cct) {	
		try {
			...
			cct.deleteSsVariable("FECHATERCERPLENO");
