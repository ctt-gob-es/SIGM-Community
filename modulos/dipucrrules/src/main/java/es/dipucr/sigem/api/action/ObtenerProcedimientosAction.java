...
public class ObtenerProcedimientosAction extends BaseAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		try{
			...
			oEntidad.setIdentificador(EntidadesAdmUtil.obtenerEntidad(cct));

			sessionId = oServicio.login(null, "", certificate, oEntidad);				

			request.getSession().setAttribute(Defs.SESION_ID, sessionId);
	   	...
	   	if (sessionId!=null){
			...
			String [] vListaServicios = clienteLigero.consultaProcedimientoByNIF(cif, codProcSvd);
			...
			if(vListaServicios != null){
	   		...
	   		ISPACInfo informacion=new ISPACInfo("El certificado el usuario no es valido.", "",false);
			request.getSession().setAttribute("infoAlert", informacion);
			IState currentstate = managerAPI.currentState(getStateticket(request));
			return NextActivity.refresh(request, mapping, currentstate);
	   	}
