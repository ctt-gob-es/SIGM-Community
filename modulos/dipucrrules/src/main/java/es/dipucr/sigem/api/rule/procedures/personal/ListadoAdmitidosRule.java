...
public class ListadoAdmitidosRule extends DipucrAutoGeneraDocIniTramiteRule {
		...
		try {
			if (refTabla.equals("%TABLA1%")) {
				String sQuery = "select exped.NUMEXP, NIFCIFTITULAR, IDENTIDADTITULAR, NREG, FREG, PUESTOTRABAJO  from spac_expedientes as exped, PERSONAL_PUESTO_TRABAJO as puesto WHERE exped.NUMEXP IN "
						+ "(SELECT NUMEXP_HIJO FROM SPAC_EXP_RELACIONADOS WHERE NUMEXP_PADRE='"+rulectx.getNumExp()+"' AND RELACION='Solicitud Convocatoria') and puesto.numexp = exped.numexp and admitido='SI' ORDER BY puestotrabajo, exped.FREG  ASC";
