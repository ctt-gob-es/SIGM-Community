...
public class DipucrTerminarTramiteTraerAcuerdoDcretoContratacionRule implements IRule {
	...
	private static final Logger logger = Logger.getLogger(DipucrTerminarTramiteTraerAcuerdoDcretoContratacionRule.class);
		...
		try{
	        ...
	        while (it.hasNext()){
	        	...
	        	if(taskTpDocCollection==null || taskTpDocCollection.toList().isEmpty()){
	        		...
	        		boolean esDecreto = false;
	        		while (itTpDOC.hasNext() && !esDecreto){
	        			...
	        			if (taskTpDoc.get("CT_TPDOC:NOMBRE").equals(Constants.TIPODOC.DOCUMENTACION_APROBACION)){
	        				...
	        				//compruebo que sea decreto asi coge solo el decreto que firma el pres-sec y no los dos.
	        				if(itemDoc.getString("NOMBRE").equals(Constants.DECRETOS._DOC_DECRETO)){
	        					esDecreto = true;
	        				}
