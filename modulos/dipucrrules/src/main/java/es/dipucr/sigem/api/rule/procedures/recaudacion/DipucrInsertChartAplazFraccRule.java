 ...
 *
 */
public class DipucrInsertChartAplazFraccRule implements IRule {
	
	private static final Logger logger = Logger.getLogger(DipucrInsertChartAplazFraccRule.class);
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException 
	{
        try
        {
        	...
        	logger.info("Encontrado el documento " + docId);
        	...
        	logger.info("Plantilla: " + strTemplateName);
        	if (!(strTemplateName.indexOf("fracc")>0) || !(strTemplateName.indexOf(" estim")>0) )
        	{
	        	logger.error("Se ha seleccionado la plantilla: " + doc.getString("DESCRIPCION") + ". No es la plantilla adecuada para el expediente: " + rulectx.getNumExp() + ". ");
	        	return new Boolean(false);
        	}
    		...
    		logger.info("Abierto fichero: " + fileName);
    		...
    		logger.info("Documento temporal guardado en: " + fileNameOut);
	...
	private void insertaTabla(IRuleContext rulectx , XComponent xComponent) throws ISPACException, Exception
	{
		...
		String strQuery = "WHERE NUMEXP='" + rulectx.getNumExp() + "' ORDER BY FECHA_FIN_PAGO, FRACCION ASC";
		...
		if (it.hasNext())
		{
	        ...
	        if (xSearchInterface != null) 
	        {
				...
				xTable.initialize(nLiquidaciones + 1, 10);
				...
				setHeaderCellText(xTable, "A1", "Liquidac.");				
				setHeaderCellText(xTable, "B1", "Fracción");				
				setHeaderCellText(xTable, "C1", "Cuota");				
				setHeaderCellText(xTable, "D1", "Intereses cuota");				
				setHeaderCellText(xTable, "E1", "Recargo");				
				setHeaderCellText(xTable, "F1", "Cuota Municipal");				
				setHeaderCellText(xTable, "G1", "Intereses Cuota Municipal");				
				setHeaderCellText(xTable, "H1", "Recargo Provincial");				
				setHeaderCellText(xTable, "I1", "Intereses Recargo Provincial");
				setHeaderCellText(xTable, "J1", "Fecha fin de pago");
				...
				while (it.hasNext())
				{
					...
					setCellText(xTable, "A"+String.valueOf(nLiquidacion+1), liquidacion.getString("LIQUIDACION"));
					setCellText(xTable, "B"+String.valueOf(nLiquidacion+1), liquidacion.getString("FRACCION"));
					setCellText(xTable, "C"+String.valueOf(nLiquidacion+1), liquidacion.getString("CUOTA"));
					setCellText(xTable, "D"+String.valueOf(nLiquidacion+1), liquidacion.getString("INTERESES_CUOTA"));
					setCellText(xTable, "E"+String.valueOf(nLiquidacion+1), liquidacion.getString("RECARGO"));
					setCellText(xTable, "F"+String.valueOf(nLiquidacion+1), liquidacion.getString("CUOTA_MUNICIPAL"));
					setCellText(xTable, "G"+String.valueOf(nLiquidacion+1), liquidacion.getString("INTERESES_CUOTA_MUNICIPAL"));
					setCellText(xTable, "H"+String.valueOf(nLiquidacion+1), liquidacion.getString("RECARGO_PROVINCIAL"));
					setCellText(xTable, "I"+String.valueOf(nLiquidacion+1), liquidacion.getString("INTERESES_RECARGO_PROVINCIAL"));
					...
					setCellText(xTable, "J"+String.valueOf(nLiquidacion+1), strDate);
				}
	        }
		}
	}
