...
public class CopyLiquidacionesIBIRule implements IRule {
	
	private static final Logger logger = Logger.getLogger(CopyLiquidacionesIBIRule.class);
        		...
        		if (!it2.hasNext())
        		{
        			...
        			for (int i=1 ; i<=20 ; i++)
        			{
	        			...
	        			if (strLiq != null && strLiq.length()>0)
	        			{
		    		        ...
		    		        {
		    		        	item.set("REF_CAT", strRef1+"-"+strRef2+"-"+strRef3+"-"+strRef4);
		    		        }
