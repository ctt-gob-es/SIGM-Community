...
public class ListadoMultiplesRegistrosRule implements IRule {
    ...
    private static final String SEPARADOR_PROPIEDADES_TAG = ";";
    ...
    public Object execute(IRuleContext rulectx) throws ISPACRuleException {
        ...
        try{
            ...
            if (codetables != null && codes != null && values != null){
                ...
                for (Iterator<?> iter = itemcol.iterator(); iter.hasNext() && j!= numRegistros;j++) {
                    ...
                    for (int i = 0; i < properties.length; i++) {
                        ...
                        if (StringUtils.isEmpty(values[i])){
                            buffer.append(item.getString(entity+":"+properties[i]));
                        }else{
                            buffer.append(item.getString(codetables[i]+":"+values[i]));
                        }
                    } 
                ...
                IEntitiesAPI entitiesAPI = rulectx.getClientContext().getAPI().getEntitiesAPI();
                IItemCollection itemcol = entitiesAPI.getEntities(entity, rulectx.getNumExp());
                int j = 0;
                ...
                for (Iterator<?> iter = itemcol.iterator(); iter.hasNext() && j!= numRegistros;j++) {
                    ...
                    for (int i = 0; i < properties.length; i++) {
                        ...
                        buffer.append(item.getString(properties[i]));
                    } 
                    buffer.append(registrySeparator);
                }
            }
