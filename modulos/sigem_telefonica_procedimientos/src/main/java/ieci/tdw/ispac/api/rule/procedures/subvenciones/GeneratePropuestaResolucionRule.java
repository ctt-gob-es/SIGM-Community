...
public class GeneratePropuestaResolucionRule implements IRule {
    ...
    public Object execute(IRuleContext rulectx) throws ISPACRuleException {
        ...
        try {
            ...
            //Inserta una tabla de 8 columnas y tantas filascomo solicitudes haya mas una de cabecera
            XTextTable xTable = LibreOfficeUtil.insertaTablaEnPosicion(xComponent, "%TABLA1%", nSolicitudesAdmitidas + 1, 8);
            ...
            //Inserta una tabla de 8 columnas y tantas filas como solicitudes haya más una de cabecera
            xTable = LibreOfficeUtil.insertaTablaEnPosicion(xComponent, "%TABLA2%", nSolicitudesRechazadas + 1, 2);
            
            if ( null != xTable){
                
                //Rellena la cabecera de la tabla
                LibreOfficeUtil.setTextoCeldaCabecera(xTable, "A1", "ENTIDAD LOCAL");                
                LibreOfficeUtil.setTextoCeldaCabecera(xTable, "B1", "MOTIVO DE LA DENEGACIÓN");                
                ...
                for (String numexpHijo : expedientesRelacionados) {
                    ...
                    if(it2.hasNext()) {
                        ...
                        if (StringUtils.isNotEmpty(strMotivo)) {    
                            nSolicitud++;
                            LibreOfficeUtil.setTextoCelda(xTable, "A" + (nSolicitud + 1), entidad.getString("NOMBRE"));
                            LibreOfficeUtil.setTextoCelda(xTable, "B" + (nSolicitud + 1), strMotivo);
                        }
                    }
                }
            }    
