...
public class GenerateBopRule implements IRule 
{
	...
	protected String STR_DocBopGeneral = "BOP - General";
	protected String STR_DocAnuncio = "BOP - Anuncio publicado";
...
//		String templateFilename = Messages.getString("cTemplate_bop"); [dipucr-Felipe #1337]
        ...
        try
    	{
			...
			IItem itemDocumento = DocumentosUtil.generarDocumento(rulectx, STR_DocBopGeneral, "borrar");
			...
			//Campo número BOP
			Object fieldMaster1 = xNamedFieldMasters.getByName(
					"com.sun.star.text.fieldmaster.User.field_num_bop");
			XPropertySet xPropertySet1 = (XPropertySet)UnoRuntime.queryInterface(
					XPropertySet.class, fieldMaster1);
			xPropertySet1.setPropertyValue("Content", "Número " + numBop);
			//Campo fecha
			Object fieldMaster2 = xNamedFieldMasters.getByName(
				"com.sun.star.text.fieldmaster.User.field_fecha_bop");
			XPropertySet xPropertySet2 = (XPropertySet)UnoRuntime.queryInterface(
					XPropertySet.class, fieldMaster2);
...
//	        		file = CommonFunctions.getFile(rulectx, doc.getString("INFOPAG"));
	        		file = DocumentosUtil.getFile(cct, doc.getString("INFOPAG"), null, null); //[eCenpri-Felipe #28 #95]
...
//	        		texto = "Número " + numAnun; //[eCenpri-Felipe #593]
	        		texto = _TEXTO_NUM_ANUNCIO + numAnun;
	    			logger.warn("NÚMERO DE ANUNCIO: " + texto);
	        		
	    			//INICIO [eCenpri-Felipe #593] Actualizamos el número de anuncio en la BBDD
	    			String strQueryAux2 = "WHERE NUMEXP = '" + anuncio.getString("NUMEXP_ANUNCIO") + "'";
	    			IItemCollection col = entitiesAPI.queryEntities("BOP_SOLICITUD", strQueryAux2);
	        ...
	        //Todos estos pasos son para acceder a la lista de referencias
	        ...
	        if (aNames.length > 0) 
	        {
		        ...
		        for (int j = 0; j < nAnuncio; j ++)
		        {
			        ...
			        XPropertySet xFieldProps = (XPropertySet) UnoRuntime.queryInterface(
				             XPropertySet.class, xDocMSF.createInstance(
...
//			int color = Color.WHITE.getRGB(); //No funciona
			...
			xText.insertString(xTextCursor, _MARCA_FIN_INDICE, false);
			//FIN [eCenpri-Felipe #593]
    		...
    		else
    		{
				...
				for (int k = 0; k < nAnuncio; k ++)
		        {
					...
					String strPrintfileName = FileTemporaryManager.getInstance().getFileTemporaryPath() 
						+ "/" + strTemplateName  + ".pdf";
					...
					strQuery = "WHERE NOMBRE = '" + STR_DocAnuncio + "'";
			...
			*/ 
    		...
    		//Guarda el resultado en gestor documental
			strQuery = "WHERE NOMBRE = '" + STR_DocBopGeneral + "'";
			logger.warn("Query: " + strQuery);
	        IItemCollection collection = entitiesAPI.queryEntities("SPAC_CT_TPDOC", strQuery);
	        it = collection.iterator();
	        int tpdoc = 0;
    		...
    		Object connectorSession = gendocAPI.createConnectorSession();
    		IItem entityOdt = gendocAPI.attachTaskInputStream(connectorSession, rulectx.getTaskId(), odtId, inODT, (int)fileODT.length(), "application/vnd.oasis.opendocument.text", STR_DocBopGeneral);
    		entityOdt.set("EXTENSION", "odt");
    		entityOdt.store(cct);
    		fileODT.delete();    		
    		/*
    		IItem newdoc = gendocAPI.createTaskDocument(rulectx.getTaskId(), tpdoc);
    		FileInputStream inDOC = new FileInputStream(fileDOC);
    		int docId = newdoc.getInt("ID");
    		IItem entityDoc = gendocAPI.attachTaskInputStream(connectorSession, rulectx.getTaskId(), docId, inDOC, (int)fileDOC.length(), "application/msword", STR_DocLibro);
...
//        	itemBop.store(cct);
	...
	/**
	 * [eCenpri-Felipe #28]
	 * Todos los anuncios serán a partir de ahora ODT
	 * Método muy similar al concatenaODT de DipucrCommonFunctions pero sin el salto de párrafo
