...
public class GenerateActaRule implements IRule {

    protected String strDocName = "Borrador de Acta de Sesión";
    protected String strFooterDocName = "Borrador de Acta de Sesión - Propuesta - Pie";
    ...
    public Object execute(IRuleContext rulectx) throws ISPACRuleException {
        try {
            ...
            if ( !TramitesUtil.existeTramite(rulectx, "Convocatoria de la sesión")) {
                throw new ISPACInfo("Es necesario realizar antes el trámite 'Convocatoria de la sesión'");
            }
            
            if (TramitesUtil.existeTramite(rulectx, strDocName)) {
                throw new ISPACInfo("Ya existe un trámite llamado '" + strDocName + "'");
            }
            ...
            while(itTpDocs.hasNext()) {
                ...
                boolean esPropuesta = 
                    nombrePlantilla.compareTo("Borrador de Acta de Sesión - Propuesta - Cabecera")==0 || 
                    nombrePlantilla.compareTo(strFooterDocName)==0 ;
            ...
            //Obtiene la cabecera
            String strInfoPag = DocumentosUtil.getInfoPagByDescripcionEquals(cct, rulectx.getNumExp(), "Borrador de Acta de Sesión - Cabecera", "");
            ...
            for ( int i=1 ; i<=nTotal ; i++) {
                descr = "Borrador de Acta de Sesión - Propuesta - Cabecera - " + df.format(i);
                ...
                descr = strFooterDocName + " - " + df.format(i);
                strInfoPag = DocumentosUtil.getInfoPagByDescripcionEquals(cct, rulectx.getNumExp(), descr, "");
                file = DocumentosUtil.getFile(cct, strInfoPag, "", "doc");
                CommonFunctions.Concatena(xComponent, "file://" + file.getPath());
                file.delete();
            ...
            strInfoPag = DocumentosUtil.getInfoPagByDescripcionEquals(cct, rulectx.getNumExp(), "Borrador de Acta de Sesión - Ruegos y preguntas", "");
            file = DocumentosUtil.getFile(cct, strInfoPag, "", "doc");
            CommonFunctions.Concatena(xComponent, "file://" + file.getPath());
            file.delete();

            //Obtiene e inserta el pie del acta.
            strInfoPag = DocumentosUtil.getInfoPagByDescripcionEquals(cct, rulectx.getNumExp(), "Borrador de Acta de Sesión - Pie", "");
            file = DocumentosUtil.getFile(cct, strInfoPag, "", "doc");
            CommonFunctions.Concatena(xComponent, "file://" + file.getPath());
            file.delete();
            ...
            String strQuery = "WHERE NOMBRE = '" + strDocName + "'";
            ...
            IItem entityDoc = gendocAPI.attachTaskInputStream(connectorSession, taskId, docId, in, (int)file.length(), "application/msword", strDocName);
            entityDoc.set(DocumentosUtil.EXTENSION, "doc");
            entityDoc.store(cct);
            file.delete();
            
            //Borra los documentos intermedios del gestor documental
            strQuery = "WHERE NUMEXP = '" + rulectx.getNumExp() + "' AND DESCRIPCION LIKE 'Borrador de Acta de Sesión -%'";
