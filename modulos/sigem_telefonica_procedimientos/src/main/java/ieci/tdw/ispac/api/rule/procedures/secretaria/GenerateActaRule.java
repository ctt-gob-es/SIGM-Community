...
public class GenerateActaRule implements IRule {
	...
	protected String STR_docName = "Borrador de Acta de Sesión";
	protected String STR_footerDocName = "Borrador de Acta de Sesión - Propuesta - Pie";
	...
	public Object execute(IRuleContext rulectx) throws ISPACRuleException
	{
		try
		{
	        ...
	        if ( CommonFunctions.existeTramite(rulectx,STR_docName))
	        {
	        	throw new ISPACInfo("Ya existe un trámite llamado '" + STR_docName + "'");
	        }
        	...
        	while(itTpDocs.hasNext())
        	{
	        	...
	        	boolean esPropuesta = 
	        		...
	        		nombrePlantilla.compareTo(STR_footerDocName)==0 ;
        	...
        	//Obtiene la cabecera
        	String strInfoPag = CommonFunctions.getInfoPag(rulectx, entitiesAPI, "Borrador de Acta de Sesión - Cabecera");
        	...
        	for ( int i=1 ; i<=nTotal ; i++)
        	{
        		...
        		descr = STR_footerDocName + " - " + df.format(i);
            	strInfoPag = CommonFunctions.getInfoPag(rulectx, entitiesAPI, descr);
        		file = CommonFunctions.getFile(gendocAPI, strInfoPag);
        		CommonFunctions.Concatena(xComponent, "file://" + file.getPath(), ooHelper);
        		file.delete();
        	...
        	strInfoPag = CommonFunctions.getInfoPag(rulectx, entitiesAPI, "Borrador de Acta de Sesión - Ruegos y preguntas");
    		file = CommonFunctions.getFile(gendocAPI, strInfoPag);
    		CommonFunctions.Concatena(xComponent, "file://" + file.getPath(), ooHelper);
    		file.delete();

    		//Obtiene e inserta el pie del acta.
        	strInfoPag = CommonFunctions.getInfoPag(rulectx, entitiesAPI, "Borrador de Acta de Sesión - Pie");
    		file = CommonFunctions.getFile(gendocAPI, strInfoPag);
    		CommonFunctions.Concatena(xComponent, "file://" + file.getPath(), ooHelper);
    		file.delete();
			...
			String strQuery = "WHERE NOMBRE = '" + STR_docName + "'";
    		...
    		IItem entityDoc = gendocAPI.attachTaskInputStream(connectorSession, taskId, docId, in, (int)file.length(), "application/msword", STR_docName);
