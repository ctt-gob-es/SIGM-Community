			...
			// --------------------------------------------------------------------
				...
				if(!datContrato.getTipoContrato().getId().equals("50")){
					...
					if(presupuesto!=null && presupuesto.length==2){
						resultadoLicitacion.setPresupuestoConIva(presupuesto[0]);
						resultadoLicitacion.setPresupuestoSinIva(presupuesto[1]);
					}
				}
				...
				resultado = platContratacion.anuncioAdjudicacion(entidad, resultadoLicitacion, publishedByUser);
