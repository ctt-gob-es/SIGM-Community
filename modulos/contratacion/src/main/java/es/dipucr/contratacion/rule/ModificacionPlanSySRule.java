...
public class ModificacionPlanSySRule extends PlanSeguridadSaludRule{
	public boolean init(IRuleContext rulectx) throws ISPACRuleException {
		...
		try{
			...
			String strQuery = "WHERE NUMEXP_PADRE='" + rulectx.getNumExp() + "' AND RELACION LIKE 'Modificación del Plan de SyS - %'";
			IItemCollection collectExpRel = entitiesAPI.queryEntities(Constants.TABLASBBDD.SPAC_EXP_RELACIONADOS, strQuery);
			relacion = "Modificación del Plan de SyS - "+(collectExpRel.toList().size()+1)+" - "+FechasUtil.getFormattedDate(new Date());
