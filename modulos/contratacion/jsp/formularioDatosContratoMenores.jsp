	...
	<ispac:tab title='<%=aux0%>'>
		<div id="dataBlock_CONTRATACION_DATOS_CONTRATO" style="position: relative; height: 300px; width: 600px">
			...
			<div id="label_CONTRATACION_DATOS_CONTRATO:CPV"
				...
				class="formsTitleB">
				<bean:write name="defaultForm" property="entityApp.label(CONTRATACION_DATOS_CONTRATO:CPV1)" />:
			</div>
			
			<div id="data_CONTRATACION_DATOS_CONTRATO:CPV1" style="position: absolute; top: 185px; left: 200px; width: 100%;">
				<nobr>
					...
					action="selectValue.do?entity=CONTRATACION_VLDTBL_CPVMENOR" 
					image="img/search-mg.gif" titleKeyLink="select.rol" imageDelete="img/borrar.gif" titleKeyImageDelete="title.delete.data.selection" 
					styleClassDeleteLink="tdlink" confirmDeleteKey="msg.delete.data.selection" showDelete="true" showFrame="true" width="640" height="480" 
					tabindex="113" jsShowFrame="hideShowFrameCPV1" jsDelete="hideDeleteCPV1">
						<ispac:parameter name="SEARCH_CONTRATACION_DATOS_CONTRATO_CPV1" id="property(CONTRATACION_DATOS_CONTRATO:CPV1)" property="VALOR" />
					</ispac:htmlTextareaImageFrame>
				</nobr>
			</div>
			
			<div style="POSITION: absolute; WIDTH: 100%; TOP: 185px; LEFT:750px" id=data_CONTRATACION_DATOS_CONTRATO:CPV>
				<ispac:htmlTextMultivalue
					property="propertyMultivalue(CONTRATACION_DATOS_CONTRATO:CPV)" readonly="false"
					propertyReadonly="readonly" styleClass="input" styleClassReadonly="inputReadOnly"  size="80" divWidth="500" 
					maxlength="1024">
				</ispac:htmlTextMultivalue>
			</div>
...
function insertCommonMultivalueElement(name, propertyName, size, maxlength, inputClass, blockHTML){
   ...
   if(valor != ""){
	   newdiv.innerHTML = '<input type="checkbox" value="' + id + '" name="checkbox_'+name+'"/><input id="'+styleId+'" type="text" readonly class="'+inputClass+'" size="'+size+'" maxlength="'+maxlength+'" name="' + propertyName + '" value="'+valor+'"/>'
	   +blockHTML;
   }
   else{
