...
public class PeticionSincronaConDatosEspecSync extends PeticionSincronaConDatosEspec{
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		OMElement elementDatosEspecificos = generarSolicitudTransmision(request, session.getClientContext());
	...
	private ActionForward envioPeticionDirecta(HttpServletRequest request, SessionAPI session, OMElement elementDatosEspecificos, String version) throws ISPACRuleException {
	...
	private List<CharSequence> invoke(String codigoCertif, String nombreProc, SessionAPI session,
			HttpServletRequest request, OMElement elementDatosEspecificos, String version) throws ISPACException {
		//----------------------------------------------------------------------------------------------
		ClientContext cct = session.getClientContext();
		...
		try {
			...
			logger.error("Error al obtener la entidad "+ state.getNumexp() +" - "+e2.getMessage(), e2);
			throw new ISPACRuleException("Error al obtener la entidad "+ state.getNumexp() +" - "+e2.getMessage(), e2);
		}
    	...
    	//SVDSCCDWS01 -> Consulta de defunción
		//SVDSCCMWS01 -> Consulta de matrimonio
		//SVDSCCNWS01 -> Consulta de nacimiento
    	//Númuero de la incidencia es 350947 seguimiento 906034
    	//Independientemente de si se añadan tomo y página, o fecha hecho registral y municipio de nacimiento... Si se introduce el dato del NIF, 
    	//la consulta se hará por ese grupo de consulta, ignorando el resto de datos.ç
    	logger.warn("Prueba SVD. numexp "+numexp+" códido del certificado "+codigoCertif+ "con DNI "+ndocPartic);
    	if(StringUtils.isNotEmpty(ndocPartic) && (!codigoCertif.equals("SVDSCCDWS01") && !codigoCertif.equals("SVDSCCMWS01") && !codigoCertif.equals("SVDSCCNWS01"))){
			...
			//Consulta de datos de identidad no utiliza NIE, CIF -> utiliza [DNI, NIE]
    		//SVDMATUNIVWS01 -> Consulta de datos de matrícula
    		if(codigoCertif.equals("SVDDGPCIWS02") || codigoCertif.equals("SVDMATUNIVWS01")){
    			if(isNumeric(caracterPrimero)){
    	    		tipoDocumentacion.setText("DNI");
    	    	}
    	    	else{
	    ...
	    if (map != null) {
			...
			if (map.get("nombre") != null) {
				...
				OMElement nombreC = fac.createOMElement("Nombre", omNs);
				nombreC.setText(nombre);
				titular.addChild(nombreC);
			}
			String priApellid = "";
			if (map.get("priApellido") != null) {
				priApellid = (String) map.get("priApellido");
				
				OMElement apellido = fac.createOMElement("Apellido1", omNs);
				apellido.setText(priApellid);
				titular.addChild(apellido);
			}
			String segApellid = "";
			if (map.get("segApellido") != null) {
				segApellid = (String) map.get("segApellido");
				
				OMElement apellido2 = fac.createOMElement("Apellido2", omNs);
				apellido2.setText(segApellid);
				titular.addChild(apellido2);
			}
	    }
		...
		if(elementDatosEspecificos!=null){
			solicitudTransmision.addChild(elementDatosEspecificos);
			/*String ruta = "";
		...
		try {
			...
			element = DipucrFuncionesComunesSVD.toDOM(res);
			client.cleanup();
		...
		if(codigoCertif.equals("SVDCDYGWS01") || codigoCertif.equals("SVDCTITWS01")){
			...
			if(!literalError.equals("USUARIO NO AUTORIZADO")){
				...
				try {
					...
					Element respuestaPdf = DipucrFuncionesComunesSVD.toDOM(res);
					client.cleanup();
					client.cleanupTransport();
