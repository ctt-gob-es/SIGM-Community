...
public class CMSSignature extends CMSPKCS7Signature{
	 ...
	 * previo a la firma, se realiza una transformación del documento a formato 
	 * <a href="http://www.unicode.org/">Unicode</a>. En este método se probará la validación
	 * con el documento original y, si este se da por no válido, se probará con el documento
	...
	public ValidationResult[] isValidSignatureOnly () throws HashingException, SignatureException, NoDocumentToSignException {
		if (this.document == null) {
			throw new NoDocumentToSignException ("La firma no es attached por lo que no se puede realizar la " +
					"validación si no se facilita el documento que originó la firma");
		}
	...
	public ValidationResult[] isValidSignatureOnly(IDocument document) throws HashingException, SignatureException {
		...
		//-- Obtener la validación con el documento original
		ValidationResult[] result = super.isValidSignatureOnly(document);
		
		//-- Comprobar si todas las firmas son válidas
		...
		if (allValid) {
			//-- Todo es válido, así que el documento no estaba en Unicode
			return result;
		}
		...
		try {
			...
			// No se debe dar porque el fichero se creó en el método toUnicode
		...
		for (int i = 0; i < result.length; i++) {
			if (!result [i].isValid() && result2[i].isValid()) {
				logger.debug ("[CMSSignature.isValidSignatureOnly]::La firma era válida en unicode");
				result[i] = result2[i];
			}
		}
	 ...
	 * 
	 * La firma se realizará con el algoritmo SHA1withRSA.
	 * 
