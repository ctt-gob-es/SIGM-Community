...
//    memset(&appID, 0, sizeof(TW_IDENTITY));
    ...
    lstrcpy(appID.Version.Info, "2009-02-14");
    ...
    if(rc==TWRC_SUCCESS){
      ...
      if(rc==TWRC_FAILURE){
        ...
        if(rc==TWRC_SUCCESS){
          char str[256]={0};
          sprintf(str,"Cannot open 64 bit Source Manager: status = %d.\n",status.ConditionCode);
          JNU_ThrowByName(env,"uk/co/mmscomputing/device/twain/TwainException",str);
          return false;
        }
      }
      JNU_ThrowByName(env,"uk/co/mmscomputing/device/twain/TwainException","Cannot open 64 bit Source Manager.");
      return false;
    }
...
JNIEXPORT jlong JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_ninitLib(JNIEnv* env, jclass clazz){
...
JNIEXPORT jbyteArray JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_ngetContainer(JNIEnv* env, jclass clazz, 
  ...
  long len = env->GetArrayLength(jcapbuf);
  if(len!=12){JNU_ThrowByName(env,"java/lang/IllegalArgumentException","jtwain.cpp: ngetContainer: Illegal argument (Cap Array Length not 12).");return NULL;}
...
JNIEXPORT void JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_nsetContainer(JNIEnv* env, jclass clazz, 
  ...
  if(len!=12){JNU_ThrowByName(env,"java/lang/IllegalArgumentException","jtwain.cpp: nsetCapability: Illegal argument (Cap Array Length not 12).");return;}

  len = env->GetArrayLength(jconbuf);
  if(len==0){JNU_ThrowByName(env,"java/lang/IllegalArgumentException","jtwain.cpp: nsetCapability: Illegal argument (Con Array Zero Length).");return;}
...
JNIEXPORT void JNICALL Java_uk_co_mmscomputing_device_twain_jtwain_nfreeContainer(JNIEnv* env, jclass clazz, 
  ...
  long len = env->GetArrayLength(jcapbuf);
  if(len!=12){JNU_ThrowByName(env,"java/lang/IllegalArgumentException","jtwain.cpp: nfreeContainer: Illegal argument (Cap Array Length not 12).");return;}

  TW_CAPABILITY* ccapbuf = (TW_CAPABILITY*)env->GetByteArrayElements(jcapbuf,NULL);
  if(ccapbuf==NULL){return;}

  HANDLE handle = ccapbuf->hContainer;
  if(handle!=0){DSM_Free(handle);}
}
