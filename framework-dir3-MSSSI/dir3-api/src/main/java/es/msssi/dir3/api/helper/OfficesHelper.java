...
public class OfficesHelper {
	...
	if (officeVO != null) {
	    ...
	    office.setInterchangeLocalGeneralAutonomousIndicator(officeVO
		.getInterchangeLocalGeneralAutonomousIndicator());
	    office.setInterchangeLocalGeneralLocalIndicator(officeVO
		.getInterchangeLocalGeneralLocalIndicator());
	    office.setInterchangeLocalStateIndicator(officeVO.getInterchangeLocalStateIndicator());
	    office.setInterchangeWithoutRestrictionIndicator(officeVO
		.getInterchangeWithoutRestrictionIndicator());
	    office.setInterchangeWithRestrictionCityHallCityHallIndicator(officeVO
		.getInterchangeWithRestrictionCityHallCityHallIndicator());
	    office.setInterchangeWithRestrictionLocalAutonomousIndicator(officeVO
		.getInterchangeWithRestrictionCityHallCityHallIndicator());
	    office.setInterchangeWithRestrictionLocalLocalIndicator(officeVO
		.getInterchangeWithRestrictionLocalLocalIndicator());
	    ...
	    office.setOrganizationalRelationshipsOFI(getRelationships(officeVO
		.getOrganizationalRelationshipsOFI()));
     ...
     */
    public static List<OfficeVO> completeOffices(
	OfficesVO officesDCO, ContactsOFIVO contactsDCO, ServicesVO servicesDCO) {
	List<OfficeVO> listOffice = null;
	if (officesDCO != null &&
	    officesDCO.getOffices() != null && officesDCO.getOffices().size() > 0) {
		...
		if (contactsDCO != null &&
		    contactsDCO.getContacts() != null) {
		    for (ContactOFIVO contact : contactsDCO.getContacts()) {
			if (contact.getOfficeId().equals(
			    codigoOffice)) {
		...
		if (servicesDCO != null &&
		    servicesDCO.getServices() != null) {
		    for (ServiceVO service : servicesDCO.getServices()) {
			if (service.getOfficeId().equals(
			    codigoOffice)) {
    ...
    /**
     * Convierte una lista de criterios del core a Criterios de oficinas del api.
     * 
     * @param criteriaCore
     *            lista de criterios de oficinas del core.
     * @return criteria lista de Criterios de oficinas del api.
     */
    public static Criteria<OfficeCriterionEnum> criteriaOFCoretoCriteriaAPIOF(
	CriteriaOF criteriaCore) {
	...
	if (criteriaCore != null) {
		...
		for (CriterionOF criterioCore : criteriaCore.getCriteria()) {
		    Criterion<OfficeCriterionEnum> criterionAPI =
			new Criterion<OfficeCriterionEnum>(
			    OfficeCriterionEnum.getCriterion(criterioCore.getName().getValue()),
			    OperatorCriterionEnum.getOperatorCriterion(criterioCore.getOperator()
				.getValue()), criterioCore.getValue());
	...
	if (criteriaCore != null) {
		...
		for (CriterionOF criterionCore : criteriaCore) {
			...
			new Criterion<OfficeCriterionEnum>(
			    OfficeCriterionEnum
				.getNameCriterion(criterionCore.getName().getValue()),
