...
public class UnitManagerImpl implements UnitManager, Serializable {
	...
	private static final Logger LOGGER = Logger
			.getLogger(UnitManagerImpl.class);
	 ...
	 */
	public int count(List<Criterion<UOCriterionEnum>> criteriaAPI)
			throws DIR3Exception {
		...
		try {
			...
			LOGGER.error(ErrorConstants.GET_COUNT_UNIT_ERROR_MESSAGE,
					sqlException);
	...
	public List<UnitVO> getAll() throws DIR3Exception {
		...
		try {
			...
			LOGGER.error(ErrorConstants.GET_ALL_UNITS_ERROR_MESSAGE,
					sqlException);
			throw new DIR3Exception(DIR3ErrorCode.GET_ALL_UNITS_ERROR,
					ErrorConstants.GET_ALL_UNITS_ERROR_MESSAGE, sqlException);
		}
		LOGGER.info("Número de unidades orgánicas encontradas: " + result != null ? result
				.size() : "null");
		return result;
	}
	 ...
	 * @throws DIR3Exception
	 */
	public List<UnitVO> find(Criteria<UOCriterionEnum> criteria)
			throws DIR3Exception {
	...
	public List<BasicDataUnitVO> findBasicData(
			Criteria<UOCriterionEnum> criteria) throws DIR3Exception {
	...
	public void save(UnitVO unit) throws DIR3Exception {
		...
		try {
			if (unit != null) {
				...
				AddressVO direccion = unit.getAddress();
				if (null != direccion) {
					LOGGER.debug("Insertando la dirección nueva");

					rellenaCiudad(direccion);
					
					addressId = addressDao.saveReturn(direccion);
				...
				if (unit.getContacts() != null && unit.getContacts().size() > 0) {
			...
			if (unit != null) {
				...
				if (addressId == null) {
					...
					if (null != address) {
						...
						rellenaCiudad(address);

						addressId = addressDao.saveReturn(address);
						LOGGER.debug("Nueva dirección con id: " + addressId);
					...
					LOGGER.debug("La unidad si tiene dirección actualmente con id: " + addressId);
					// SI TENIA DIRECCION
					if (unit.getAddress() != null) {
						...
						address.setId(Integer.toString(addressId));
						tempUnit.setAddress(address);
						LOGGER.debug("Se modifica la dirección " + addressId);
				...
				if (unit.getContacts() != null && unit.getContacts().size() > 0) {
					...
					if (contactsBBDD != null && contactsBBDD.size() > 0) {
						...
						for (ContactUOVO contact : unit.getContacts()) {
							...
							if (!exists) {
								...
								if (!contactbbdd.getStatusId().equals(unit.getStatus())) {
					...
					if (contactsBBDD != null && contactsBBDD.size() > 0) {
						...
						if (unit.getStatus().equals("E") || unit.getStatus().equals("T") || unit.getStatus().equals("A")) {
							// SI LA UNIDAD TIENE ESTADO E,A O T, los contactos vigentes se modifican a estado E
							for (ContactUOVO contactBBDD : contactsBBDD) {
								contactBBDD.setStatusId("E");
								contactDao.update(contactBBDD);
							}
	...
	private void rellenaCiudad(AddressVO direccion) {
		String dir3Ciudad = "L01";
		try{
			if(null != direccion ){
				...
				dir3Ciudad += direccion.getProvinceId() + direccion.getCityId();
			
				UnitVO unidad = unitDao.get(dir3Ciudad);
				if(null != unidad && StringUtils.isNotEmpty(unidad.getName())){
					String nombreCiudad = UTFHelper.parseUTF8ToISO885916(unidad.getName());
					nombreCiudad = nombreCiudad.replaceAll("Ayuntamiento de ", "");
					nombreCiudad = nombreCiudad.replaceAll("Concello de ", "");
					nombreCiudad = nombreCiudad.replaceAll("Ajuntament de ", "");
					nombreCiudad = nombreCiudad.replaceAll("Ajuntament D'", "");
			...
			LOGGER.error("ERROR al recuperar el municipio con DIR3: " + dir3Ciudad + ". " + e.getMessage(), e);					
	 ...
	 */
	private boolean compareContact(ContactUOVO contactbbdd,
			ContactUOVO contactxml) {
		boolean result = false;
		if (contactbbdd != null && contactxml != null) {
			if ((contactbbdd.getContactInfo() != null && contactbbdd
					.getContactInfo().equals(contactxml.getContactInfo()))
					&& (contactbbdd.getContactTypeId() != null && contactbbdd
							.getContactTypeId().equals(
	 ...
	 *            valor del campo a guardar.
	 */
	public void setContactDao(ContactUODao contactDao) {
	 ...
	 *            valor del campo a guardar.
	 */
	public void setHistoryUODao(HistoryDao historyUODao) {
	...
	public void insertUpdateUnits(List<UnitVO> unitsDCO) throws DIR3Exception {
		...
		while (itr.hasNext()) {
			try {
				...
				LOGGER.error(ErrorConstants.INSERTUPDATE_UNIT_ERROR_MESSAGE
	 ...
	 * @throws DIR3Exception .
	 */
	public void updateUnits(UnitsVO unitsDCO, ContactsUOVO contactsUO,
			HistoriesVO historiesUO) throws DIR3Exception {
		...
		/*
		 * if (historiesUO != null && historiesUO.getHistories() != null) {
	 ...
	 * @throws DIR3Exception .
	 */
	public void saveHistoriesUnits(HistoriesVO historiesUO)
			throws DIR3Exception {
		...
		try {
					...
					try {
						...
						if (!exist) {
							...
							if (previousUnit.getDeleteObservations() != null) {
								causeId = historyUODao.getCauseId(previousUnit
										.getDeleteObservations());
								if (causeId == null) {
									causeId = "ZZZ";
									history.setObservations(previousUnit
											.getDeleteObservations());
								}
			...
			LOGGER.error(ErrorConstants.INSERT_HISTORIES_UNIT_ERROR_MESSAGE,
					sqlException);
			throw new DIR3Exception(DIR3ErrorCode.INSERT_HISTORIES_UNIT_ERROR,
					ErrorConstants.INSERT_HISTORIES_UNIT_ERROR_MESSAGE,
					sqlException);
		} finally {
	...
	/**
	 * Modifica todos los elementos de las unidades no organicas que entran como
	 * parámetros.
	 * 
	 * @param unitsDCO
	 *            lista de unidades a modificar.
