...
public class UnitRegistroDaoImpl extends SqlMapClientBaseDao implements UnitRegistroDao {
	...
	public UnitVO get(String id) throws SQLException {
		...
		UnitVO unit;
		unit = (UnitVO) getSqlMapClientTemplate().queryForObject(GETUNIT, id);
		return unit;
	}
	...
	public void save(UnitVO entity) throws SQLException {
		...
		getSqlMapClient().update(NEXTIDUNIT);
		...
		getSqlMapClient().insert(INSERTUNIT, registro);
	}
	
	public void save(UnitRegistro registro) throws SQLException {
		logger.info("Guardar una unidad orgánica");
		getSqlMapClient().update(NEXTIDUNIT);
		if (registro.getCodeFather() != null) {
			registro.setIdFather((Integer) getSqlMapClient().queryForObject( GETIDUNIT, registro.getCodeFather()));
		}
		if (registro.getCodeRoot() != null) {
			registro.setIdRoot((Integer) getSqlMapClient().queryForObject( GETIDUNIT, registro.getCodeRoot()));
		}
	...
	public void update(UnitVO entity) throws SQLException {
		...
		getSqlMapClient().update(UPDATEUNIT, registro);
	}
	
	public void update(UnitRegistro registro) throws SQLException {
		logger.info("Modifico una unidad orgánica");
		if (registro.getCodeFather() != null) {
			registro.setIdFather((Integer) getSqlMapClient().queryForObject( GETIDUNIT, registro.getCodeFather()));
		}
	...
	public void delete(String code) throws SQLException {
		logger.info("Guardar una unidad orgánica");
		getSqlMapClient().update(DELETEUNIT, code);
	}

	/**
	 * Método para recuperar los datos básicos de todas las unidades que cumplan
	 * los criterios introducidos.
	 * 
	 * @param criterios
