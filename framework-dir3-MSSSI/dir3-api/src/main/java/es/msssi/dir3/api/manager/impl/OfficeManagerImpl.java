...
public class OfficeManagerImpl implements OfficeManager, Serializable {
	 ...
	 */
	public int count(List<Criterion<OfficeCriterionEnum>> criteria)
			throws DIR3Exception {
		...
		try {
			...
			LOGGER.error(ErrorConstants.GET_COUNT_OFFICE_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(DIR3ErrorCode.GET_COUNT_OFFICE_ERROR, ErrorConstants.GET_COUNT_OFFICE_ERROR_MESSAGE, sqlException);
		}
		LOGGER.info("Número de oficinas: " + result);
		return result;
	}
	...
	public int count() throws DIR3Exception {
		...
		LOGGER.info("Número total de oficinas: " + result);
		return result;
	}
	 ...
	 * @throws DIR3Exception .
	 */
	public OfficeVO get(String codeOffice) throws DIR3Exception {
		OfficeVO result = null;
		try {
			LOGGER.info("Recuperando una oficina con id: " + codeOffice);
			// Buscamos datos sobre la oficina
			result = (OfficeVO) officeDao.get(codeOffice);
		} catch (SQLException sqlException) {
			LOGGER.error(ErrorConstants.GET_OFFICE_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(DIR3ErrorCode.GET_OFFICE_ERROR, ErrorConstants.GET_OFFICE_ERROR_MESSAGE, sqlException);
		}
		return result;
	}
	 ...
	 * @throws DIR3Exception .
	 */
	public List<OfficeVO> getAll() throws DIR3Exception {
		...
		try {
			...
			LOGGER.error(ErrorConstants.GET_ALL_OFFICES_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(DIR3ErrorCode.GET_ALL_OFFICES_ERROR, ErrorConstants.GET_ALL_OFFICES_ERROR_MESSAGE, sqlException);
		}
		LOGGER.info("Número de oficinas encontradas: " + result != null ? result.size() : "null");
		return result;
	}
	 ...
	 * @throws DIR3Exception .
	 */
	public List<OfficeVO> find(Criteria<OfficeCriterionEnum> criteria)
			throws DIR3Exception {
		...
		try {
			...
			LOGGER.error(ErrorConstants.FIND_OFFICES_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(DIR3ErrorCode.FIND_OFFICES_ERROR, ErrorConstants.FIND_OFFICES_ERROR_MESSAGE, sqlException);
		}
		LOGGER.info("Número de oficinas encontradas: " + result != null ? result.size() : "null");
		return result;
	}
	...
	public List<BasicDataOfficeVO> findBasicData(
			Criteria<OfficeCriterionEnum> criteria) throws DIR3Exception {
	...
	public boolean exists(String id) throws DIR3Exception {
		...
		try {
			LOGGER.info("Comprobando la existencia de una oficina con id:" + id);
			// Buscamos datos sobre la oficina
			result = officeDao.exists(id);
		} catch (SQLException sqlException) {
			LOGGER.error(ErrorConstants.EXISTS_OFFICE_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(DIR3ErrorCode.EXISTS_OFFICE_ERROR, ErrorConstants.EXISTS_OFFICE_ERROR_MESSAGE, sqlException);
		}
		LOGGER.info("La oficina existe?: " + result);
		return result;
	}
	 ...
	 * 
	 */
	public void save(OfficeVO office) throws DIR3Exception {
		LOGGER.info("Insertando una oficina");
		try {
			Integer addressId = null;
			OfficeVO tempOffice = null;
			if (office != null) {
				LOGGER.info("Insertando la oficina: " + office.getId());
				tempOffice = office;
				...
				if (office.getAddress() != null && office.getAddress().getStreetName() != null) {
					LOGGER.debug("Insertando la dirección nueva");
					addressId = addressDao.saveReturn(office.getAddress());
					LOGGER.debug("Nueva dirección es: " + addressId);
				...
				if (office.getServices() != null && office.getServices().size() > 0) {
					LOGGER.debug("Insertando los servicios");
					for (ServiceVO service : office.getServices()) {
						serviceDao.save(service);
					}
				}
				if (office.getContacts() != null && office.getContacts().size() > 0) {
			...
			LOGGER.error(ErrorConstants.SAVE_OFFICE_ERROR_MESSAGE, sqlException);
			LOGGER.error("CÓDIGO DE OFICINA FALLIDA: " + office.getId());
			throw new DIR3Exception(DIR3ErrorCode.SAVE_OFFICE_ERROR, ErrorConstants.SAVE_OFFICE_ERROR_MESSAGE, sqlException);
		} finally {
	 ...
	 * @throws DIR3Exception .
	 */
	public void update(OfficeVO office) throws DIR3Exception {
		...
		try {
			if (office != null) {
				...
				if (addressId == null) {
					if (office.getAddress() != null) {
						...
						addressId = addressDao.saveReturn(office.getAddress());
						LOGGER.debug("Nueva dirección: " + addressId);
						AddressVO address = office.getAddress();
						address.setId(Integer.toString(addressId));
						tempOffice.setAddress(address);
					}
					...
					if (office.getAddress() != null) {
						...
						address.setId(Integer.toString(addressId));
						tempOffice.setAddress(address);
						LOGGER.debug("modificamos dirección: " + addressId);
						...
						LOGGER.debug("modificamos oficina");
						officeDao.update(tempOffice);
						LOGGER.debug("borramos dirección: " + addressId);
				...
				if (office.getContacts() != null && office.getContacts().size() > 0) {
					...
					// TRAE CONTACTOS DIR3
					if (contactsBBDD != null && contactsBBDD.size() > 0) {
						...
						for (ContactOFIVO contactBBDD : contactsBBDD) {
							...
							while (!exists && iterator.hasNext()) {
								...
								exists = compareContact(contactBBDD, contactxml);
							}
							if (!exists) {
						...
						for (ContactOFIVO contact : office.getContacts()) {
							...
							if (!exists) {
								...
								if (!contactbbdd.getStatusId().equals( office.getStatus())) {
					...
					if (contactsBBDD != null && contactsBBDD.size() > 0) {
						...
						if (office.getStatus().equals("E") || office.getStatus().equals("T") || office.getStatus().equals("A")) {
			...
			LOGGER.error(ErrorConstants.UPDATE_OFFICE_ERROR_MESSAGE, sqlException);
			LOGGER.error("CÓDIGO DE OFICINA FALLIDA: " + office.getId());
			throw new DIR3Exception(DIR3ErrorCode.UPDATE_OFFICE_ERROR_MESSAGE, ErrorConstants.UPDATE_OFFICE_ERROR_MESSAGE, sqlException);
		} finally {
			officeDao.endTransaction();
		}
	}
	 ...
	 */
	private boolean compareContact(ContactOFIVO contactbbdd, ContactOFIVO contactxml) {
		boolean result = false;
		if (contactbbdd != null && contactxml != null) {
			if ((contactbbdd.getContactInfo() != null && contactbbdd.getContactInfo().equals(contactxml.getContactInfo()))
					&& (contactbbdd.getContactTypeId() != null && contactbbdd.getContactTypeId().equals( contactxml.getContactTypeId()))) {
				result = true;
			}
		}
		return result;
	 ...
	 * @throws DIR3Exception .
	 */
	public void insertUpdateOffices(List<OfficeVO> officesDCO) throws DIR3Exception {
		...
		while (itr.hasNext()) {
			try {
				...
				LOGGER.error(ErrorConstants.INSERTUPDATE_OFFICE_ERROR_MESSAGE + office.getId(), dir3Exception);
			}
		}
	}
	 ...
	 *            valor del campo a guardar.
	 */
	public void setContactDao(ContactOFIDao contactDao) {
	 ...
	 *            valor del campo a guardar.
	 */
	public void setHistoryOFDao(HistoryDao historyOFDao) {
	 ...
	 * @throws DIR3Exception .
	 */
	public void updateOffices(OfficesVO officesDCO, ContactsOFIVO contactsDCO, ServicesVO servicesDCO, HistoriesVO historiesDCO)
			throws DIR3Exception {
		LOGGER.info("Guardando oficinas, contactos, servicios.");
		if (officesDCO != null && officesDCO.getOffices() != null) {
			List<OfficeVO> listOF = OfficesHelper.completeOffices(officesDCO, contactsDCO, servicesDCO);
			insertUpdateOffices(listOF);
		}
		LOGGER.info("Guardando históricos.");
		/*
		 * if (historiesDCO != null && historiesDCO.getHistories() != null) {
	 ...
	 * @throws DIR3Exception .
	 */
	public void saveHistoriesOffice(HistoriesVO histories) throws DIR3Exception {
		try {
			if (histories != null && histories.getHistories() != null) {
				...
				for (HistoryVO history : histories.getHistories()) {
					try {
						...
						hist.put("lastId", history.getLastId());
						hist.put("previousId", history.getPreviousId());
						...
						if (!exist) {
							...
							if (previousUnit.getDeleteObservations() != null) {
								causeId = historyOFDao.getCauseId(previousUnit.getDeleteObservations());
								if (causeId == null) {
									causeId = "ZZZ";
						...
						LOGGER.error( ErrorConstants.INSERT_HISTORIES_OFFICE_ERROR_MESSAGE + "LastId:" + history.getLastId() + " PreviousId:" + history.getPreviousId() + "; ", exception);
					}
				}
				historyOFDao.commitTransaction();
			}
		} catch (SQLException sqlException) {
			LOGGER.error(ErrorConstants.INSERT_HISTORIES_OFFICE_ERROR_MESSAGE, sqlException);
			throw new DIR3Exception(
					DIR3ErrorCode.INSERT_HISTORIES_OFFICE_ERROR, ErrorConstants.INSERT_HISTORIES_UNIT_ERROR_MESSAGE, sqlException);
		} finally {
