...
import ieci.tecdoc.core.ftp.FtpError;
...
public final class FssMdoFile
{
   ...
   //ftsInfo como null
   public static int storeFileInVolume(int volId, FssFileInfo info, FssFtsInfo ftsInfo, byte[] fileCont) throws Exception {
      ...
      if (volInfo.m_repType != FssMdoUtil.RT_CENTERA) {
   	  ...
   	  checkCanStoreInVol(volInfo.m_repType,volRecA.m_stat, volMaxSize, Double.parseDouble(volRecA.m_actSize), fileSize);
      ...
      if (repInfo.m_type != FssMdoUtil.RT_CENTERA) {
         fileFullPath = repInfo.m_path + sep + volInfo.m_path + sep + fileRecAc.m_loc;
         ...
         if ( ((info.m_flags & FssMdoUtil.FILE_FLAG_FTS) != 0) || repInfo.m_type == FssMdoUtil.RT_DB) {
            ...
            if (repInfo.m_type == FssMdoUtil.RT_DB) {
               ...
               ftsRecAc = fillFtsRecAc(fileRecAc, info, ftsInfo, repRecA.m_type, sep, fileFullPath);
            }
         }
	         ...
	         try {
	            createDirectory(repInfo, dirFullPath);
	        	 ...
	        	 if(e instanceof IeciTdException){
	        		 if(((IeciTdException) e).getErrorCode().equals(FtpError.EC_NATIVE_ERR)){	        	 
      ...
      reflectStoreInTables(repInfo.m_type, volId, actDirRecAc.m_actDir, fileSize, volMaxSize, fileRecAc, ftsRecAc, info.m_ext,fileCont);
   ...
   public static int storeFileInList(int listId, FssFileInfo info, FssFtsInfo ftsInfo, byte[] fileCont)throws Exception {
      ...
      for (i = 0; i < volIds.count(); i++) {
         try {
            ...
            break;
        	 ...
        	 //log.warn(e.getMessage());
        	 if(e instanceof IeciTdException){
        		 if(((IeciTdException) e).getErrorCode().equals(FssError.EC_VOL_IS_FULL) || ((IeciTdException) e).getErrorCode().equals(FssError.EC_VOL_IS_READONLY)){
        			 LOG.info(e.getMessage());
        		 } else {
        			 LOG.error(e.getMessage(), e);
        		 }
      ...
      if (!store) {
         ...
         throw new IeciTdException(FssError.EC_UNABLE_TO_STORE_IN_LIST, FssError.EM_UNABLE_TO_STORE_IN_LIST);
      }
   ...
   private static void checkVolIsReady(int volStat) throws Exception {
      ...
      if ((volStat & FssMdoUtil.VOL_STAT_NOT_READY) != 0) {
         throw new IeciTdException(FssError.EC_VOL_NOT_READY, FssError.EM_VOL_NOT_READY);
      }
   }
   ...
   private static void checkVolIsNotFull(int volStat) throws Exception {
      ...
      if ((volStat & FssMdoUtil.VOL_STAT_FULL) != 0) {
         throw new IeciTdException(FssError.EC_VOL_IS_FULL, FssError.EM_VOL_IS_FULL);
      }
   }
   ...
   private static void checkVolIsNotReadOnly(int volStat) throws Exception {
      ...
      if ((volStat & FssMdoUtil.VOL_STAT_READONLY) != 0) {
         throw new IeciTdException(FssError.EC_VOL_IS_READONLY, FssError.EM_VOL_IS_READONLY);
      }
   }

   private static void checkCanStoreInVol(int repType,int volStat, double maxSize, double actSize, int fileSize) throws Exception {
   ...
   private static void checkRepIsNotOffLine(int repStat) throws Exception {
      ...
      if ((repStat & FssMdoUtil.REP_STAT_OFFLINE) != 0) {
         throw new IeciTdException(FssError.EC_REP_OFF_LINE, FssError.EM_REP_OFF_LINE);
      }
   }
   ...
   private static void checkRepIsNotReadOnly(int repStat) throws Exception {
      ...
      if ((repStat & FssMdoUtil.REP_STAT_READONLY) != 0) {
         throw new IeciTdException(FssError.EC_REP_IS_READONLY, FssError.EM_REP_IS_READONLY);
      }
   }
   ...
   private static byte[] getDBFile(FssRepInfo repInfo, String locId, int volId) throws Exception {
   ...
   private static byte[] readFile(FssRepInfo repInfo, String fileFullPath) throws Exception {
   ...
   private static byte[] readCenteraFile(FssRepInfo repInfo, String fileId,int fileSize) throws Exception {
   ...
   private static byte[] readFtpFile(FssRepInfo repInfo, String fileFullPath) throws Exception {
   ...
   private static int getFileSize(FssRepInfo repInfo, String fileFullPath) throws Exception {
   ...
   private static int getFtpFileSize(FssRepInfo repInfo, String fileFullPath) throws Exception {
   ...
   private static void storeFileDB(FssDaoFtsRecAc ftsRecAc, String locId, byte[] fileCont, int fileSize, String fileExt) throws Exception{
   ...
   private static void writeFile(FssRepInfo repInfo, String fileFullPath, byte[] fileCont) throws Exception {
   ...
   private static String writeCenteraFile(FssRepInfo repInfo,byte[] fileCont) throws Exception {
   ...
   private static void createDirectory(FssRepInfo repInfo, String dirFullPath) throws Exception {
   ...
   private static void createFtpDirectory(FssRepInfo repInfo, String dirFullPath) throws Exception {
   ...
   private static void deleteFile(FssRepInfo repInfo, String fileFullPath,String fileLoc) throws Exception {
   ...
   private static void deleteFtpFile(FssRepInfo repInfo, String fileFullPath) throws Exception {
   ...
   private static void writeFtpFile(FssRepInfo repInfo, String fileFullPath, byte[] fileCont) throws Exception {
   ...
   private static FssDaoFileRecAc fillRecAc(int repType, int volId, String dirPath, char sep, FssFileInfo info, double fileSize) throws Exception {
      ...
      fileId = NextId.generateNextId(FssMdoUtil.NEXT_ID_TBL_NAME, FssMdoUtil.NEXT_ID_TYPE_FILE);
   ...
   private static FssDaoVolRecUc fillVolRecUcToStoreFile(int repType, int fileSize, double volMaxSize, FssDaoVolRecA volRecA) throws Exception {
   ...
   private static String getFstPath(FssFtsInfo ftsInfo, int repType, char fileNameSep, String fileFullPath) throws Exception {
      ...
      } else if ( (ftsInfo.m_ftsPlatform == FssMdoUtil.OS_NT) || (ftsInfo.m_ftsPlatform == FssMdoUtil.OS_WINDOWS)) { 
   ...
   private static FssDaoFtsRecAc fillFtsRecAc(FssDaoFileRecAc fileRecAc, FssFileInfo info, FssFtsInfo ftsInfo, int repType, char fileNameSep, String fileFullPath) throws Exception {
   ...
   private static FssDaoFtsRecAc fillDBFtsRecAc(FssDaoFileRecAc fileRecAc, FssFileInfo     info) throws Exception {
   ...
   private static void reflectStoreInTables(int repType, int volId, int actDir, int fileSize, double volMaxSize, FssDaoFileRecAc fileRecAc, FssDaoFtsRecAc ftsRecAc, String fileExt, byte[] fileCont) throws Exception {
      ...
      try {
         ...
         if (repType != FssMdoUtil.RT_DB) {
            ...
            if ((actDirRecA.m_numFiles >= FssMdoUtil.DIR_MAX_NUM_FILES) && (actDirRecA.m_actDir == actDir)){
   ...
   private static FssDaoVolRecUc fillVolRecUcToDeleteFile(int repType,int fileSize, double volMaxSize, FssDaoVolRecA volRecA) throws Exception {
      ...
      if (repType != FssMdoUtil.RT_CENTERA) {
         if ((newActSize < volMaxSize) &&  ((volRecA.m_stat & FssMdoUtil.VOL_STAT_FULL) != 0)) {
        	 stat = volRecA.m_stat ^ FssMdoUtil.VOL_STAT_FULL;
         } else {
        	 stat = volRecA.m_stat;
         }
   ...
   private static void reflectDeleteInTables(int repType,int fileId, int fileSize, boolean isFts, FssDaoFileRecA fileRecA, double volMaxSize) throws Exception {
   ...
   // PARCHE P9.0.2. Permite recuperar el contenido del documento en un File.
   private static File readCenteraFile(FssRepInfo repInfo, String fileId,int fileSize, File parentFolder) throws Exception {
   ...
   public static int storeFileInList (int listId, FssFileInfo info, FssFtsInfo ftsInfo, File inputFile) throws Exception {
   ...
   //Si el fichero no tiene búsqueda en contenido de documento se pasa
   //ftsInfo como null
   public static int storeFileInVolume(int volId, FssFileInfo info,  FssFtsInfo ftsInfo, File inputFile) throws Exception {
      ...
      //Se pasa fileCont pues si el tipo de repositorio es DB hay que hacerlo
      //dentro de la misma transacción
      reflectStoreInTables(repInfo.m_type, volId, actDirRecAc.m_actDir, fileSize, volMaxSize, fileRecAc, ftsRecAc, info.m_ext,inputFile);
   ...
   // PARCHE P9.0.2. Permite guardar el contenido del documento desde un File.
   private static void reflectStoreInTables (int repType, int volId, int actDir, int fileSize, double volMaxSize, FssDaoFileRecAc fileRecAc, FssDaoFtsRecAc ftsRecAc, String fileExt, File inputFile) throws Exception {
   ...
   private static void storeFileDB(FssDaoFtsRecAc ftsRecAc, String locId, File inputFile, int fileSize, String fileExt) throws Exception {
