...
public class IdiomaUtils {
	...
	private static final Logger logger = Logger.getLogger(IdiomaUtils.class);

	protected static IdiomaUtils _instance = null;
	 ...
	 * @return
	 */
	public synchronized static IdiomaUtils getInstance() {
		if (_instance == null) {
			_instance = new IdiomaUtils();
		}
	...
	/**
	 * Metodo que obtiene el locale java a partir del codigo del idioma
	 * @param numIdioma
	 * @return {@link Locale}
	 */
	public  Locale getLocale(Long numIdioma){
		// Obtenemos el locale java para el codigo de idioma seleccionado por el usuario.
		Locale result= (Locale) microsoftLocalesID2DefaultLocales.get(numIdioma);
		return result;
	}

	public static Object[][] getMicrosoftLocales() {
