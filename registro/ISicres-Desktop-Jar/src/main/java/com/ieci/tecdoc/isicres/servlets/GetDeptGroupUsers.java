...
public class GetDeptGroupUsers extends HttpServlet implements Keys {
	...
	private void doWork(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Identificador de de tipo de usuario. Departamento (1), Grupo (2),
		// Usuario (3).
		Integer userTypeId = RequestUtils.parseRequestParameterAsInteger( request, "UserType", new Integer(0));
		...
		try {
			writer.write(validationListUseCase.getDeptsGroupsUsers(useCaseConf,userTypeId));
			writer.flush();
			writer.close();
			...
			_logger.fatal("Error al recuperar la lista de usuarios", e);
			ResponseUtils.ResponseErrorMsg(writer, RBUtil.getInstance(useCaseConf.getLocale()).getProperty( Keys.I18N_EXCEPTION_IMPOSSIBLETOLOADVALUES));
		}
	}
}