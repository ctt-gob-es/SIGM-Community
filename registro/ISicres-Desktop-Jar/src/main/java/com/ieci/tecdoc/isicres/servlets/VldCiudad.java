...
public class VldCiudad extends HttpServlet implements Keys {

	private static Logger _logger = Logger.getLogger(VldCiudad.class);
	...
	private void doWork(HttpServletRequest request, HttpServletResponse response)
		...
		Integer idProv = RequestUtils.parseRequestParameterAsInteger(request,
				"ProvId", new Integer(0));
		...
		try {
			writer.write(validationListUseCase
					.getVldCiudad(useCaseConf, idProv));
			writer.flush();
			writer.close();
			...
			ResponseUtils.generateJavaScriptLogSessionExpiredProvCityDir(writer, e
					.getMessage());
			...
			ResponseUtils.generateJavaScriptLog(writer, RBUtil.getInstance(
					useCaseConf.getLocale()).getProperty(
					Keys.I18N_ISICRESSRV_ERR_CREATING_CITIES_OBJ));
		}
	}
}