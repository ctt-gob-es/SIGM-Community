...
public class VldTypeDocs extends HttpServlet implements Keys {

	private static final long serialVersionUID = 1L;

	private static Logger _logger = Logger.getLogger(VldTypeDocs.class);
	...
	private void doWork(HttpServletRequest request, HttpServletResponse response)
		...
		Integer personType = RequestUtils.parseRequestParameterAsInteger(
				request, "PersonType", new Integer(0));
		...
		try {
			writer.write(validationListUseCase.getVldTypeDocs(useCaseConf,
					personType));
			writer.flush();
			writer.close();
			...
			ResponseUtils.generateJavaScriptLog(writer, RBUtil.getInstance(
					useCaseConf.getLocale()).getProperty(
					Keys.ISICRESSRV_ERR_CREATING_TYPE_DOCS_OBJ));
		}
	}
}