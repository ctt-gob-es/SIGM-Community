...
public class ConsultDistribution implements Keys {
	...
	public static InfoDistribution consultDistributionInfo(String sessionID, User user, AxSf axsf, ScrDistreg distReg, Idocarchhdr idocarch, Interested[] interesados, String entidad, String comentarios) {
		
		if (shortFormatter == null) {
			shortFormatter = new SimpleDateFormat(RBUtil.getInstance( user.getLocale()).getProperty(I18N_DATE_SHORTFORMAT));
		}		
		String sourceDesc = "";
		String targetDesc = "";

		try {
			sourceDesc = DistributionSession.getOrigDestDescription(sessionID, distReg, true, entidad);
			targetDesc = DistributionSession.getOrigDestDescription(sessionID, distReg, false, entidad);
			
		} catch (Exception e) {
		}

		InfoDistribution distInfo = getDistributionList(distReg, user, sourceDesc, targetDesc, axsf, idocarch, interesados, comentarios);

		return distInfo;
	}

	public static boolean isDistributionInList(List<?> distributionList, Integer bookId, Integer folderId, Integer dtrId) {
		
		if (distributionList != null && !distributionList.isEmpty()) {
			for (Iterator<?> iterator = distributionList.iterator(); iterator.hasNext();) {
				InfoDistribution distInfo = (InfoDistribution) iterator.next();

				if ((distInfo.getBookId().intValue() == bookId.intValue()) && (distInfo.getFolderId().intValue() == folderId.intValue()) && (distInfo.getDtrId().intValue() == dtrId.intValue())) {
					return true;
				}
			}
		}
	...
	private static InfoDistribution getDistributionList(ScrDistreg distReg, User user, String senderName, String destinationName, AxSf axsf, Idocarchhdr idocarch, Interested[] interesados, String comentarios) { 
		...
		wsDistributionInfo.setDistributionDate(shortFormatter.format(distReg.getDistDate()));
		...
		wsDistributionInfo.setDestinationType(new Integer(distReg.getTypeDest()));
		...
		wsDistributionInfo.setStateDescription(RBUtil.getInstance( user.getLocale()).getProperty( I18N_BOOKUSECASE_DISTRIBUTIONHISTORY_MINUTA_DIST_STATE + distReg.getState()));
		wsDistributionInfo.setStateDate(shortFormatter.format(distReg.getStateDate()));

		wsDistributionInfo.setMessage(textRemarks);
		wsDistributionInfo.setUser(user.getUserName());

		wsDistributionInfo.setRegisterNumber(axsf.getAttributeValueAsString("fld1"));
		...
		wsDistributionInfo.setRegisterDestinationName(getAdminUnit(axsf.getFld8()));

		if (axsf instanceof AxSfIn) {
			wsDistributionInfo.setRegisterType(RBUtil.getInstance( user.getLocale()).getProperty( I18N_BOOKUSECASE_NODE_INBOOK_NAME));
			wsDistributionInfo.setRegisterMatterTypeName(getAsuntType(((AxSfIn) axsf).getFld16()));
			wsDistributionInfo.setRegisterMatter(axsf.getAttributeValueAsString("fld17"));
		} else {
			wsDistributionInfo.setRegisterType(RBUtil.getInstance( user.getLocale()).getProperty( I18N_BOOKUSECASE_NODE_OUTBOOK_NAME));
			wsDistributionInfo.setRegisterMatterTypeName(getAsuntType(((AxSfOut) axsf).getFld12()));
			wsDistributionInfo.setRegisterMatter(axsf.getAttributeValueAsString("fld13"));
		}
		
		wsDistributionInfo.setSendersOrReceivers(interesados);
		
		wsDistributionInfo.setComentarios(comentarios);
