 ...
 *
 */
public class ValidationSessionEx extends ValidationSessionUtil implements ServerKeys, HibernateKeys {

	private static final Logger LOG = Logger.getLogger(ValidationSessionEx.class);

	/***************************************************************************
	 * Public methods
	 **************************************************************************/

	public static Object getTopLevelParentScrOrg(String sessionID, int scrID, String language, String entidad) throws AttributesException, SessionException, ValidationException {
		...
		try {
			...
			do {
				result = session.load(EntityByLanguage.getScrOrgLanguage(language), new Integer(scrID));
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to load getTopLevelParentScrOrg for session [" + sessionID + "]", e);
	...
	public static Object getScrOrg(String sessionID, int id, String language, String entidad) throws AttributesException, SessionException, ValidationException {
		...
		try {
			...
			result = session.load(EntityByLanguage.getScrOrgLanguage(language), new Integer(id));
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to load ScrOrg for session [" + sessionID + "]", e);
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_VALIDATIONLISTFIELDS);
	...
	public static Object getScrOrgValidate(String sessionID, int id, String language, String entidad) throws SessionException, ValidationException {
	...
	public static Object getScrTypeAdm(String sessionID, int id, String language, String entidad) throws AttributesException, SessionException, ValidationException {
		...
		try {
			...
			result = session.load( EntityByLanguage.getScrTypeAdmLanguage(language), new Integer(id));
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to load ScrTypeAdm for session [" + sessionID + "]", e);
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_VALIDATIONLISTFIELDS);
	...
	public static String getOneScrtt(String sessionID, String valueTransport, String entidad) throws AttributesException, SessionException, ValidationException {
		...
		try {
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to load ScrTt for session [" + sessionID + "]", e);
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_VALIDATIONLISTFIELDS);
	...
	public static String getScrCities(String sessionID, Integer provId, String entidad) throws AttributesException, SessionException, ValidationException {
		
		String result = null;
		
		try {
			String xmlParamId = XMLPersons.createXMLParamIdInfo( provId.toString(), sessionID, entidad);

			result = PersonValidationFactory.getCurrentPersonValidation().getCities(xmlParamId);
	...
	public static String getDir(String sessionID, Integer personID, int maxResults, String entidad, int typeAddress) throws AttributesException, SessionException, ValidationException {
		
		return getDirTemp(sessionID, personID, maxResults, false, entidad, typeAddress);
	}

	public static String getScrProv(String sessionID, String entidad) throws AttributesException, SessionException, ValidationException {
		
		String result = null;
		
		try {
			String xmlParamId = XMLPersons.createXMLParamIdInfo(null, sessionID, entidad);

			result = PersonValidationFactory.getCurrentPersonValidation().getProvicies(xmlParamId);
	...
	public static String getTypeDocs(String sessionID, Integer personType, String entidad) throws SessionException, AttributesException {
		...
		try {
			...
			String xmlParamId = XMLPersons.createXMLParamIdInfo(personTypeStr, sessionID, entidad);

			result = PersonValidationFactory.getCurrentPersonValidation().getDocsType(xmlParamId);
			...
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_TYPE_DOCS);
		}
	}

	public static String getTypeAddresses(String sessionID, String entidad) throws SessionException, AttributesException {
		...
		try {
			...
			result = PersonValidationFactory.getCurrentPersonValidation().getAddressesType(xmlParamId);
			...
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_TYPE_ADDRESSES);
		}
	}

	/**
	 * Metodo que se usa para hacer cambio de oficina al usuario actual
	 * conectado. Desbloquea todos los registros bloqueados por el usuario
	 * actual Actualiza el usuario Actualiza la oficina Actualiza la zona
	 * horaria de la oficina
	 *
	 * @param sessionID
	 * @param code
	 * @param entidad
	 ...
	 */
	public static void validateOfficeCode(String sessionID, String code, String entidad) throws SessionException, ValidationException {
		...
		try {
			...
			// desbloquea registros del usuario actual, para ello obtenemos
			// todos los libros que tiene abiertos y borramos los bloqueos de
			...
			for (Iterator it = cacheBag.keySet().iterator(); it.hasNext();) {
				...
				if (o instanceof Integer) {
					...
					query.append(" scr WHERE scr.archid=? AND scr.userid=? and scr.stat=1");
					int result = session.delete(query.toString(), new Object[] { bookID, user.getId() }, new Type[] { Hibernate.INTEGER, Hibernate.INTEGER });
			...
			// elimina de la cachebah los libros abiertos por el usuario y
			// oficina anterior
			...
			DBEntityDAOFactory.getCurrentDBEntityDAO().deleteIdsGenerationTable(user.getId(), entidad);
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to validate office code: " + code + " for session [" + sessionID + "]", e);
			throw new ValidationException( ValidationException.ERROR_ATTRIBUTE_NOT_FOUND);
	 ...
	 * @throws ValidationException
	 */
	public static List getDeptsGroupsUsers(String sessionID, Integer userTypeId, String entidad) throws SessionException, ValidationException {
		...
		try {
			...
			if (userTypeId.intValue() == 1) {
				...
				if (scrOfic != null) {
					deptsGroupsUserList = ISicresQueries.getDepts(session, new Integer(scrOfic.getDeptid()));					
				} else {
					deptsGroupsUserList = ISicresQueries.getDepts(session, null);
				}
	...
	public static List getGroupsUsersLDAP(String sessionID, Integer userTypeId, String entidad) throws SessionException, ValidationException {
		...
		try {
			...
			if (userTypeId.intValue() == 1) {
				...
				ldapGroupsUsersList = ISicresQueries.getLdapUsers(session, user.getId());
				...
				if (userDepts != null && userDepts.size() > 0) {
					...
					for (Iterator it = userDepts.listIterator(); it.hasNext();) {
						Iuserdepthdr deptHdr = ISicresQueries.getUserDeptHdrByDeptId(session, new Integer(it.next().toString()));
						userLdapGroups.add(new Integer(deptHdr.getCrtrid()));
					}
				}
				
				ldapGroupsUsersList = ISicresQueries.getLdapGroups(session, userLdapGroups);
			}
	...
	public static List getOtherOffices(String sessionID, Locale locale, String entidad) throws AttributesException, SessionException, ValidationException {
		...
		try {
			...
			Criteria criteriaResults = session.createCriteria(EntityByLanguage.getScrOficLanguage(locale.getLanguage()));
			...
			Criteria criteriaResults1 = session.createCriteria(EntityByLanguage.getScrOficLanguage(locale.getLanguage()));
			...
			HibernateUtil.rollbackTransaction(tran);
			LOG.error("Impossible to load other offices for user for session [" + sessionID + "]", e);
			throw new AttributesException( AttributesException.ERROR_CANNOT_FIND_OTHER_OFFICE_USER);
	...
	public static List getOtherOfficesLDAP(String sessionID, String entidad) throws ValidationException, SessionException, AttributesException {
		...
		try {
			...
			for (Iterator it = allOfficesUser.listIterator(); it.hasNext();) {
				ScrOfic tmpOfic = ISicresQueries.getScrOficByDeptId(session, new Integer(it.next().toString()));
