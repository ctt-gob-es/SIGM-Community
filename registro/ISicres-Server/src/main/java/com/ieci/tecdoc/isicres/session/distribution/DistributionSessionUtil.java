...
public class DistributionSessionUtil extends UtilsSession {
    ...
    protected static void lockListFolderDist(
	Session session, Integer bookId, List listIdsRegister, AuthenticationUser user,
	String entidad)
	...
	for (Iterator it = listIdsRegister.iterator(); it.hasNext();) {
	    ...
	    if (!lockFolderDist(
		session, user, bookId, folderID.intValue(), entidad)) {
     ...
     * @throws Exception
     */
    protected static boolean lockFolderDist(
	Session session, AuthenticationUser user, Integer bookID, int fdrid, String entidad)
	throws HibernateException, Exception {

	boolean result = false;
	List list = ISicresQueries.getIdocfdrstat(
	    session, bookID, fdrid);
	Timestamp currentDate = DBEntityDAOFactory.getCurrentDBEntityDAO().getDBServerDate(
	    entidad);
	BBDDUtils bbddUtils = new BBDDUtils();
	if (log.isDebugEnabled())
	    log.info("Libro [" +
		bookID + "] Carpeta [" + fdrid + "] Usuario [" + user.getId() + "]");
	...
	if (list != null &&
	    ...
	    ISicresSaveQueries.saveIDocFdrStat(
		session, bookID, fdrid, user, bbddUtils.getDate(currentDate));
	    result = true;
    ...
    protected static void lockScrDistReg(
	List ids, String entidad)
    ...
    protected static List lockFolderDistById(
	Session session, AuthenticationUser user, List ids, Map distIds, List archidFdr,
	boolean isLockScrDistReg, String entidad)
	throws DistributionException, HibernateException, SQLException, Exception {
	...
	for (Iterator it = treemap.keySet().iterator(); it.hasNext();) {
		...
		if (!lockFolderDist(
		    session, user, new Integer(
			distReg.getIdArch()), distReg.getIdFdr(), entidad)) {
    ...
    protected static List getUserGroups(
	Session session, AuthenticationUser user, boolean isLdap)
	throws HibernateException {
	...
	AuthenticationUser user, List idArchs) {
	String tableName = null;

	if (idArchs != null &&
	    !idArchs.isEmpty()) {
	    tableName = " SCR_TMPREG" +
		user.getId() + " ";
	}
    ...
    protected static StringBuffer getDistributionQuerySize(
	Session session, AuthenticationUser user, List iUserGroupUser, int typeDist,
	...
	case DISTRIBUCION_IN_DIST: {
		...
		user, iUserGroupUser, bookList, oficAsoc));
    ...
    protected static String getDistributionFinalWhere(
	List idArchs, String regWhere, String querySize, String tableName, String entidad)
	...
	if (idArchs != null &&
		...
		boolean isInBook = Repository.getInstance(
		    entidad).isInBook(
		    auxBookId).booleanValue();
		String processRegWhere = getRegWhere(
		    regWhere, isInBook);
		createSentence.append(DBEntityDAOFactory.getCurrentDBEntityDAO()
		    .getTemporalTableDistributionQuerySentence(
			tableName, auxBookId, processRegWhere, index));
		index++;
	    }
    ...
    protected static int getDistributionCount(
	String finalWhere, String entidad)
	throws SQLException, Exception {
	StringBuffer selectCount = new StringBuffer();
	selectCount.append("select count(*) from scr_distreg where ");
	selectCount.append(finalWhere.substring(
	    0, finalWhere.indexOf("order")));
	int distributionCount = DBEntityDAOFactory.getCurrentDBEntityDAO().getDistributionSize(
	    selectCount.toString(), entidad);

	return distributionCount;
    }

    protected static DistributionResults getDistributionResults(
	Session session, List list, int distributionCount, Date currentDate, int typeDist,
	...
	if (distributionResults.getTotalSize() > 0) {
		...
		distType.put(
		    distReg.getId(), getDistributionResultDistType(
			session, distReg, entidad));
		String id = getDistributionResultId(
		    distReg, typeDist, entidad, isLdap);
		...
		if (!id.equals("")) {
		    ...
		    Integer auxID = new Integer(
			distReg.getIdArch());
		    if (!distributionResults.getIdocarchhdr().containsKey(
			auxID)) {
			...
			distributionResults.getIdocarchhdr().put(
			    auxID,
			    session.load(
				EntityByLanguage.getIdocarchhdrLanguage(locale.getLanguage()),
				auxID));
		    }
		}
		else {
     ...
     */
    protected static int getSizeFolderDistByStateByDeptId(
	String sessionID, String entidad, int state, boolean isLdap)
	...
	try {
	    ...
	    List iUserGroupUser = getUserGroups(
		session, user, isLdap);

	    Date dateUserLastConnection = (Date) cacheBag.get(LAST_CONNECTION_USER);

	    String querySize = getFolderDistQuerySizeByStateAndByDeptId(
		user, iUserGroupUser, dateUserLastConnection, state);

	    size = ((Integer) session.iterate(
		querySize).next()).intValue();
	    ...
	    hibernateUtil.rollbackTransaction(tran);
	    log.error(
		"Impossible to obtain the distribution for the session [" +
		    sessionID + "]", e);
	    throw new DistributionException(
    ...
    protected static int getSizeFolderDistByState(
	String sessionID, String entidad, int state, boolean isLdap)
	throws DistributionException, SessionException, ValidationException {
	Validator.validate_String_NotNull_LengthMayorZero(
	    sessionID, ValidationException.ATTRIBUTE_SESSION);
	...
	try {
	    ...
	    String querySize = getFolderDistQuerySizeByState(
		user, iUserGroupUser, dateUserLastConnection, state);
    ...
    protected static void unlockDistRegs(
	Session session, Transaction tran, AuthenticationUser user, List archidFdr)
	throws HibernateException, DistributionException {
	if (archidFdr != null &&
	    !archidFdr.isEmpty()) {
		...
		unlock(
		    session, new Integer(
			distReg.getIdArch()), distReg.getIdFdr(), user);
	    }
	}
	HibernateUtil hibernateUtil = new HibernateUtil();
	hibernateUtil.commitTransaction(tran);
    ...
    protected static void unlockDistRegsById(
	String sessionID, List list, String entidad)
	...
	try {
		...
		unlock(
		    session, new Integer(
			scrDistReg.getIdArch()), scrDistReg.getIdFdr(), user);
	    }
	    ...
	    hibernateUtil.rollbackTransaction(tran);
	    log.error(
		"Impossible to update the distribution for the session [" +
		    sessionID + "].", e);
	    throw new BookException(
    ...
    protected static ScrOrg getScrOrg(
	Session session, Iuserusertype userusertype, ScrOfic scrOfic, String code, String entidad)
	throws BookException, HibernateException, SQLException, Exception {
	...
	try {
	    scrOrg = ISUnitsValidator.getUnit(
		session, code, true, privOrgs);
	}
    ...
    protected static void closeDistRegs(
	String sessionID, List list, int error, String entidad)
	throws BookException, SessionException, ValidationException {
	if (list != null &&
	    !list.isEmpty() && error == 1) {
	    for (Iterator it4 = list.iterator(); it4.hasNext();) {
		ScrDistreg distReg = (ScrDistreg) it4.next();
		FolderSession.closeFolderFromDistribution(
    ...
    protected static ScrDistreg updateDistReg(
	Session session, AuthenticationUser user, String sessionID, String event,
	ScrDistreg scrDistReg, int distState, Integer ids, String remarks, String entidad)
	throws DistributionException, SessionException, ValidationException, HibernateException,
	SQLException, Exception {
	if (event != null) {
		...
		if (DISTRIBUTION_ACCEPT_EVENT.equals(event)) {
		    EventsSession.distAceptEvent(
			sessionID, event, scrDistReg, entidad);
		}
		else if (DISTRIBUTION_ARCHIVE_EVENT.equals(event)) {
		    EventsSession.distArchiveEvent(
			sessionID, event, scrDistReg, entidad);
		}
		else if (DISTRIBUTION_REJECT_EVENT.equals(event)) {
		    EventsSession.distRejectEvent(
			sessionID, event, scrDistReg, entidad);
		}
		else if (DISTRIBUTION_REDISTRIBUTE_EVENT.equals(event)) {
		    EventsSession.distDestChangeEvent(
			sessionID, event, scrDistReg, entidad);
		}
		else {
	...
	Date currentDate = new Date(
	    DBEntityDAOFactory.getCurrentDBEntityDAO().getDBServerDate(
		entidad).getTime());
	...
	iSDist.setDistState(
	    session, ids.intValue(), distState, currentDate, user.getName(), user.getId(), entidad,
	    isDBCaseSensitive);

	return scrDistReg;
    }

    protected static void updateAcceptDistribution(
	String sessionID, List ids, List archidFdr, String entidad)
	...
	try {
		...
		if (scrDistReg.getState() != ISDistribution.STATE_PENDIENTE) {
		    ...
		    scrDistReg =
			updateDistReg(
			    session, user, sessionID, DISTRIBUTION_ACCEPT_EVENT, scrDistReg,
			    ISDistribution.STATE_ACEPTADO, ids1, null, entidad);

		    updateInsertDistAccept(
			scrOfic, scrDistReg, currentDate, 0, user.getName(), entidad);
	    ...
	    hibernateUtil.rollbackTransaction(tran);
	    log.error(
		"Impossible to accept the distribution for the session [" +
		    sessionID + "]", e);
	    throw new DistributionException(
	    ...
	    hibernateUtil.closeSession(entidad);
	    closeDistRegs(
		sessionID, archidFdr, error, entidad);
	}
    }
     ...
     * @throws ValidationException
     */
    protected static void updateChangeDistribution(
	String sessionID, List scrs, List distList, int typeDist, Integer id,
	Integer canDestWithoutList, String entidad)
	throws DistributionException, BookException, SessionException, ValidationException {
	Transaction tran = null;
	HibernateUtil hibernateUtil = new HibernateUtil();
	int error = 0;
	try {
		...
		scrDistReg =
		    updateDistRegByTypeFromChangeDistribution(
			session, user, sessionID, scrDistReg, typeDist, distList,
		...
		updateInsertDistAccept(
		    scrOfic, scrDistReg, currentDate, state, user.getName(), entidad);
	    }
	    hibernateUtil.commitTransaction(tran);
	}
	    ...
	    hibernateUtil.closeSession(entidad);
	    closeDistRegs(
		sessionID, scrs, error, entidad);
	}
    }

    protected static void updateFolderChangeDistribution(
	String sessionID, List scrs, ScrOrg scrOrg, Integer launchDistOutRegister, Locale locale,
	String entidad)
	throws BookException, SessionException, ValidationException {
	int error = 0;
	try {
		...
		AxSf axsf = new AxSf();
		axsf.setAttributeValue(
		    "fld8", scrOrg.getId());
		FolderSession.updateFolderEx(
		    sessionID, new Integer(
			scrDistReg.getIdArch()), scrDistReg.getIdFdr(), axsf, null, null, false,
     ...
     * @throws HibernateException
     */
    protected static Map getOutputInputBookType(
	Session session, String entidad, List ids, Map distIds, Map outputInputBookType,
	...
	for (Iterator it = treemap.keySet().iterator(); it.hasNext();) {
	    ...
	    String id = (String) aux.keySet().iterator().next();
	    ScrDistreg distReg = (ScrDistreg) aux.get(id);
	    boolean isOutBook = Repository.getInstance(
		entidad).isOutBook(
		new Integer(
		    distReg.getIdArch())).booleanValue();
	    if (ids.contains(distReg.getId()) &&
		isOutBook) {
		outputInputBookType.put(
		    distReg.getId(), new Boolean(
			isOutBook));
	    }
	    else if (ids.contains(distReg.getId()) &&
		!isOutBook) {
		...
		if (distReg.getTypeDest() == 2) {
			...
			(!scrOficIdOrig.getId().equals(scrOficIdDest.getId()) )) {
			outputInputBookType.put(
			    distReg.getId(), new Boolean(
				isOutBook));

			log.debug("La distribucion [" +
			    distReg.getId() + "] generará un nuevo registro para la oficina [" +
			    distReg.getIdDest() + "]");
		    }
		    ...
		    if ((scrOficIdDest == null) ||
			(scrOficIdOrig.getScrOrg().getId().intValue() != scrOficIdDest.getScrOrg()
			    .getId().intValue())) {
			outputInputBookType.put(
			    distReg.getId(), new Boolean(
				isOutBook));
			log.debug("La distribucion [" +
			    distReg.getId() + "] generará un nuevo registro para el usuario [" +
			    distReg.getIdDest() + "]");
		    }
		}
	    }
    ...
    protected static Map generateNewRegsId(
	Session session, Integer idBook, Map outputInputBookType, List ids, Integer userId,
	String entidad)
	throws SQLException, Exception {
	...
	if (!outputInputBookType.isEmpty()) {
		...
		if (outputInputBookType.containsKey(id)) {
		    int newRegisterID =
			DBEntityDAOFactory.getCurrentDBEntityDAO().getNextIdForRegister(
			    idBook, entidad);
		    newRegisterIdOutInBook.put(
			id, new Integer(
			    newRegisterID));
		    DBEntityDAOFactory.getCurrentDBEntityDAO().lastRegister(
			new Integer(
			    newRegisterID), userId, idBook, entidad);
		}
		else {
		    ScrDistreg scrDistReg = (ScrDistreg) session.load(
			ScrDistreg.class, id);
		    DBEntityDAOFactory.getCurrentDBEntityDAO().lastRegister(
			new Integer(
			    scrDistReg.getIdFdr()), userId, new Integer(
			    scrDistReg.getIdArch()), entidad);
		}
	    }
	}
	...
	String sessionID, Integer idBook, Integer launchDistOutRegister, Locale locale,
	...
	try {
		...
		MiscFormat miscFormat = getMiscFormat(
		    session, idBook, outputInputBookType);

		BookTypeConf bookTypeConf = bookTypeConf(
		    1, entidad);
		BookConf bookConf = bookConf(
		    idBook, entidad);

		Timestamp timestamp = DBEntityDAOFactory.getCurrentDBEntityDAO().getDBServerDate(
		    entidad);
		...
		for (Iterator it = scrDistRegIds.keySet().iterator(); it.hasNext();) {
			...
			if (scrOfic != null) {
				...
				sessionID, idBook, newInputRegister, interList, registerId,
			...
			bookTypeConf = completeBookTypeConf(
			 ...
			 createNewRegisterDocs(
			    sessionID, user, idBook, scrDistReg, registerId, timestamp, miscFormat,
    ...
    protected static void createRegsAsoc(
	String sessionID, Map newRegisterIdOutInBook, List ids, List archidFdr, Integer idBook,
	String entidad)
	throws BookException, SessionException, ValidationException, DistributionException {
	Transaction tran = null;
	int error = 0;
	...
	try {
		...
		for (Iterator it = ids.iterator(); it.hasNext();) {
		    ...
		    if (newRegisterIdOutInBook.containsKey(ids1)) {
			int assocId =
			    DBEntityDAOFactory.getCurrentDBEntityDAO().getNextIdForScrRegAsoc(
				user.getId(), entidad);

			int idSec = ((Integer) newRegisterIdOutInBook.get(ids1)).intValue();

			ISicresSaveQueries.saveScrRegAsoc(
			    session, assocId, scrDistReg.getIdArch(), idBook.intValue(),
     ...
     * @throws Exception
     */
    private static Integer insertDistribute(
	Session session, Integer bookID, int fdrid, int typeDest, int idDest,
	Timestamp currentDate, int deptId, String userName, Integer userId, int distributionType,
	...
	BBDDUtils bbddUtils = new BBDDUtils();
	List distReg = ISicresQueries.getScrDistreg(
	    session, bookID, fdrid, typeDest, idDest);
	...
	if (distribute) {
	    distributionID = DBEntityDAOFactory.getCurrentDBEntityDAO().getNextIdForScrDistReg(
		userId, entidad);

	    Date dateState = bbddUtils.getDateFromTimestamp(currentDate);

	    ISicresSaveQueries.saveScrDistreg(
		session, distributionID, bookID, fdrid, dateState, 2, deptId, typeDest, idDest, 1,
		dateState, messageForUser);

	    ISDistribution isDist = new ISDistribution();
	    isDist.setDistState(
		session, distributionID, ISDistribution.STATE_PENDIENTE, dateState, userName,
		userId, entidad, isDataBaseCaseSensitive(entidad));
	    isDist.changeStateAcceptRedis(
		session, bookID, fdrid, deptId, userName, userId, distributionType, currentDate,
		entidad, isDataBaseCaseSensitive(entidad));
	}
	return new Integer(
	    distributionID);
    }
	...
	Session session, Integer bookID, int fdrid, int typeDest, int idDest,
	Timestamp currentDate, int typeOrig, int idOrig, String userName, Integer userId,
	...
	if (distribute) {
	    ...
	    ISicresSaveQueries.saveScrDistreg(
		session, distributionID, bookID, fdrid, dateState, typeOrig, idOrig, typeDest,
		idDest, 1, dateState, messageForUser);
		...
		userId, entidad, isDataBaseCaseSensitive(entidad));
	    isDist.changeStateAcceptRedis(
		session, bookID, fdrid, idOrig, userName, userId, distributionType, currentDate,
		entidad, isDataBaseCaseSensitive(entidad));
	}
	return new Integer(
	    distributionID);
	...
	Session session, String sessionID, Locale locale, ScrDistreg scrDistReg,
	AuthenticationUser user, Integer canDestWithoutList, int typeDist, ScrOfic scrOfic,
	Date currentDate, Integer userType, List distList, String messageForUser, String entidad)
	throws DistributionException, SessionException, ValidationException, HibernateException,
	Exception {
	...
	scrDistReg =
	    updateDistRegByTypeFromChangeDistribution(
		session, user, sessionID, scrDistReg, typeDist, distList, canDestWithoutList,
	...
	for (Iterator it = distList.iterator(); it.hasNext();) {
	    ...
	    createDistribution(
		session, sessionID, scrDistReg.getIdArch(), 2, scrOfic.getDeptid(), userId,
		userType, messageForUser, disIdFolder, user, scrOfic, locale, entidad);
	}

    }

    protected static Map getScrDistRegIds(
	Session session, List ids)
	throws HibernateException {
    ...
    protected static Map getRegStates(
	Session session, List ids, Map regStates)
	throws HibernateException {
	for (Iterator it = ids.iterator(); it.hasNext();) {
	    Integer ids1 = (Integer) it.next();
	    ScrDistreg scrDistReg = (ScrDistreg) session.load(
		ScrDistreg.class, ids1);
	    if (!regStates.containsKey(new Integer(
		scrDistReg.getIdArch()))) {
		ScrRegstate regState = ISicresQueries.getScrRegstate(
		    session, new Integer(
			scrDistReg.getIdArch()));
		regStates.put(
		    new Integer(
			scrDistReg.getIdArch()), new Integer(
	...
	AuthenticationUser user, List iUserGroupUser, Date dateUserLastConnection, int state)
	throws Exception {
	...
	switch (state) {
	case ISDistribution.STATE_PENDIENTE:
	    querySize.append(getFolderDistQuerySizeByStatePendiente(
		user, iUserGroupUser));
	    break;
	case ISDistribution.STATE_RECHAZADO:
	    querySize.append(getFolderDistQuerySizeByStateRechazado(
		user, iUserGroupUser));
	    break;
	default:
	    break;
     ...
     * @throws Exception
     */
    protected static void autoArchiving(
	Session session, String querySize, int autoArchiv, AuthenticationUser user, String entidad)
	throws Exception {
	ScrDistreg scrDistReg = null;
	String query = "state = 2 and " +
	    querySize;
	Date currentDate = new Date(
	    DBEntityDAOFactory.getCurrentDBEntityDAO().getDBServerDate(
		entidad).getTime());
	...
	if (list != null &&
		...
		Date stateDate = new Date(
		    scrDistReg.getStateDate().getTime());
		Calendar gc = Calendar.getInstance();
		gc.setTime(stateDate);
		gc.add(
		    Calendar.HOUR, autoArchiv);
		...
		if (timeOutDate.before(currentDate)) {
		    ...
		    isDist.setDistState(
			session, scrDistReg.getId().intValue(), ISDistribution.STATE_ARCHIVADO,
			currentDate, user.getName(), user.getId(), entidad,
     ...
     * @return
     */
    protected static String getInDistQuery(
	AuthenticationUser user, List iUserGroupUser, List bookList, boolean oficAsoc) {
     ...
     * @return
     */
    protected static String getOutDistQuery(
	AuthenticationUser user, List iUserGroupUser, List bookList, boolean oficAsoc) {
	StringBuffer query = new StringBuffer();
    ...
    private static String getDistWhere(
	String distWhere, int typeDist)
	throws HibernateException {
	...
	if (pos >= 0) {
	    auxDistWhere = auxDistWhere.substring(
		pos, auxDistWhere.length());
	    pos1 = auxDistWhere.indexOf("and");
	    if (pos1 > 0) {
		cadenaDestOrg = auxDistWhere.substring(
		    0, pos1);
	    }
	    else {
		cadenaDestOrg = auxDistWhere.substring(
		    0, auxDistWhere.length());
	    }
	    auxDistWhere = cadenaDestOrg;
	    auxDistWhere = auxDistWhere.substring(
		5, auxDistWhere.length()).trim();
	    boolean digit = Character.isDigit(auxDistWhere.charAt(1));
	    if (digit) {
		distWhereOperator = auxDistWhere.substring(
		    0, 1);
		auxDistWhere = auxDistWhere.substring(
		    1, auxDistWhere.length());
	    }
	    else {
		distWhereOperator = auxDistWhere.substring(
		    0, 2);
		auxDistWhere = auxDistWhere.substring(
		    2, auxDistWhere.length());
	    }
	}
	else {
	    auxDistWhere = "";
	...
	return getDistWhere(
	    auxDistWhere, distWhere, typeDist, distWhereOperator, cadenaDestOrg);
    }

    private static String getDistWhere(
	String auxDistWhere, String distWhere, int typeDist, String distWhereOperator,
	String cadenaDestOrg) {
	String result = "";
	if (auxDistWhere.length() > 0) {
		...
		if (destType != null &&
		    ...
		    auxDistWhere =
			"(TYPE_DEST =" +
			    destType + " AND ID_DEST" + distWhereOperator + idDestOrig.toString() +
			    ") ";
		    result = distWhere.replaceAll(
		...
		if (destType != null &&
		    ...
		    auxDistWhere =
			"(TYPE_ORIG =" +
			    destType + " AND ID_ORIG" + distWhereOperator + idDestOrig.toString() +
			    ") ";
		    result = distWhere.replaceAll(
			cadenaDestOrg, auxDistWhere);
		}
    ...
    private static String getRegWhere(
	String regWhere, boolean typeBook)
	throws HibernateException {
	String result = "";
	String aux = regWhere;
    ...
    private static String getIUserDeptHdrInfo(
	String id, String entidad)
	throws HibernateException {
	...
	try {
		...
		Iuserdepthdr.class, new Integer(
		    id));
    ...
    private static String getIUserGroupHdrInfo(
	String id, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
		...
		Iusergrouphdr.class, new Integer(
		    id));
    ...
    private static String getIUserUserHdrInfo(
	String id, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
		...
		Iuseruserhdr.class, new Integer(
		    id));
    ...
    private static String getScrOrgInfo(
	String id, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
	    Session session = hibernateUtil.currentSession(entidad);
	    ScrOrg iuser = (ScrOrg) session.load(
		ScrOrg.class, new Integer(
		    id));
    ...
    private static String getScrUsrIdentInfo(
	String userId, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
		...
		ScrUsrIdent.class, new Integer(
		    userId));

	    if (scrUsrIdent != null) {
		result = scrUsrIdent.getFirstName() +
		    " " + scrUsrIdent.getSecondName() + "," + scrUsrIdent.getSurName();
	    }
	}
	catch (ObjectNotFoundException e) {
	    result = "";
	...
	Session session, AuthenticationUser user, String sessionID, ScrDistreg scrDistReg,
	int typeDist, List distList, Integer canDestWithoutList, Integer id, String entidad)
	throws DistributionException, SessionException, ValidationException, HibernateException,
	SQLException, Exception {
	if (typeDist == 1) {
		...
		scrDistReg =
		    updateDistReg(
			session, user, sessionID, DISTRIBUTION_REDISTRIBUTE_EVENT, scrDistReg,
	    ...
	    if ((scrDistReg.getState() == ISDistribution.STATE_RECHAZADO && !distList.isEmpty()) ||
		(scrDistReg.getState() == ISDistribution.STATE_RECHAZADO &&
		    distList.isEmpty() && canDestWithoutList.intValue() != 0)) {
		scrDistReg =
		    updateDistReg(
			session, user, sessionID, null, scrDistReg,
			ISDistribution.STATE_REDISTRIBUIDO, id, null, entidad);
	    }
    ...
    public static AxSf getNewInputRegister(
	Session session, AxSf axsf, String entidad)
	throws NumberFormatException, HibernateException {
	AxSf newInputRegister = new AxSfIn();
	if (axsf instanceof AxSfOut) {
	    setAxSfAttributesFromAxSfOut(
		axsf, newInputRegister);
	}
	else {
	    setAxSfAttributesFromAxSfIn(
		session, axsf, newInputRegister, entidad);
	}

	return newInputRegister;
    }

    private static AxSf setAxSfAttributesFromAxSfIn(
	Session session, AxSf axsf, AxSf newInputRegister, String entidad)
	throws NumberFormatException, HibernateException {
	HibernateUtil hibernateUtil = new HibernateUtil();
	session = hibernateUtil.currentSession(entidad);
	if (axsf.getAttributeValue("fld8") != null) {
	    newInputRegister.setAttributeValue(
		"fld8", new Integer(
		    axsf.getAttributeValueAsString("fld8")));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld8", axsf.getAttributeValue("fld8"));
	}
	if (axsf.getAttributeValue("fld7") != null) {
	    newInputRegister.setAttributeValue(
		"fld7", new Integer(
		    axsf.getAttributeValueAsString("fld7")));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld7", axsf.getAttributeValue("fld7"));
	}
	newInputRegister.setAttributeValue(
	    "fld9", axsf.getAttributeValue("fld9"));
	if (axsf.getAttributeValue("fld13") != null) {
	    newInputRegister.setAttributeValue(
		"fld13", axsf.getAttributeValue("fld13"));
	}
	else {
	    ScrOfic scrOficOrigin = ISicresQueries.getScrOficById(
		session, new Integer(
		    axsf.getAttributeValueAsString("fld5")));
	    newInputRegister.setAttributeValue(
		"fld13", scrOficOrigin.getScrOrg().getId());
	}
	newInputRegister.setAttributeValue(
	    "fld14", axsf.getAttributeValue("fld14"));
	newInputRegister.setAttributeValue(
	    "fld15", axsf.getAttributeValue("fld15"));
	if (axsf.getAttributeValue("fld16") != null) {
	    newInputRegister.setAttributeValue(
		"fld16", new Integer(
		    axsf.getAttributeValueAsString("fld16")));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld16", axsf.getAttributeValue("fld16"));
	}
	newInputRegister.setAttributeValue(
	    "fld17", axsf.getAttributeValue("fld17"));
	if (axsf.getAttributeValue("fld10") != null) {
	    newInputRegister.setAttributeValue(
		"fld10", axsf.getAttributeValue("fld10"));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld10", axsf.getAttributeValue("fld1"));
	}
	if (axsf.getAttributeValue("fld11") != null) {
	    newInputRegister.setAttributeValue(
		"fld11", axsf.getAttributeValue("fld11"));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld11", new Integer(
	...
	if (axsf.getAttributeValue("fld12") != null) {
	    newInputRegister.setAttributeValue(
		"fld12", axsf.getAttributeValue("fld12"));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld12", axsf.getAttributeValue("fld2"));
	}

	// Referencia de Expediente: FLD19
	newInputRegister.setAttributeValue(
	    "fld19", axsf.getAttributeValue("fld19"));
    ...
    private static AxSf setAxSfAttributesFromAxSfOut(
	AxSf axsf, AxSf newInputRegister) {
	if (axsf.getAttributeValue("fld8") != null) {
	    newInputRegister.setAttributeValue(
		"fld8", new Integer(
		    axsf.getAttributeValueAsString("fld8")));
	...
	newInputRegister.setAttributeValue(
	    "fld12", axsf.getAttributeValue("fld2"));
	newInputRegister.setAttributeValue(
	    "fld14", axsf.getAttributeValue("fld10"));
	newInputRegister.setAttributeValue(
	    "fld15", axsf.getAttributeValue("fld11"));
	if (axsf.getAttributeValue("fld12") != null) {
	    newInputRegister.setAttributeValue(
		"fld16", new Integer(
		    axsf.getAttributeValueAsString("fld12")));
	}
	else {
	    newInputRegister.setAttributeValue(
		"fld16", axsf.getAttributeValue("fld12"));
	}
	newInputRegister.setAttributeValue(
	    "fld17", axsf.getAttributeValue("fld13"));
    ...
    private static void updateInsertDistAccept(
	ScrOfic scrOfic, ScrDistreg scrDistReg, Date currentDate, int state, String userName,
	String entidad)
	throws Exception {
	boolean exist = DBEntityDAOFactory.getCurrentDBEntityDAO().getDistAcceptExist(
	    new Integer(
		scrDistReg.getIdArch()), scrDistReg.getIdFdr(), entidad);
	if (scrOfic != null) {
	    DBEntityDAOFactory.getCurrentDBEntityDAO().updateInsertDistAccept(
		exist, new Integer(
		    scrDistReg.getIdArch()), scrDistReg.getIdFdr(), scrOfic.getId().intValue(),
		state, userName, currentDate, scrDistReg.getDistDate(), entidad);
	}
	else {
	    DBEntityDAOFactory.getCurrentDBEntityDAO().updateInsertDistAccept(
		exist, new Integer(
		    scrDistReg.getIdArch()), scrDistReg.getIdFdr(), -1, state, userName,
		currentDate, scrDistReg.getDistDate(), entidad);
	}
    }

    public static BookTypeConf completeBookTypeConf(
	BookTypeConf bookTypeConf, AuthenticationUser user, AxSf axsf, ScrOfic scrOfic,
	Locale locale) {
	if (bookTypeConf != null) {
	    bookTypeConf.setStringBookType(axsf.getLocaleAttributeLiteralTypeBook(
		locale, new Integer(
		    1)));
    ...
    private static String[] createDistribute(
	Session session, AuthenticationUser user, String sessionID, Integer bookId,
	Integer folderID, Integer senderType, Integer senderId, Integer userId, Integer userType,
	ScrOfic scrOfic, int stateReg, String messageForUser, AxSf axsf, String entidad)
	throws ValidationException, SessionException, DistributionException, HibernateException,
	BookException, SQLException, Exception {
	...
	if (stateReg == ISicresKeys.SCR_ESTADO_REGISTRO_COMPLETO ||
		...
		notDistributedRegisters
		    .append(" :El registro no puede tener distribuciones pendientes o reenviadas para iniciar una distribución nueva.");
		notDistributedRegisters.append(";");
	    }
	    else {
		...
		Integer distId =
		    insertDistribute(
			session, bookId, folderID.intValue(), userType.intValue(),
			userId.intValue(), currentDate, senderType, senderId, user.getName(),
			user.getId(), 1, messageForUser, entidad);
		if (distId.intValue() != 0) {
			...
			!eventsManagerClassName.equals("")) {
			field =
			    (String) EventsSession.distributionEvent(
				sessionID, DISTRIBUTION_MANUAL_EVENT, bookId, folderID, axsf,
		    ...
		    notDistributedRegisters.append(axsf.getAttributeValue("fld1"));
		    notDistributedRegisters
			.append(" :El registro no puede hacer la misma distribución si la antigua esté en pendiente, aceptada o archivada.");
		    notDistributedRegisters.append(";");
		}
	    }
	}
	    ...
	    notDistributedRegisters.append(axsf.getAttributeValue("fld1"));
	    notDistributedRegisters
		.append(" :El registro debe estar en estado completo para poder ser distribuido.");
	    notDistributedRegisters.append(";");
	}
    ...
    private static String createDistributionResponse(
	String field, String distributedRegisters, String notDistributedRegisters) {
	StringBuffer resp = new StringBuffer();
	if (field != null &&
	    !distributedRegisters.equals("")) {
	    resp.append(field);
	    resp.append("#");
	...
	if (!notDistributedRegisters.toString().equals(
		...
		DistributionException.ERROR_REGISTERS_NOT_DISTRIBUTED).getMessage()+";");
	    resp.append(" ");
    ...
    private static void createNewRegisterDocs(
	String sessionID, AuthenticationUser user, Integer idBook, ScrDistreg scrDistReg,
	int registerId, Timestamp timestamp, MiscFormat miscFormat, BookTypeConf bookTypeConf,
	BookConf bookConf, int imageAuth, String entidad)
	throws DistributionException, SessionException, Exception {

	List docs = FolderFileSession.getBookFolderDocsWithPages(
	    sessionID, new Integer(
		scrDistReg.getIdArch()), scrDistReg.getIdFdr(), entidad);
	...
	for (Iterator it1 = docs.iterator(); it1.hasNext();) {
	    ...
	    int docID = DBEntityDAOFactory.getCurrentDBEntityDAO().getNextDocID(
		bookIdIn, entidad);
	    AxPKById docPk = new AxPKById(
		bookIdIn.toString(), registerId, docID);
	    AxDochEntity axDochEntity = new AxDochEntity();
	    axDochEntity.create(
		docPk, user.getId().intValue(), axDoch.getName(), timestamp, entidad);
		...
		if ((axPageh.getLoc().toUpperCase().equals(
		    "TIF") || axPageh.getLoc().toUpperCase().equals(
		    "TIFF")) &&
		    ...
		    ISRepositorySignDocumentVO signVO =
			ISRepositoryDocumentHelper.getRepositorySingDocumentVO(
			    scrDistReg.getIdArch(), scrDistReg.getIdFdr(), axPageh.getDocId(),
			    axPageh.getId(), miscFormat.getId(), bookTypeConf, bookConf, entidad,
			    true);

		    RepositoryFactory.getCurrentPolicy().signDocument(
			signVO);
		...
		int order = axPagehEntity.getOrderID(
		    bookIdIn, registerId, docID, entidad);

		AxXnidEntity axXnidEntity = new AxXnidEntity();
		int pageID = axXnidEntity.getNextID(
		    bookIdIn, registerId, entidad);

		AxPKById pagePk = new AxPKById(
		    bookIdIn.toString(), registerId, pageID);

		axPagehEntity.create(
		    pagePk, user.getId().intValue(), axPageh.getName(), order++, docID,
		    axPageh.getFileId(), axPageh.getLoc(), timestamp, entidad);
		DBEntityDAOFactory.getCurrentDBEntityDAO().insertScrSharedFiles(
		    axPageh.getFileId(), scrDistReg.getIdArch(), scrDistReg.getIdFdr(),
		    bookIdIn.intValue(), registerId, entidad);

		// REFERENCIA AL DOCUMENTO EN SCR_PAGEREPOSITORY CON LOS NUEVOS
		// BOOKID, FDRID, PAGEID
		ISRepositoryRetrieveDocumentVO findVO =
		    ISRepositoryDocumentHelper.getRepositoryRetrieveDocumentVO(
			new Integer(
			    scrDistReg.getIdArch()), new Integer(
			    scrDistReg.getIdFdr()), new Integer(
			    axPageh.getId()), entidad, true);

		ISRepositoryRetrieveDocumentVO retrieveVO =
		    RepositoryFactory.getCurrentPolicy().getDocUID(
			findVO);

		String docUID = retrieveVO.getDocumentUID();

		DBEntityDAOFactory.getCurrentDBEntityDAO().insertScrPageRepository(
		    bookIdIn.intValue(), registerId, pageID, docUID, entidad);

		generateHashInOut(
		    sessionID, scrDistReg.getIdArch(), scrDistReg.getIdFdr(), bookIdIn.intValue(),
		    registerId, axPageh.getId(), pageID, axPageh.getFileId(), isSign, entidad);
	    }
	}
    }

    private static void generateHashInOut(
	String sessionID, int bookIdOut, int fdrIdOut, int bookIdIn, int fdrIdIn, int pageIdOut,
	...
	try {
		...
		ISRepositoryDocumentHelper.getRepositoryRetrieveDocumentVO(
		    new Integer(
			bookIdOut), new Integer(
			fdrIdOut), new Integer(
			pageIdOut), entidad, true);
		...
		ISRepositoryRetrieveDocumentVO documentRetrievedVO =
		    RepositoryFactory.getCurrentPolicy().retrieveDocument(
			findVO);
		String findHash = DBEntityDAOFactory.getCurrentDBEntityDAO().getHashDocument(
		    new Integer(
			bookIdOut), fdrIdOut, pageIdOut, null, true, entidad);
		...
		ISicresSaveQueries.saveScrPageInfo(
		    session, new Integer(
     ...
     * @throws Exception
     */
    public static List getNewRegisterInterList(
	Integer bookId, int fdrid, String entidad)
	throws Exception {

	List aScrRegInt = DBEntityDAOFactory.getCurrentDBEntityDAO().getScrRegisterInter(
	    bookId, fdrid, true, entidad);
    ...
    public static MiscFormat getMiscFormat(
	Session session, Integer idBook, Map outputInputBookType)
	throws HibernateException {
	MiscFormat miscFormat = null;
	if (!outputInputBookType.isEmpty()) {
	    THashMap bookInformation = new THashMap(
		10);
	    getIdocarchdet(
		session, idBook, bookInformation);
	    Idocarchdet idocMisc =
		(Idocarchdet) bookInformation.get(IDocKeys.IDOCARCHDET_MISC_ASOBJECT);
	    miscFormat = new MiscFormat(
		idocMisc.getDetval());
	}
    ...
    protected static Integer getDistributionResultDistType(
	Session session, ScrDistreg distReg, String entidad)
	throws HibernateException {
	if (Repository.getInstance(
		...
		ScrOfic scrOficIdDest = ISicresQueries.getScrOficByDeptId(
			...
			distReg.getIdDest()));
		if ((scrOficIdDest == null) ||
		    (scrOficIdOrig.getScrOrg().getId() != scrOficIdDest.getScrOrg().getId())) {
		    return new Integer(
			1);
		}
		else {
    ...
    protected static String getDistributionResultId(
	ScrDistreg distReg, int typeDist, String entidad, boolean isLdap)
	throws HibernateException {
	Integer type = null;
	String id = null;
	...
	if (type.intValue() == 1) {
	    String idAux = getScrUsrIdentInfo(
		id, entidad);
	    if (idAux.equals("")) {
		id = getIUserNameorRemarksInfo(
		    isLdap, id, entidad);
	    }
	    else {
		id = idAux;
	    ...
	    id = getIUserDeptHdrInfo(
		id, entidad);
	}
	else if (type.intValue() == 3) {
	    id = getIUserGroupInfo(
		isLdap, id, entidad);
	}
    ...
    private static boolean isPropOrg(
	Session session, AxSf axsf)
	throws HibernateException {
	boolean launchDistribution = false;
	ScrOrg scrorg = null;
	if (axsf.getAttributeValue("fld8") != null) {
	    scrorg = (ScrOrg) session.load(
		ScrOrg.class, new Integer(
		    axsf.getAttributeValueAsString("fld8")));
	}
	launchDistribution = scrorg != null &&
	    scrorg.getScrTypeadm() != null && scrorg.getScrTypeadm().getId().equals(
		new Integer(
		    1));
    ...
    // autenticacion
    private static String getIUserNameorRemarksInfo(
	boolean isLdap, String id, String entidad)
	throws HibernateException {
	String result = "";
	if (isLdap) {
	    result = getIUserLdapUserHdrInfo(
		id, entidad);
	}
	else {
	    result = getIUserUserHdrInfo(
		id, entidad);
	}
	return result;
    }

    private static String getIUserLdapUserHdrInfo(
	String id, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
		...
		Iuserldapuserhdr.class, new Integer(
		    id));
    ...
    private static String getIUserGroupInfo(
	boolean isLdap, String id, String entidad)
	throws HibernateException {
	String result = "";
	if (isLdap) {
	    result = getIUserLdapGroupHdrInfo(
		id, entidad);
	}
	else {
	    result = getIUserGroupHdrInfo(
		id, entidad);
	}
	return result;
    }

    private static String getIUserLdapGroupHdrInfo(
	String id, String entidad)
	throws HibernateException {
	String result = "";
	HibernateUtil hibernateUtil = new HibernateUtil();
	try {
		...
		Iuserldapgrphdr.class, new Integer(
		    id));
