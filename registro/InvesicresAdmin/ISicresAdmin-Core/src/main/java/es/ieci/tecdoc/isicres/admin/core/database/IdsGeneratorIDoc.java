	...
	public static int generateNextId(int tabla, String entidad)
		...
		try {
			...
			if (rs.next()) {
				...
				id = 1;
				logger.warn("No se ha encontrado el contador para la tabla '" + tabla + "', se inserta en IDOCNEXTID y se devuelve el valor 1");
			...
			try {
				...
				if(db!=null && db.existConnection()) db.close();
			} catch (Exception e) {
