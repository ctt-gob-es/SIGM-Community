...
public class DefinicionFilterOficina extends DefinicionFilterField{

	String entidad;
	public DefinicionFilterOficina(int tipoLibro, String entidad) {
		super(tipoLibro);
	...
	public String encodeFilter(FiltroImpl filtro) {
		...
		sb.append("|");
		try {
			SicresOficinaImpl oficina = ISicresRPAdminOficinaManager.getOficinaByCode(filtro.getValor(), entidad);
			sb.append(oficina.getId());
		} catch (Exception e) {
			sb.append("0");
		}
		sb.append("|");
	...
	public String encodeWhere(FiltroImpl filtro, String entidad) {
		...
		if(filtro.getOperador()==FiltroImpl.OPERADOR_IGUAL_A) {
			try {
				SicresOficinaImpl oficina = ISicresRPAdminOficinaManager.getOficinaByCode(filtro.getValor(), entidad);
				sb.append(" = ").append(oficina.getId());
			} catch (Exception e) {
				sb.append(" = 0");
			}
		}
		else if(filtro.getOperador()==FiltroImpl.OPERADOR_EMPIEZA_POR) {
			sb.append(" IN (SELECT ID FROM SCR_OFIC WHERE CODE LIKE '").append(filtro.getValor()).append("%')");
		}
