...
<script>
	...
	function chequearSessionBuscarUsuarios( url ) {
	...
	function chequearSessionBuscarUsuariosLdap( url ) {
		var check = chequearSession('<html:rewrite page="/chequearSession.do"/>');
		if( check == "false") {
			abreUsuariosLdap( url );
		} else {
			window.document.location.href = urlSessionExpired;
		}
	}
	...
	function selectTreeNode(nombre, id){
  		window.document.forms[0].nombre.value = nombre;
  		window.document.forms[0].treeId.value = id;
	}
...
<body onload="init()">
<html:form action="/nuevoUsuario.do">
<html:hidden property="tipoUsuario"/>
<html:hidden property="treeId" styleId="treeId"/>
<div id="contenedora">
  <!-- Inicio Cabecera -->
          ...
          <div class="cuadro">
			...
			<div class="col" align="right">
				...
				<tr>
					...
					<td class="col_guardar" onclick="llamadaAction('<html:rewrite page="/guardarNuevoUsuario.do"/>')"><bean:message key="ieci.tecdoc.sgm.rpadmin.botones.guardar"/></td>
					<td class="col_eliminar" onclick="llamadaAction('<html:rewrite page="/listadoUsuarios.do"/>')"><bean:message key="ieci.tecdoc.sgm.rpadmin.botones.cancelar"/></td>
				</tr>
				</table>
			</div>
				...
				<div style="height: 200px;">
					...
					<table cellpadding="0" cellspacing="0" border="0">
						...
						<tr class="col">
							...
							<td>
								<html:select property="idPerfil" styleId="idPerfil" styleClass="textInput" onchange="checkSuperUser(this.value, 'NUEVO', false)">
