				 ...
				 requestURI="/listadoUnidadHijos.do" class="tablaListado" sort="page" style="width:97%" >
					...
					<display:column style="width: 25px;">
						<table cellpadding="0" cellspacing="0" border="0">
							<tr>
								<td>
									<a href="#" onclick="chequearIdUnidad('<html:rewrite page="/editarUnidad.do"/>','<bean:write name="fila" property="id" />')"><img src="<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/ico_edit.gif" alt='<bean:message key="ieci.tecdoc.sgm.rpadmin.botones.editar"/>' border="0"/></a>
								</td>
							</tr>
						</table>
					</display:column>
						...
						<table cellpadding="0" cellspacing="0" border="0">
							<tr>
								<td>
									...
									<a href="#" onclick="<c:out value="${funcionchequearIdUnidadEliminar}"/>" ><img src='<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/ico_delete.gif' alt='<bean:message key="ieci.tecdoc.sgm.rpadmin.botones.eliminar"/>' border="0"/></a>
								</td>
							</tr>
						</table>
					</display:column>
					...
					<display:column style="width: 25px;">
						<table cellpadding="0" cellspacing="0" border="0">
							<tr>
								<td>
								 <a href="#" onclick="chequearIdDistribucion('<html:rewrite page="/listadoDistribucionUnidad.do"/>','<bean:write name="fila" property="id" />')"><img src='<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/ico_distrib.gif' alt='<bean:message key="ieci.tecdoc.sgm.rpadmin.botones.distribucion"/>' border = "0"/></a>
								</td>
							</tr>
						</table>
					</display:column>
