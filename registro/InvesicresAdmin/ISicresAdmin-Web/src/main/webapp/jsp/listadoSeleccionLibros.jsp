...
<script>
	...
	function guardar() {
		...
		window.opener.addLibro(id, codigo, nombre);
		window.close();
	}
	...
	<tr>
		<td valign="top">
			...
			<table cellpadding="0" cellspacing="0" border="0" width="100%">
				...
				<tr>
					<td>
						<div id="tablaListas" style="overflow:auto; width:320px;">
						<display:table name="listaLibros" uid="fila" requestURI="/listadoSeleccionLibros.do" class="tablaListado" sort="page">
							...
							<display:column titleKey="ieci.tecdoc.sgm.rpadmin.oficinas.codigo" sortable="false" style="width: 15%;">
								<bean:write name="fila" property="id"/>
								<input type="hidden" name="ids" id='ids_<bean:write name="fila_rowNum"/>' value ='<bean:write name="fila" property="id"/>'/>
								<input type="hidden" name="codigos" id='codigos_<bean:write name="fila_rowNum"/>' value ='<bean:write name="fila" property="id"/>'/>
							</display:column>
							<display:column titleKey="ieci.tecdoc.sgm.rpadmin.oficinas.nombre" sortable="false">
								<bean:write name="fila" property="nombre"/>
								<input type="hidden" name="nombres" id='nombres_<bean:write name="fila_rowNum"/>' value='<bean:write name="fila" property="nombre"/>'/>
							</display:column>
					</display:table>
						</div>
					</td>
