  ...
  <logic:equal value="true" scope="session" name="isSuperuser">
    <logic:equal parameter="pestana" value="oficinas">
      <li class="submen1on">
        ...
        <img src="<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/subme2_<c:out value="${imageOn}" />.gif" />
        <bean:message key="ieci.tecdoc.sgm.rpadmin.botones.oficinas"/>
        <img src="<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/subme2_on_0_bis.gif" />
      </li>
    </logic:equal>
    <logic:notEqual parameter="pestana" value="oficinas">
      <li class="submen1off">
        <img src="<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/subme2_<c:out value="${imageOff}" />.gif" />
        <a href="<html:rewrite page='/listadoOficinas.do'/>"><bean:message key="ieci.tecdoc.sgm.rpadmin.botones.oficinas"/></a>
        <img src="<%=((Context) new InitialContext().lookup("java:comp/env")).lookup("URLRoot")%>/img/subme2_of_0.gif" />
      </li>
    </logic:notEqual>
