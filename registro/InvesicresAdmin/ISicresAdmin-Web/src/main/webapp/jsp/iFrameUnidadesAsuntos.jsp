			...
			<script>
				...
				function eventExpand(id){
					if((id!=null)&&(id!="undefined")){
			    	    ...
			    	    if( check == "false" ) {
							document.location.href = '<html:rewrite page="/cargarUnidadesAsuntos.do"/>' + '?id='+id+"&idCampoFiltro="+idCampoFiltro;
				    	} else {
		    	    ...
		    	    if( check == "false" ) {
		    	    	if( idCampoFiltro != "null" && idCampoFiltro != "") {
		    	    	  ...
		    	    	  window.parent.nombreUnidad = nombreUnidad;
		    	 	   	  ...
		    	 	   	  window.top.opener.asignarUnidadAdministrativa(id,codigo,nombre);
						window.top.close();
		    	 	   	  //llamaActionIFrame('<html:rewrite page="/listadoUnidad.do"/>', id, tipo);
		    	 	    }
		    	    } else {
			    		window.parent.redirect('<%=AutenticacionAdministracion.obtenerUrlLogin(request, ConstantesGestionUsuariosAdministracion.APLICACION_REGISTRO) %>');
			    	}
