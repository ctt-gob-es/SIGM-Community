...
public class CargarDepartamentosImportarUnidadAction extends RPAdminWebAction {
	private static final Logger logger = Logger.getLogger(CargarDepartamentosImportarUnidadAction.class);
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form,
		...
		ServiceTree serviceTree = request.getSession(false).getAttribute("serviceTreeDepartamentosNuevaOficina") != null ?
								  (ServiceTree)request.getSession(false).getAttribute("serviceTreeDepartamentosNuevaOficina") : null;
        ...
        if (id == null ) {
            ...
            nodoRaiz.setTitle(rb.getString("ieci.tecdoc.sgm.rpadmin.unidades.distribucion.departamentos"));
            if (hasChild)
                serviceTree.addChildren(nodoRaiz.getId(), oServicio, entidad, Distribucion.TIPO_DEPARTAMENTO, false);

            borraCookies = true;
            request.getSession(false).setAttribute("serviceTreeDepartamentosNuevaOficina", serviceTree);
        }
        else
