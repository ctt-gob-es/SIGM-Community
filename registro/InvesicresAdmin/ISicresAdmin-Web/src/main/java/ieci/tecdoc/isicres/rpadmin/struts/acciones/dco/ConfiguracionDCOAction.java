...
public class ConfiguracionDCOAction extends RPAdminWebAction {
	...
	public ActionForward executeAction(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boolean superUser = (Boolean) request.getSession().getAttribute("isSuperuser");
		...
		if(superUser){
			try{
				...
				int numeroOficinas = oServicio.countOficinasDirectorioComun(criterios);
			...
			ActionError error = new ActionError("ieci.tecdoc.rpadmin.dco.error.user.not.permissions");
