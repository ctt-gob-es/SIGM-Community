...
public class ServicioObtenerInicializacionDCOWSClientImpl implements
	...
	private static final Logger LOGGER = Logger.getLogger(ServicioObtenerInicializacionDCOWSClientImpl.class);
	...
	public void initServices() throws MalformedURLException {
		if (servicioVolcadoUnidades == null) {
			...
			QName serviceNameVU = new QName("http://impl.manager.directorio.map.es", "SC01UN_VolcadoDatosBasicosService");
			servicioVolcadoUnidades = new SC01UNVolcadoDatosBasicosService(wsdlLocationVU, serviceNameVU);
		}
		if (servicioVolcadoOficinas == null) {
			URL wsdlLocationVO = new URL(servicioVolcadoOficinasURL);
			QName serviceNameVO = new QName("http://impl.manager.directorio.map.es", "SC11OF_VolcadoDatosBasicosService");
			servicioVolcadoOficinas = new SC11OFVolcadoDatosBasicosService(wsdlLocationVO, serviceNameVO);
		}
	}
	...
	public String getFicheroInicializarOficinasDCO() {
		...
		try {
			...
			RespuestaWS respuesta = getServicioVolcadoOficinas().getSC11OFVolcadoDatosBasicos().exportar(
					getLogin(), 
					getPass(), 
					getFileFormat(), 
	...
	public String getFicheroInicializarUnidadesDCO() {
		...
		try {
			initServices();
			tempZipFile = File.createTempFile("dec", ".zip", new File(getTempFilesDir()));
			RespuestaWS respuesta = getServicioVolcadoUnidades().getSC01UNVolcadoDatosBasicos().exportar(getLogin(), getPass(), getFileFormat(), getUnidadesQueryType(), "", "", "", "", "", "", "", "", "");
			Base64Utils.getInstance().decodeToFile(respuesta.getFichero(), tempZipFile.getAbsolutePath());
			List<String> filesUnzipped = ZipUtils.getInstance().unzipFile( tempZipFile.getAbsolutePath(), getTempFilesDir());
