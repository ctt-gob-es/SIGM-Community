...
<head>
	...
	<c:set var="numExp" value="${requestScope.numExp}"/>
	<jsp:useBean id="numExp" type="java.lang.String"/>
	...
	<script type="text/javascript">
		...
		function loadReceivers(numExp){
			RegisterAPI.getReceivers(numExp, function(data) {
				if (data) {
					DWRUtil.removeAllOptions("receivers");
					DWRUtil.addOptions("receivers", data, "key", "value");
				}
			})
		}
		...
		function showMsgInProgress(){
			...
			document.getElementById('receivers').disabled=true;
		}		
			...
			<div class="cuerpo_ficha">
				<div class="seccion_ficha">
					...
					<html:form action="insertGroupedOutputRegistry.do" method="post">
						...
						<p>
							<label class="popUp"><nobr><bean:message key="registro.salida.form.field.destino"/>:</nobr></label>
							<html:select styleId="receivers" property="property(DESTINO_ID)">
							</html:select>
						</p>
