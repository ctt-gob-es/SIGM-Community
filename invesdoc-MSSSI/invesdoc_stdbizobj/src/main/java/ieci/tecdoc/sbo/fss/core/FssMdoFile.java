		            ...
		            if (cfgMdoFile != null) 
		            {
		               ...
		               if (listConfig.size() > 0) 
		               {
		                  ...
		                  for (int i = 0; i < listConfig.size (); i++) 
		                  {
		                     ...
		                     if(config.getPath() != null && config.getPath().indexOf("sigem.path") !=-1){
		                    	 
		                    	 String pathVar = (String)System.getProperty("sigem.path");
		                    	 
		                    	 CfgMdoFile.RepositoryConfig configAux = new CfgMdoFile.RepositoryConfig(config.getId(),config.getPath().replace("${sigem.path}", pathVar),config.getOperatingSystem());
		                    	 
		                    	 configurations.put (new Integer (configAux.getId()), configAux);
		                     }else
