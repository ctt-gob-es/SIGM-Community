...
public class DaoArchHdrTbl
{
   ...
   private static final String TN = "IDOCARCHHDR";
  ...
  (CD_ACCESSTYPE, CD_ACSID);
   ...
   public static void selectChildrenRows(int parentDirId, DaoOutputRows rows) 
                                        throws Exception
   {  
   ...
   { 
      ...
      name = DbSelectFns.selectShortText(TN, CD_NAME.getName(),
                                         getDefaultQual(id));

      return name;
   }
   
   public static String selectTblPrefix(int id) throws Exception
   { 
      String name;
      
      name = DbSelectFns.selectShortText(TN, CD_TBLPREFIX.getName(),
                                         getDefaultQual(id));

      return name;
   }
   
   public static Idoc8DbAcsRec selectAcsInfo(int id)
                               throws Exception
   {
      
      Idoc8DbAcsRec rec = new Idoc8DbAcsRec();  

      DbSelectFns.select(TN, ACSCN, getDefaultQual(id), rec);
